...
#yangx add
    itk_option define -tabIndex tabIndex TabIndex 0
    private common s_nameList [list A B C D E F G H I J K L M N O P]
...
#yangx add
#	-tabIndex optoin is being set in the file RobotView.tcl 
#	 itk_component add left_cas {
#		DCSCassetteView $LeftSite.cv -tabIndex 0 -purpose forMount \
#		-offset 0 -systemIdleOnly 0 -activeClientOnly 0 -onClick "$this setValue"
#	 } ..........

#	it will call configbody ITKCassetteView::tabIndex() method
#	so all the follwing statement will be added in this method.

#	puts "yangx tabIndex = $itk_option(-tabIndex)"
#	set name_A_index [expr $itk_option(-tabIndex) * 4]
#	set name_B_index [expr $name_A_index + 1]
#	set name_C_index [expr $name_A_index + 2]
#	set name_D_index [expr $name_A_index + 3]
#	puts "name_A_index =$name_B_index " 
#	$itk_component(puckA) configure -name [lindex $s_nameList $name_A_index]	
#	$itk_component(puckB) configure -name [lindex $s_nameList $name_B_index]
#	$itk_component(puckC) configure -name [lindex $s_nameList $name_C_index]
#	$itk_component(puckD) configure -name [lindex $s_nameList $name_D_index]
...
#yangx add
    set A_index [expr $A_index * 4]
    set B_index [expr $A_index + 1]
    set C_index [expr $A_index + 2]
    set D_index [expr $A_index + 3]
 
    set AA [lindex $s_nameList $A_index]
    set BB [lindex $s_nameList $B_index]
    set CC [lindex $s_nameList $C_index]
    set DD [lindex $s_nameList $D_index]
    ...
    -portNamePrefix ${m_cassetteName}$AA \
    -offset $offsetPuckA

    $itk_component(puckB) config \
    -portNamePrefix ${m_cassetteName}$BB \
    -offset $offsetPuckB

    $itk_component(puckC) config \
    -portNamePrefix ${m_cassetteName}$CC \
    -offset $offsetPuckC

    $itk_component(puckD) config \
    -portNamePrefix ${m_cassetteName}$DD \
    -offset $offsetPuckD
...
configbody ITKCassetteView::tabIndex {

#	    puts "yangx itk_option(-tabIndex) = $itk_option(-tabIndex)"
	    set name_A_index [expr $itk_option(-tabIndex) * 4]
            set name_B_index [expr $name_A_index + 1]
            set name_C_index [expr $name_A_index + 2]
            set name_D_index [expr $name_A_index + 3]
...
body DCSCassetteView::handleStringStatusEvent { stringName_ targetReady_ alias_ contents_ - } {
    ...
    set ct [lrange $contents_ $start $end]

#puts "xyang DCS start=$start end=$end content=$ct"
