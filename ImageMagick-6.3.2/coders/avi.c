...
#define BI_RLE8  1
  ...
  for (y=0; y < (long) image->rows; )
  {
    ...
    if (count != 0)
      {
        count=(ssize_t) MagickMin(count,(size_t) (q-p));
        /*
          Encoded mode.
        */
        ...
        switch (count)
        {
          ...
          default:
          {
            /*
              Absolute mode.
            */
            count=(long) MagickMin(count,(size_t) (q-p));
            if (compression == BI_RLE8)
              for (i=0; i < (long) count; i++)
        ...
        if (LocaleCompare(bmp_info.compression,"MJPG") == 0)
          {
            ...
            */
            ...
            (void) ThrowMagickException(exception,GetMagickModule(),
              CorruptImageError,message,"`%s'",image->filename);
            for ( ; chunk_size != 0; chunk_size--)
              (void) ReadBlobByte(image);
            continue;
#endif
        ...
        */
        else
        {
          ...
          */
          ...
          status=AllocateImageColormap(image,
            number_colors != 0 ? number_colors : 256);
          if (status == MagickFalse)
            ThrowReaderException(ResourceLimitError,"MemoryAllocationFailed");
          if (number_colors != 0)
        ...
        if (LocaleCompare(stream_info.data_type,"vids") == 0)
          {
            ...
            if (number_colors != 0)
              {
                colormap=(PixelPacket *) AcquireMagickMemory((size_t)
                  number_colors*sizeof(PixelPacket));
