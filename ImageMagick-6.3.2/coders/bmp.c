    ...
    */
    ...
    switch (bmp_info.compression)
    {
      ...
      case BI_BITFIELDS:
        break;
      case BI_JPEG:
        ThrowReaderException(CoderError,"JPEGCompressionNotSupported");
      case BI_PNG:
        ThrowReaderException(CoderError,"PNGCompressionNotSupported");
      default:
        ThrowReaderException(CorruptImageError,"UnrecognizedImageCompression");
    }
    image->columns=(unsigned long) bmp_info.width;
...
static MagickBooleanType WriteBMPImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  do
  {
    ...
    */
    pixels=(unsigned char *) AcquireMagickMemory((size_t) bmp_info.image_size);
    if (pixels == (unsigned char *) NULL)
      ...
      if (image->compression != NoCompression)
        {
          ...
          */
          ...
          bmp_data=(unsigned char *) AcquireMagickMemory(length+bytes_per_line);
          ...
          bmp_info.file_size-=bmp_info.image_size;
          bmp_info.image_size=(unsigned long)
    ...
    if (image->storage_class == PseudoClass)
      {
        ...
        */
        ...
        if (bmp_colormap == (unsigned char *) NULL)
          ThrowWriterException(ResourceLimitError,"MemoryAllocationFailed");
        q=bmp_colormap;
        for (i=0; i < (long) MagickMin(image->colors,bmp_info.number_colors); i++)
        {
