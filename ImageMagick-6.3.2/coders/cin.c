...
#define RGBColorType  3
  ...
  */
  ...
  for (i=0; i < 4; i++)
    (void) ReadBlobByte(image);
  image->depth=(unsigned long) ReadBlobByte(image);
  (void) ReadBlobByte(image);
...
static MagickBooleanType WriteCINImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  (void) CopyMagickString(filename,image->filename,100);
  (void) WriteBlobString(image,filename);
  for (i=0; i < (long) (100-strlen(filename)); i++)
  ...
  (void) WriteBlobByte(image,(unsigned char) image->depth);  /* Bits per pixel... */
  ...
  (void) WriteBlobByte(image,(unsigned char) image->depth);
  (void) WriteBlobByte(image,0);
