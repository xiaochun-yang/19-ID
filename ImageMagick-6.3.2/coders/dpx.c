...
static Image *ReadDPXImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  (void) ReadBlob(image,sizeof(dpx.file.filename),(unsigned char *)
    dpx.file.filename);
  ...
  (void) ReadBlob(image,sizeof(dpx.file.timestamp),(unsigned char *)
    dpx.file.timestamp);
  if (*dpx.file.timestamp != '\0')
    (void) FormatImageProperty(image,"dpx:file.tilestamp","%.24s",
      dpx.file.timestamp);
  (void) ReadBlob(image,sizeof(dpx.file.creator),(unsigned char *)
    dpx.file.creator);
  ...
  (void) ReadBlob(image,sizeof(dpx.file.project),(unsigned char *)
    dpx.file.project);
  ...
  (void) ReadBlob(image,sizeof(dpx.file.copyright),(unsigned char *)
    dpx.file.copyright);
  ...
  (void) ReadBlob(image,sizeof(dpx.file.reserve),(unsigned char *)
    dpx.file.reserve);
  /*
  ...
  */
  ...
  for (i=0; i < 8; i++)
  {
    ...
    (void) ReadBlob(image,sizeof(single),single);
    ...
    (void) ReadBlob(image,sizeof(dpx.image.image_element[i].description),
  ...
  (void) ReadBlob(image,sizeof(dpx.image.reserve),(unsigned char *)
    dpx.image.reserve);
  ...
  if (dpx.file.image_offset >= 1664UL)
    {
      ...
      */
      ...
      (void) ReadBlob(image,sizeof(dpx.orientation.reserve),(unsigned char *)
  ...
  if (dpx.file.image_offset >= 1920UL)
    {
      ...
      */
      (void) ReadBlob(image,sizeof(dpx.film.id),(unsigned char *) dpx.film.id);
      if (*dpx.film.type != '\0')
        (void) FormatImageProperty(image,"dpx:film.id","%.2s",dpx.film.id);
      (void) ReadBlob(image,sizeof(dpx.film.type),(unsigned char *)
        dpx.film.type);
      if (*dpx.film.type != '\0')
        (void) FormatImageProperty(image,"dpx:film.type","%.2s",dpx.film.type);
      (void) ReadBlob(image,sizeof(dpx.film.offset),(unsigned char *)
        dpx.film.offset);
      if (*dpx.film.offset != '\0')
        (void) FormatImageProperty(image,"dpx:film.offset","%.2s",
          dpx.film.offset);
      (void) ReadBlob(image,sizeof(dpx.film.prefix),(unsigned char *)
        dpx.film.prefix);
      if (*dpx.film.prefix != '\0')
        (void) FormatImageProperty(image,"dpx:film.prefix","%.6s",
          dpx.film.prefix);
      (void) ReadBlob(image,sizeof(dpx.film.count),(unsigned char *)
        dpx.film.count);
      if (*dpx.film.count != '\0')
        (void) FormatImageProperty(image,"dpx:film.count","%.4s",
          dpx.film.count);
      (void) ReadBlob(image,sizeof(dpx.film.format),(unsigned char *)
        dpx.film.format);
      ...
      (void) ReadBlob(image,sizeof(dpx.film.frame_id),(unsigned char *)
        dpx.film.frame_id);
      if (*dpx.film.frame_id != '\0')
        (void) FormatImageProperty(image,"dpx:film.frame_id","%.32s",
          dpx.film.frame_id);
      (void) ReadBlob(image,sizeof(dpx.film.slate),(unsigned char *)
        dpx.film.slate);
      if (*dpx.film.slate != '\0')
        (void) FormatImageProperty(image,"dpx:film.slate","%.100s",
          dpx.film.slate);
      (void) ReadBlob(image,sizeof(dpx.film.reserve),(unsigned char *)
        dpx.film.reserve);
    }
  if (dpx.file.image_offset >= 2048UL)
    {
      ...
      */
      ...
      (void) ReadBlob(image,sizeof(dpx.television.reserve),(unsigned char *)
  ...
  if (dpx.file.image_offset >= 2080UL)
    {
      ...
      */
      (void) ReadBlob(image,sizeof(dpx.user.id),(unsigned char *) dpx.user.id);
      if (*dpx.user.id != '\0')
  ...
  */
  ...
  if ((quantum_type == GrayPadQuantum) && (image->depth == 10))
    {
      unsigned long
        pixel;

      /*
        Cineon 10-bit pixel format.
      */
      pixel=0UL;
      i=0;
        ...
        for (x=0; x < (long) image->columns; x++)
        {
          switch (i % 3)
          {
            case 0:
            {
              ...
              q->red=(Quantum) ((MagickRealType) QuantumRange*((pixel >> 0) &
                0x3ff)/1023+0.5);
              break;
            }
            case 1:
            {
              q->red=(Quantum) ((MagickRealType) QuantumRange*((pixel >> 10) &
                0x3ff)/1023+0.5);
              break;
            }
            case 2:
            {
              q->red=(Quantum) ((MagickRealType) QuantumRange*((pixel >> 20) &
                0x3ff)/1023+0.5);
              break;
            }
          }
...
static MagickBooleanType WriteDPXImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  (void) WriteBlob(image,sizeof(dpx.file.filename),(unsigned char *)
    dpx.file.filename);
  seconds=time((time_t *) NULL);
  (void) FormatMagickTime(seconds,sizeof(dpx.file.timestamp),
    dpx.file.timestamp);
  (void) WriteBlob(image,sizeof(dpx.file.timestamp),(unsigned char *)
    dpx.file.timestamp);
  (void) CopyMagickString(dpx.file.creator,GetMagickVersion((unsigned long *)
    NULL),sizeof(dpx.file.creator));
  (void) WriteBlob(image,sizeof(dpx.file.creator),(unsigned char *)
    dpx.file.creator);
  value=GetImageProperty(image,"dpx:file.project");
  if (value != (const char *) NULL)
    (void) CopyMagickString(dpx.file.project,value,sizeof(dpx.file.project));
  (void) WriteBlob(image,sizeof(dpx.file.project),(unsigned char *)
    dpx.file.project);
  value=GetImageProperty(image,"dpx:file.copyright");
  if (value != (const char *) NULL)
    (void) CopyMagickString(dpx.file.copyright,value,
      sizeof(dpx.file.copyright));
  (void) WriteBlob(image,sizeof(dpx.file.copyright),(unsigned char *)
    dpx.file.copyright);
  dpx.file.encrypt_key=(~0UL);
  (void) WriteBlobLong(image,dpx.file.encrypt_key);
  (void) WriteBlob(image,sizeof(dpx.file.reserve),(unsigned char *)
    dpx.file.reserve);
  /*
    Write image header.
  */
  ...
  for (i=0; i < 8; i++)
  {
    ...
    (void) WriteBlobLong(image,dpx.image.image_element[i].end_of_image_padding);
    (void) WriteBlob(image,sizeof(dpx.image.image_element[i].description),
      (unsigned char *) dpx.image.image_element[i].description);
  }
  (void) WriteBlob(image,sizeof(dpx.image.reserve),(unsigned char *)
    dpx.image.reserve);
  /*
  ...
  */
  ...
  value=GetImageProperty(image,"dpx:orientation.filename");
  if (value != (const char *) NULL)
    (void) CopyMagickString(dpx.orientation.filename,value,
      sizeof(dpx.orientation.filename));
  (void) WriteBlob(image,sizeof(dpx.orientation.filename),(unsigned char *)
    dpx.orientation.filename);
  (void) WriteBlob(image,sizeof(dpx.orientation.timestamp),(unsigned char *)
    dpx.orientation.timestamp);
  (void) WriteBlob(image,sizeof(dpx.orientation.device),(unsigned char *)
    dpx.orientation.device);
  (void) WriteBlob(image,sizeof(dpx.orientation.serial),(unsigned char *)
    dpx.orientation.serial);
  for (i=0; i < 4; i++)
  {
  ...
  (void) WriteBlob(image,sizeof(dpx.orientation.reserve),(unsigned char *)
    dpx.orientation.reserve);
  /*
    Write film header.
  */
  (void) WriteBlob(image,sizeof(dpx.film.id),(unsigned char *) dpx.film.id);
  (void) WriteBlob(image,sizeof(dpx.film.type),(unsigned char *) dpx.film.type);
  (void) WriteBlob(image,sizeof(dpx.film.offset),(unsigned char *)
    dpx.film.offset);
  (void) WriteBlob(image,sizeof(dpx.film.prefix),(unsigned char *)
    dpx.film.prefix);
  (void) WriteBlob(image,sizeof(dpx.film.count),(unsigned char *)
    dpx.film.count);
  (void) WriteBlob(image,sizeof(dpx.film.format),(unsigned char *)
    dpx.film.format);
  (void) WriteBlobLong(image,dpx.film.frame_position);
  (void) WriteBlobLong(image,dpx.film.sequence_length);
  (void) WriteBlobLong(image,dpx.film.held_count);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.film.frame_rate);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.film.shutter_angle);
  (void) WriteBlob(image,sizeof(dpx.film.frame_id),(unsigned char *)
    dpx.film.frame_id);
  (void) WriteBlob(image,sizeof(dpx.film.slate),(unsigned char *)
    dpx.film.slate);
  (void) WriteBlob(image,sizeof(dpx.film.reserve),(unsigned char *)
    dpx.film.reserve);
  /*
    Write television header.
  */
  (void) WriteBlobLong(image,dpx.television.time_code);
  (void) WriteBlobLong(image,dpx.television.user_bits);
  (void) WriteBlobByte(image,dpx.television.interlace);
  (void) WriteBlobByte(image,dpx.television.field_number);
  (void) WriteBlobByte(image,dpx.television.video_signal);
  (void) WriteBlobByte(image,dpx.television.padding);
  (void) WriteBlob(image,4,(unsigned char *)
    &dpx.television.horizontal_sample_rate);
  (void) WriteBlob(image,4,(unsigned char *)
    &dpx.television.vertical_sample_rate);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.frame_rate);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.time_offset);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.gamma);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.black_level);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.black_gain);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.break_point);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.white_level);
  (void) WriteBlob(image,4,(unsigned char *) &dpx.television.integration_times);
  (void) WriteBlob(image,sizeof(dpx.television.reserve),(unsigned char *)
    dpx.television.reserve);
  /*
    Write user header.
  */
  (void) WriteBlob(image,sizeof(dpx.user.id),(unsigned char *) dpx.user.id);
  offset=TellBlob(image);
  for (i=0; i < (0x2000-offset); i++)
