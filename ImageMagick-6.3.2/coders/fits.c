  ...
  */
  ...
  for (scene=0; scene < (long) fits_info.number_planes; scene++)
  {
    ...
    */
    ...
    fits_pixels=(unsigned char *) AcquireMagickMemory(packet_size*
      image->columns*image->rows*sizeof(*fits_pixels));
        ...
        if (fits_info.bits_per_pixel == 16)
          {
            if ((long_quantum[0] & 0x80) == 0)
              pixel=(long_quantum[0] << 8) | long_quantum[1];
            else
              pixel=((long_quantum[0] << 8) | long_quantum[1] | ~0xFFFF);
          }
        if (fits_info.bits_per_pixel == -32)
          {
  ...
  Image *image)
{
  ...
  */
  ...
  (void) strncpy(fits_info+offset,header,strlen(header));
  offset+=80;
  (void) FormatMagickString(header,MaxTextExtent,"HISTORY %s",
  ...
  for (i=0; i < (long) number_planes; i++)
  {
    for (y=(long) image->rows-1; y >= 0; y--)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        ...
        if (image->depth > 16)
          {
            (void) WriteBlobByte(image,((unsigned long)
              (pixel+0.5) >> 24) & 0xff);
            (void) WriteBlobByte(image,((unsigned long)
              (pixel+0.5) >> 16) & 0xff);
          }
        if (image->depth > 8)
          (void) WriteBlobByte(image,((unsigned long)
            (pixel+0.5) >> 8) & 0xff);
        (void) WriteBlobByte(image,(unsigned long) (pixel+0.5) & 0xff);
        p++;
      }
    }
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
