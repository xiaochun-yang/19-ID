    ...
    if ((summary_info.title.length != 0) &&
        (summary_info.title.ptr != (unsigned char *) NULL))
      {
        ...
        /*
          Note image label.
        */
        label=(char *) AcquireMagickMemory(summary_info.title.length+
          MaxTextExtent);
        if (label == (char *) NULL)
          {
            FPX_ClearSystem();
    ...
    if ((summary_info.comments.length != 0) &&
        (summary_info.comments.ptr != (unsigned char *) NULL))
      {
        ...
        /*
          Note image comment.
        */
        comments=(char *) AcquireMagickMemory(summary_info.comments.length+
          MaxTextExtent);
  ...
  if (label != (const char *) NULL)
    {
      ...
      */
      ...
      summary_info.title.length=strlen(label);
      summary_info.title.ptr=(unsigned char *)
        AcquireMagickMemory(strlen(label)+MaxTextExtent);
