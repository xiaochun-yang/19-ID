...
#define LSBFirstOrder(x,y)  (((y) << 8) | (x))
  ...
  */
  ...
  global_colormap=(unsigned char *) AcquireMagickMemory((size_t)
    (3*MagickMax(global_colors,256))*sizeof(*global_colormap));
    ...
    if (BitSet((int) flag,0x80) == 0)
    ...
    else
      {
        ...
        /*
          Read local colormap.
        */
        colormap=(unsigned char *) AcquireMagickMemory((3*image->colors)*
          sizeof(*colormap));
    ...
    if (LocaleCompare(write_info->magick,"GIF87") != 0)
      {
        ...
        */
        ...
        if (opacity >= 0)
          c|=0x01;
        (void) WriteBlobByte(image,(unsigned char) c);
        delay=(unsigned long) (100*image->delay/MagickMax((size_t) image->ticks_per_second,1));
