...
static Image *ReadJP2Image(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (i=0; i < (long) number_components; i++)
  {
    maximum_component_depth=(unsigned int) MagickMax(jas_image_cmptprec(
      jp2_image,components[i]),(long) maximum_component_depth);
      ...
      case 4:
      {
        ...
        */
        for (x=0; x < (long) image->columns; x++)
        {
          ...
          q->blue=ScaleShortToQuantum((unsigned short) (scale[2]*
            jas_matrix_getv(pixels[2],x/x_step[2])));
          q->opacity=QuantumRange-ScaleShortToQuantum((unsigned short)
...
static MagickBooleanType WriteJP2Image(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  for (i=0; i < (long) number_components; i++)
  {
    ...
    component_info[i].prec=(unsigned int) MagickMin(image->depth,16);
