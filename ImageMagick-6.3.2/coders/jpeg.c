  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  (void) SetImageProperty(image,"jpeg:sampling-factor",sampling_factors);
  (void) LogMagickEvent(CoderEvent,GetMagickModule(),
  ...
  Image *image)
{
  ...
  */
  ...
  if (number_pixels == (MagickSizeType) ((size_t) number_pixels))
    {
      ...
      */
      status=AcquireMagickResource(MemoryResource,8*number_pixels);
      if (status != MagickFalse)
        {
          const char
            *option;
  ...
  sampling_factors=(const char *) NULL;
  value=GetImageProperty(image,"jpeg:sampling-factor");
  if (value != (char *) NULL)
    {
      sampling_factors=value;
      if (image->debug != MagickFalse)
        (void) LogMagickEvent(CoderEvent,GetMagickModule(),
  ...
  if (sampling_factors == (const char *) NULL)
    {
      if (image->quality >= 90)
  ...
  if (value != (char *) NULL)
    for (i=0; i < (long) strlen(value); i+=65533L)
      jpeg_write_marker(&jpeg_info,JPEG_COM,(unsigned char *) value+i,
        (unsigned int) MagickMin((long) strlen(value+i),65533L));
