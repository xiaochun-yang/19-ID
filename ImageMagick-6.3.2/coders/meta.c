    ...
    if ((q-p+1) >= (int) len)
      {
        ...
        tlen=q-p;
        len<<=1;
        p=(unsigned char *) ResizeMagickMemory(p,(size_t) (len+2));
        *b=(char *) p;
...
static long parse8BIM(Image *ifile, Image *ofile)
{
  ...
  dataset = 0;
  recnum = 0;
  line = (char *) AcquireMagickMemory((size_t) inputlen);
  ...
  while (super_fgets(&line,&inputlen,ifile)!=NULL)
  {
    ...
    token = (char *) AcquireMagickMemory((size_t) inputlen);
    newstr = (char *) AcquireMagickMemory((size_t) inputlen);
      ...
      if (LocaleCompare(image_info->magick,"APP1JPEG") == 0)
        {
          ...
          AttachBlob(iptc->blob,GetStringInfoDatum(profile),GetStringInfoLength(profile));
      ...
      else
        {
          ...
          buffer=(unsigned char *) AcquireMagickMemory(MagickMaxBufferSize*
            sizeof(*buffer));
...
static int format8BIM(Image *ifile, Image *ofile)
{
  ...
  while (c != EOF)
  {
    ...
    */
    ...
    str=(unsigned char *) AcquireMagickMemory((size_t) count);
    if (str == (unsigned char *) NULL)
      {
        printf("MemoryAllocationFailed");
        return 0;
  ...
  if (LocaleCompare(image_info->magick,"8BIMTEXT") == 0)
    {
      ...
      AttachBlob(buff->blob,GetStringInfoDatum(profile),GetStringInfoLength(profile));
