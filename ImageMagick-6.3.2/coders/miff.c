  ...
  *id='\0';
  ...
  do
  {
            ...
            */
            switch (*keyword)
            {
              ...
              case 'O':
              {
                ...
                if (LocaleCompare(keyword,"origin") == 0)
                  {
                    char
                      *geometry;

                    geometry=GetPageGeometry(options);
                    (void) ParseAbsoluteGeometry(geometry,&image->origin);
    ...
    */
    ...
    length=MagickMax(BZipMaxExtent(packet_size*image->columns),
  ...
  do
  {
    ...
    */
    ...
    if ((image->origin.x != 0) || (image->origin.y != 0))
      {
        (void) FormatMagickString(buffer,MaxTextExtent,"origin=%+ld%+ld\n",
