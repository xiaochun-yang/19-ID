...
static void MSLEndDocument(void *context)
{
  ...
  */
  ...
  if (msl_info->content != (char *) NULL)
    msl_info->content=(char *) RelinquishMagickMemory(msl_info->content);
}
...
static void MSLPushImage(MSLInfo *msl_info,Image *image)
{
  ...
  msl_info->draw_info=(DrawInfo **) ResizeMagickMemory(msl_info->draw_info,
  ...
  switch (*tag)
  {
              ...
              switch (*keyword)
              {
                ...
                case 'c':
                {
                  if (LocaleCompare(keyword,"compose") == 0)
                    {
                      option=ParseMagickOption(MagickCompositeOptions,
                        MagickFalse,value);
                      if (option < 0)
      ...
      if (LocaleCompare((char *) tag,"color-floodfill") == 0)
        {
          ...
          */
          ...
          (void) ColorFloodfillImage(msl_info->image[n],draw_info,target,
            geometry.x,geometry.y,paint_method);
          draw_info=DestroyDrawInfo(draw_info);
          break;
        }
    ...
    else if (LocaleCompare((char *) tag, "group") == 0)
    {
      ...
      msl_info->group_info=(MSLGroupInfo *) ResizeMagickMemory(
        msl_info->group_info,(msl_info->number_groups+1)*
      ...
      if (LocaleCompare((char *) tag,"matte-floodfill") == 0)
        {
          ...
          */
          ...
          MatteFloodfillImage(msl_info->image[n],target,RoundToQuantum(opacity),
            geometry.x,geometry.y,paint_method);
          break;
        }
...
static void MSLCharacters(void *context,const xmlChar *c,int length)
{
  ...
  */
  ...
  msl_info=(MSLInfo *) context;
  if (msl_info->content != (char *) NULL)
    msl_info->content=(char *) ResizeMagickMemory(msl_info->content,
      strlen(msl_info->content)+length+MaxTextExtent);
  else
    {
      msl_info->content=(char *) AcquireMagickMemory(length+MaxTextExtent);
      if (msl_info->content != (char *) NULL)
        *msl_info->content='\0';
    }
  if (msl_info->content == (char *) NULL)
