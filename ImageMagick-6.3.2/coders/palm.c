  ...
  */
  ...
  do
  {
    ...
    */
    ...
    one_row=(unsigned char *) AcquireMagickMemory(bytes_per_row*
      sizeof(*one_row));
    if (one_row == (unsigned char *) NULL)
      ThrowReaderException(ResourceLimitError,"MemoryAllocationFailed");
    lastrow=(unsigned char *) NULL;
    if (compressionType == PALM_COMPRESSION_SCANLINE) {
      lastrow=(unsigned char *) AcquireMagickMemory(bytes_per_row);
      ...
      else
        {
          ...
          if (compressionType == PALM_COMPRESSION_SCANLINE)
            {  
              ...
              {
                count=(ssize_t) ReadBlobByte(image);
                byte=MagickMin(bytes_per_row-i,8);
    ...
    if (flags & PALM_HAS_TRANSPARENCY_FLAG)
      {
        if (bits_per_pixel != 16)
          SetMagickPixelPacket(image->colormap+(mask-transparentIndex),
            (const IndexPacket *) NULL,&transpix);
        (void) PaintTransparentImage(image,&transpix,TransparentOpacity);
      }
  ...
  Image *image)
{
  ...
  */
  ...
  do
  {
    ...
    if (bits_per_pixel == 8)
      {
      ...
      else  /* Map colors to Palm standard colormap */
        {
          ...
          for (y=0; y < (long) image->rows; y++)
          {
            ...
            indexes=GetIndexes(image);
            for (x=0; x < (long) image->columns; x++)
              indexes[x]=(IndexPacket) FindColor(&image->colormap[indexes[x]]);
          }
      ...
      if (bits_per_pixel == 16)
        {
          ...
          {
            color16=(unsigned short) ((((31*p->red)/QuantumRange) << 11) |
              (((63*p->green)/QuantumRange) << 5) |
              ((31*p->blue)/QuantumRange));
        ...
        if (image->compression == FaxCompression)
          {
            ...
            {
              tptr = tmpbuf;
              for (bit=0, byte=0; bit < (unsigned char) MagickMin(8,bytes_per_row-x); bit++)
              {
    ...
    if (flags & PALM_HAS_TRANSPARENCY_FLAG)
      {
        (void) SeekBlob(image,currentOffset+6,SEEK_SET);
        (void) WriteBlobMSBShort(image,flags);
        (void) SeekBlob(image,currentOffset+12,SEEK_SET);
        (void) WriteBlobByte(image,(unsigned char) transparentIndex);  /* trans index */
      }
    if (bits_per_pixel == 16)
      {
        (void) SeekBlob(image,currentOffset+20,SEEK_SET);
        (void) WriteBlobByte(image,0);  /* reserved by Palm */
    ...
    if (flags & PALM_IS_COMPRESSED_FLAG)  /* fill in size now */
      {
        (void) SeekBlob(image,currentOffset+offset,SEEK_SET);
