...
static Image *ReadPCXImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  if (LocaleCompare(image_info->magick,"DCX") == 0)
    {
      ...
      */
      ...
      page_table=(MagickOffsetType *)
         AcquireMagickMemory(1024*sizeof(*page_table));
  ...
  for (id=1; id < 1024; id++)
  {
    ...
    */
    image->columns=(pcx_info.right-pcx_info.left)+1UL;
    image->rows=(pcx_info.bottom-pcx_info.top)+1UL;
    image->depth=pcx_info.bits_per_pixel <= 8 ? 8U : QuantumDepth;
    image->units=PixelsPerInchResolution;
    image->x_resolution=(double) pcx_info.horizontal_resolution;
    image->y_resolution=(double) pcx_info.vertical_resolution;
    image->colors=16;
    pcx_colormap=(unsigned char *)
      AcquireMagickMemory(3*256*sizeof(*pcx_colormap));
    ...
    */
    pcx_packets=(unsigned long) image->rows*pcx_info.bytes_per_line*
      pcx_info.planes;
    pcx_pixels=(unsigned char *) AcquireMagickMemory(pcx_packets*
     sizeof(*pcx_pixels));
  ...
  do
  {
    ...
    */
    ...
    pcx_pixels=(unsigned char *)
      AcquireMagickMemory(length*sizeof(*pcx_pixels));
    if (pcx_pixels == (unsigned char *) NULL)
      ThrowWriterException(ResourceLimitError,"MemoryAllocationFailed");
    q=pcx_pixels;
    if ((image->storage_class == DirectClass) || (image->colors > 256))
