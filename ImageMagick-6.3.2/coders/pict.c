        ...
        switch (code)
        {
          ...
          case 0x9b:
          {
            ...
            */
            ...
            if ((code != 0x9a) && (code != 0x9b) &&
                (bytes_per_line & 0x8000) == 0)
              pixels=DecodeImage(image_info,image,tile_image,1UL*
                bytes_per_line,1);
            else
              pixels=DecodeImage(image_info,image,tile_image,1UL*bytes_per_line,
                1U*pixmap.bits_per_pixel);
            if (pixels == (unsigned char *) NULL)
              {
                tile_image=DestroyImage(tile_image);
                ThrowReaderException(ResourceLimitError,
...
#define PictVersion  0x11
  ...
  */
  ...
  packed_scanline=(unsigned char *) AcquireMagickMemory((size_t)
   (row_bytes+MaxCount));
