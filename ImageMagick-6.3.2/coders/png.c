...
static MagickBooleanType CompressColormapTransFirst(Image *image)
{
  ...
  */
  ...
  if (image->storage_class != PseudoClass || image->colors > 256 ||
      image->colors < 2)
    return(MagickFalse);
  marker=(unsigned char *) AcquireMagickMemory(image->colors);
  if (marker == (unsigned char *) NULL)
    ThrowBinaryException(ResourceLimitError,"MemoryAllocationFailed",
      image->filename);
  opacity=(IndexPacket *)
    AcquireMagickMemory(image->colors*sizeof(IndexPacket));
  ...
  */
  ...
  colormap=(PixelPacket *)
    AcquireMagickMemory(image->colors*sizeof(PixelPacket));
  ...
  */
  map=(IndexPacket *) AcquireMagickMemory((size_t) number_colors*sizeof(IndexPacket));
...
static MagickBooleanType ImageIsMonochrome(Image *image)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=(long) image->columns-1; x >= 0; x--)
    {
      if ((p->red != 0) && (p->red != QuantumRange))
        return(MagickFalse);
      if (IsGray(p) == MagickFalse)
        return(MagickFalse);
      if ((p->opacity != OpaqueOpacity) && (p->opacity != TransparentOpacity))
        return(MagickFalse);
      p++;
    }
  }
          ...
          if (mng_info->ob[i]->reference_count == 0)
            {
              ...
              mng_info->ob[i]=(char *) RelinquishMagickMemory(mng_info->ob[i]);
            }
        }
...
{
  png_ptr=png_ptr;
  ptr=(char *) RelinquishMagickMemory(ptr);
    ...
    const ImageInfo *image_info, ExceptionInfo *exception)
{
  ...
  */
  ...
  image->interlace=ping_info->interlace_type != 0 ? LineInterlace : NoInterlace;
  if (logging != MagickFalse)
    {
      (void) LogMagickEvent(CoderEvent,GetMagickModule(),
  ...
  */
  ...
  if (num_passes > 1)
    png_pixels=(unsigned char *)
      AcquireMagickMemory(ping_info->rowbytes*image->rows);
  else
    png_pixels=(unsigned char *)
      AcquireMagickMemory(ping_info->rowbytes);
  if (png_pixels == (unsigned char *) NULL)
    ThrowReaderException(ResourceLimitError,"MemoryAllocationFailed");
  ...
  if (image->storage_class == DirectClass)
    ...
    for (pass=0; pass < num_passes; pass++)
    {
      ...
      */
      ...
      quantum_scanline=(Quantum *) AcquireMagickMemory((image->matte ?  2 : 1) *
  ...
  */
  ...
  for (;;)
  {
    ...
    */
    ...
    if (length)
      {
        chunk=(unsigned char *) AcquireMagickMemory(length);
        ...
        if (length == 16)
          {
            ...
            image->interlace=jng_image_interlace_method != 0 ? LineInterlace :
              NoInterlace;
        ...
        if (memcmp(type,mng_MHDR,4) == 0)
          {
            ...
            if (mng_info->ticks_per_second == 0)
              ...
              default_frame_delay=1UL*image->ticks_per_second/mng_info->ticks_per_second;
            ...
            if (length && (length < 769))
              {
                if (mng_info->global_plte == (png_colorp) NULL)
                  mng_info->global_plte=
                   (png_colorp) AcquireMagickMemory(256*sizeof(png_color));
...
#define QM Quantum
                ...
                next=(PixelPacket *) AcquireMagickMemory(length);
                prev=(PixelPacket *) AcquireMagickMemory(length);
  ...
  if (insert_layers)
    {
      ...
      if (next_image == (Image *) NULL)
        ThrowMagickFatalException(image->exception.severity,
...
ModuleExport void RegisterPNGImage(void)
{
  ...
  *version='\0';
  ...
  entry->magick=(MagickHandler *) IsPNG;
  entry->adjoin=MagickFalse;
  entry->description=ConstantString("8-bit indexed PNG, binary transparency only");
  entry->module=ConstantString("PNG");
  (void) RegisterMagickInfo(entry);
  ...
  *version='\0';
  ...
  entry->magick=(MagickHandler *) IsPNG;
  entry->adjoin=MagickFalse;
  entry->description=ConstantString("24-bit RGB PNG, opaque only");
  entry->module=ConstantString("PNG");
  (void) RegisterMagickInfo(entry);
  ...
  entry->magick=(MagickHandler *) IsPNG;
  entry->adjoin=MagickFalse;
  entry->description=ConstantString("32-bit RGBA PNG, semitransparency OK");
  entry->module=ConstantString("PNG");
  (void) RegisterMagickInfo(entry);
      ...
      image->depth=ping_info->bit_depth;
        {
          ...
          */
...
#endif
          palette=(png_color *)
            AcquireMagickMemory((number_colors+1)*sizeof(png_color));
          ...
          if (matte)
            {
              number_colors++;
              palette[i].red=ScaleQuantumToChar(QuantumRange);
              palette[i].green=ScaleQuantumToChar(QuantumRange);
              palette[i].blue=ScaleQuantumToChar(QuantumRange);
            }
            ...
            if (matte)
            {
              ...
              */
              ping_info->trans=(unsigned char *)
                AcquireMagickMemory(number_colors);
                  ...
                  if (p->opacity != OpaqueOpacity)
                    {
                      ...
                      ping_info->trans[indexes[x]]=(png_byte) (255-
  ...
  if (quality > 9)
    {
      ...
      level=(int) MagickMin(quality/10,9);
      if (logging != MagickFalse)
        (void) LogMagickEvent(CoderEvent,GetMagickModule(),
...
#undef PNG_HAVE_IDAT
  ...
  */
  ...
  png_pixels=(unsigned char *) AcquireMagickMemory(rowbytes);
  if (png_pixels == (unsigned char *) NULL)
    ThrowWriterException(ResourceLimitError,"MemoryAllocationFailed");
  /*
  ...
  */
  ...
  while (property != (const char *) NULL)
  {
    ...
    if ((*property != '[') && (value != (const char *) NULL))
      {
#if (PNG_LIBPNG_VER > 10005)
        text=(png_textp) png_malloc(ping,(png_uint_32) sizeof(png_text));
 ...
 * 1.0.5a is leaky */
        if (ping_info->num_text == 0)
          {
            ping_info->text=(png_text *) AcquireMagickMemory(256*
              sizeof(png_text));
  ...
  (void) SetImageProperty(image,"[png_bit_depth]",s);
      ...
      if (jng_alpha_compression_method==0)
        {
          ...
          value=GetImageProperty(jpeg_image,"[png_bit_depth]");
          if (value != (char *) NULL)
