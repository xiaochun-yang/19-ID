  ...
  */
  ...
  do
  {
    ...
    */
    ...
    image->depth=(unsigned long) MagickMin(iris_info.depth,QuantumDepth);
    if (iris_info.pixel_format == 0)
      image->depth=(unsigned long) MagickMin(8*iris_info.bytes_per_pixel,QuantumDepth);
    ...
    if ((int) iris_info.storage != 0x01)
      {
        ...
        */
        scanline=(unsigned char *) AcquireMagickMemory(bytes_per_pixel*
          iris_info.columns*sizeof(*scanline));
    ...
    else
      {
        ...
        */
        ...
        packets=(unsigned char *) AcquireMagickMemory(4*(size_t)
          iris_info.columns+10);
  ...
  do
  {
    ...
    */
    ...
    iris_info.maximum_value=(unsigned long) (image->depth <= 8 ?
      ScaleQuantumToChar(QuantumRange) : ScaleQuantumToShort(QuantumRange));
    /*
      Write SGI header.
    */
    ...
    */
    ...
    iris_pixels=(unsigned char *) AcquireMagickMemory((size_t)
      (4*iris_info.bytes_per_pixel*number_pixels*sizeof(*iris_pixels)));
    if (iris_pixels == (unsigned char *) NULL)
      ThrowWriterException(ResourceLimitError,"MemoryAllocationFailed");
    /*
    ...
    */
    ...
    switch (image->compression)
    {
      ...
      default:
      {
        ...
        */
        offsets=(ssize_t *) AcquireMagickMemory(iris_info.rows*iris_info.depth*
          sizeof(*offsets));
        packets=(unsigned char *) AcquireMagickMemory(4*(2*(size_t)
          iris_info.columns+10)*image->rows*sizeof(*packets));
        runlength=(unsigned long *) AcquireMagickMemory(iris_info.rows*
          iris_info.depth*sizeof(*runlength));
        if ((offsets == (ssize_t *) NULL) ||
            (packets == (unsigned char *) NULL) ||
