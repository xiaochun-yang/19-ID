    ...
    if (tga_info.id_length != 0)
      {
        ...
        /*
          TGA image comment.
        */
        comment=(char *) AcquireMagickMemory((size_t) (tga_info.id_length+
          ...
          switch (tga_info.colormap_size)
          {
            ...
            case 16:
            {
              ...
              */
              ...
              pixel.red=ScaleAnyToQuantum(1UL*(k & 0x7c) >> 2,31);
              pixel.green=ScaleAnyToQuantum((1UL*(k & 0x03) << 3)+
                (1UL*(j & 0xe0) >> 5),31);
              pixel.blue=ScaleAnyToQuantum(1UL*(j & 0x1f),31);
              break;
            }
            case 24:
            case 32:
            ...
            case 16:
            {
              ...
              */
              ...
              if (image->matte != MagickFalse)
                pixel.opacity=(k & 0x80) != 0? OpaqueOpacity :
                  TransparentOpacity; 
              if (image->storage_class == PseudoClass)
...
ModuleExport void RegisterTGAImage(void)
{
  ...
  entry->magick=(MagickHandler *) IsTGA;
    ...
    if (targa_info.image_type == TargaColormap)
      {
        ...
        */
        targa_colormap=(unsigned char *) AcquireMagickMemory((size_t)
          (3*targa_info.colormap_length)*sizeof(*targa_colormap));
            ...
            *q++=(unsigned char) ScaleQuantumToChar(PixelIntensityToQuantum(p));
          else
            {
              ...
              *q++=ScaleQuantumToChar(p->red);
              if (image->matte != MagickFalse)
                *q++=(unsigned char) ScaleQuantumToChar(QuantumRange-
                  p->opacity);
