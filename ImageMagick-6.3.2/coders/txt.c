...
static Image *ReadTXTImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  count=(ssize_t) sscanf(text+32,"%lu,%lu,%lu,%s",&image->columns,
    &image->rows,&max_value,format);
  if (count != 4)
    ThrowReaderException(CorruptImageError,"ImproperImageHeader");
  if (LocaleNCompare(format,"CMYK",4) == 0)
    image->colorspace=CMYKColorspace;
  image->matte=strchr(format,'A') != (char *) NULL ? MagickTrue : MagickFalse;
  while (ReadBlobString(image,text) != (char *) NULL)
  {
    ...
    if (q == (PixelPacket *) NULL)
      continue;
    q->red=ScaleAnyToQuantum(pixel.red,max_value);
    q->green=ScaleAnyToQuantum(pixel.green,max_value);
    q->blue=ScaleAnyToQuantum(pixel.blue,max_value);
    if (image->colorspace == CMYKColorspace)
      {
        indexes=GetIndexes(image);
        *indexes=ScaleAnyToQuantum(pixel.index,max_value);
      }
    if (image->matte != MagickFalse)
      q->opacity=QuantumRange-ScaleAnyToQuantum(pixel.opacity,max_value);
...
static inline void ConcatenateColorComponent(const MagickPixelPacket *pixel,
  const ChannelType channel,char *tuple)
{
  ...
  switch (channel)
  {
    case RedChannel: color=pixel->red; break;
    case GreenChannel: color=pixel->green; break;
    case BlueChannel: color=pixel->blue; break;
    case OpacityChannel: color=QuantumRange-pixel->opacity; break;
    case IndexChannel: color=pixel->index; break;
    default: break;
  }
  if (pixel->depth > 16)
    {
...
static MagickBooleanType WriteTXTImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  if (status == MagickFalse)
    return(status);
  if (image->colorspace != CMYKColorspace)
    (void) CopyMagickString(format,"RGB",MaxTextExtent);
  else
    (void) CopyMagickString(format,"CMYK",MaxTextExtent);
  if (image->matte != MagickFalse)
    (void) ConcatenateMagickString(format,"A",MaxTextExtent);
  image->depth=GetImageQuantumDepth(image,MagickTrue);
  quantum_range=(1UL << image->depth)-1;
  if (image->depth == 32)
     quantum_range=4294967295UL;
  (void) FormatMagickString(buffer,MaxTextExtent,
    "# ImageMagick pixel enumeration: %lu,%lu,%lu,%s\n",image->columns,
    image->rows,quantum_range,format);
    ...
    */
    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        ...
        ConcatenateColorComponent(&pixel,RedChannel,tuple);
        (void) ConcatenateMagickString(tuple,",",MaxTextExtent);
        ConcatenateColorComponent(&pixel,GreenChannel,tuple);
        (void) ConcatenateMagickString(tuple,",",MaxTextExtent);
        ConcatenateColorComponent(&pixel,BlueChannel,tuple);
        if (pixel.colorspace == CMYKColorspace)
          {
            (void) ConcatenateMagickString(tuple,",",MaxTextExtent);
            ConcatenateColorComponent(&pixel,IndexChannel,tuple);
          }
        if (pixel.matte != MagickFalse)
          {
            (void) ConcatenateMagickString(tuple,",",MaxTextExtent);
            ConcatenateColorComponent(&pixel,OpacityChannel,tuple);
          }
        (void) ConcatenateMagickString(tuple,")",MaxTextExtent);
