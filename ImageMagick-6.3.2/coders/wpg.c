...
static void Rd_WP_DWORD(Image *image,unsigned long *d)
{
  ...
  b=(unsigned char) ReadBlobByte(image);
  *d=b;
  if (b < 0xFF)
    return;
  b=(unsigned char) ReadBlobByte(image);
...
static int UnpackWPGRaster(Image *image,int bpp)
{
  ...
  BImgBuff=(unsigned char *) AcquireMagickMemory((size_t) ldblk);
  ...
  while(y<(long) image->rows)
    {
      ...
      switch(bbuf)
        ...
        case 0x7E:
          ...
          break;
        case 0x7F:
          RunCount=(unsigned char) ReadBlobByte(image);   /* BLK */
          ...
          break;
        case 0xFD:
          RunCount=(unsigned char) ReadBlobByte(image);   /* EXT */
          for (i=0; i <= (int) RunCount; i++)
            for (bbuf=0; bbuf < (unsigned char) SampleSize; bbuf++)
              InsertByte(SampleBuffer[bbuf]);
          break;
        case 0xFE:
          RunCount=(unsigned char) ReadBlobByte(image);  /* RST */
          if(x!=0)
            {
              fprintf(stderr,
          ...
          break;
        case 0xFF:
          RunCount=(unsigned char) ReadBlobByte(image);  /* WHT */
...
float CTM[3][3];        /* current transform matrix (currently ignored) */
 ...
 (void) ResetMagickMemory(CTM,0,sizeof(CTM));     /* CTM.erase();CTM.resize(3,3) */
 CTM[0][0]=1;
 CTM[1][1]=1;
 CTM[2][2]=1;
                                                                                
 Flags=ReadBlobLSBShort(image);
 if(Flags & LCK) x=(long) ReadBlobLSBLong(image);  /* Edit lock */
 if(Flags & OID)
  {
  if(Precision==0)
    {x=(long) ReadBlobLSBShort(image);}  /* ObjectID */
  else
    {x=(long) ReadBlobLSBLong(image);}  /* ObjectID (Double precision). */
  }
 if(Flags & ROT)
  {
  x=(long) ReadBlobLSBLong(image);  /* Rot Angle. */
  if(Angle) *Angle=1.0*x/65536.0;
  }
 if(Flags & (ROT|SCL))
  {
  x=(long) ReadBlobLSBLong(image); /* Sx*cos(). */
  CTM[0][0]=1.0*x;
  x=(long) ReadBlobLSBLong(image); /* Sy*cos(). */
  CTM[1][1]=1.0*x;
  }
 if(Flags & (ROT|SKW))
  {
  x=(long) ReadBlobLSBLong(image);       /* Kx*sin(). */
  CTM[1][0]=1.0*x;
  x=(long) ReadBlobLSBLong(image);       /* Ky*sin(). */
  CTM[0][1]=1.0*x;
  }
 if(Flags & TRN)
  {
  x=(long) ReadBlobLSBLong(image); DenX=(unsigned int) ReadBlobLSBLong(image);  /* Tx */
  CTM[0][2]=(float)x + ((x >= 0)?1:-1)*(float)DenX/0x10000;
  x=(long) ReadBlobLSBLong(image); DenX=(unsigned int) ReadBlobLSBLong(image);  /* Ty */
  CTM[1][2]=(float)x + ((x >= 0)?1:-1)*(float)DenX/0x10000;
  }
 if(Flags & TPR)
  {
  x=(long) ReadBlobLSBLong(image);  /* Px. */
  CTM[2][0]=1.0*x;
  x=(long) ReadBlobLSBLong(image);  /* Py. */
  CTM[2][1]=1.0*x;
  }
 return(Flags);
}
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  switch(Header.FileType)
      ...
      while(!EOFBlob(image)) /* object parser loop */
        {
          ...
          switch(Rec.RecType)
            ...
            case 0x14:  /* bitmap type 2 */
              ...
              image->columns=1UL*BitmapHeader2.Width;
              image->rows=1UL*BitmapHeader2.Heigth;
              bpp=(int) BitmapHeader2.Depth;

            UnpackRaster:
              if (image->colors == 0 && bpp!=24)
                {
                  image->colors=1UL << bpp;
                  if (AllocateImageColormap(image,image->colors) == MagickFalse)
                    {
                    NoMemory:
                ...
                if (bpp < 24)
                  if( image->colors<(1UL<<bpp) && bpp!=24 )
                    image->colormap=(PixelPacket *) ResizeMagickMemory(
                      image->colormap,(size_t) ((1<<bpp)*sizeof(PixelPacket)));
              }
    ...
    case 2:  /*WPG level 2*/
      (void) ResetMagickMemory(&StartWPG,0,sizeof(StartWPG));
      while(!EOFBlob(image)) /* object parser loop */

        {
          ...
          switch(Rec2.RecType)
            ...
            case 0x0C:    /*Color palette */
              ...
              break;
            case 0x0E:
              Bitmap2Header1.Width=(unsigned char) ReadBlobLSBShort(image);
              Bitmap2Header1.Heigth=(unsigned char) ReadBlobLSBShort(image);
                ...
                case 0:    /*Uncompressed raster*/
                  {
                    ...
                    if((BImgBuff=(unsigned char *)
                        AcquireMagickMemory((size_t) ldblk))==NULL)
                      goto NoMemory;
            ...
            case 0x1B:          /*bitmap rectangle*/
              WPG2Flags=(int) LoadWPG2Flags(image,(char) StartWPG.PosSizePrecision,NULL);
              break;
            }
        }
...
ModuleExport void RegisterWPGImage(void)
{
  ...
  entry->description=ConstantString("Word Perfect Graphics");
