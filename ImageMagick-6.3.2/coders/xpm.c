  ...
  if (xpm_buffer != (char *) NULL)
    while (ReadBlobString(image,p) != (char *) NULL)
    {
      ...
      if ((size_t) (p-xpm_buffer+MaxTextExtent) < length)
        continue;
      length<<=1;
      xpm_buffer=(char *) ResizeMagickMemory(xpm_buffer,(length+MaxTextExtent)*
        sizeof(*xpm_buffer));
      if (xpm_buffer == (char *) NULL)
        break;
    ...
    if (q != (char *) NULL)
      {
        while ((isspace((int) ((unsigned char) *q)) == 0) && (*q != '\0'))
          q++;
        if (next != (char *) NULL)
          (void) CopyXPMColor(target,q,(size_t) (next-q));
        else
  ...
  */
  ...
  for (i=0; i < (long) colors; i++)
  {
    /*
      Define XPM color.
    */
    SetMagickPixelPacket(picon->colormap+i,(IndexPacket *) NULL,&pixel);
        ...
        if (image->colormap[i].opacity != OpaqueOpacity)
          {
            ...
            alpha=TransparentOpacity-(MagickRealType)
              image->colormap[i].opacity;
            beta=TransparentOpacity-(MagickRealType)
  ...
  */
  ...
  for (i=0; i < (long) image->colors; i++)
  {
    /*
      Define XPM color.
    */
    SetMagickPixelPacket(image->colormap+i,(IndexPacket *) NULL,&pixel);
    pixel.colorspace=RGBColorspace;
    pixel.depth=8;
    pixel.opacity=(MagickRealType) OpaqueOpacity;
    (void) QueryMagickColorname(image,&pixel,XPMCompliance,MagickFalse,name,
