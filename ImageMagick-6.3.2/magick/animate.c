      ...
      if (filelist == (char **) NULL)
        {
          (void) ThrowMagickException(&(*image)->exception,GetMagickModule(),
            ResourceLimitError,"MemoryAllocationFailed","`%s'",filenames);
          return((Image *) NULL);
        }
      number_files=1;
      ...
      if ((status == MagickFalse) || (number_files == 0))
        {
          if (number_files == 0)
            (void) ThrowMagickException(&(*image)->exception,GetMagickModule(),
              ImageError,"NoImagesWereLoaded","`%s'",filenames);
          else
            (void) ThrowMagickException(&(*image)->exception,GetMagickModule(),
              ResourceLimitError,"MemoryAllocationFailed","`%s'",filenames);
          return((Image *) NULL);
  ...
  XResourceInfo *resource_info,Image *images)
{
  ...
  if (window_info.id == (Window) NULL)
    {
      (void) ThrowMagickException(&images->exception,GetMagickModule(),
        XServerError,"NoWindowWithSpecifiedIDExists","`%s'",
        resources.window_id);
      return;
    }
  /*
  ...
  if (visual_info == (XVisualInfo *) NULL)
    {
      ...
      */
      ...
      if (map_info == (XStandardColormap *) NULL)
        ThrowMagickFatalException(ResourceLimitError,"MemoryAllocationFailed",
          images->filename);
  ...
  if (GetNextImageInList(images) != (Image *) NULL)
    {
      ...
      if (coalesce_image == (Image *) NULL)
        ThrowMagickFatalException(images->exception.severity,
  ...
  */
  window_info.pixmaps=(Pixmap *)
    AcquireMagickMemory(number_scenes*sizeof(*window_info.pixmaps));
...
#define MagickTitle  "Commands"
  ...
  */
  ...
  windows->image.pixmaps=(Pixmap *)
    AcquireMagickMemory(number_scenes*sizeof(*windows->image.pixmaps));
