...
static unsigned long *EncodeSJIS(const Image *image,const char *text,
  size_t *count)
{
  int
    c;
  ...
  *count=0;
  ...
  encoding=(unsigned long *) AcquireMagickMemory(length*sizeof(*encoding));
...
%
*/
static unsigned long *EncodeText(const Image *image,const char *text,
  size_t *count)
{
  register const char
    *p;
...
static unsigned long *EncodeUnicode(const Image *image,const char *text,
  size_t *count)
{
  long
    c;
  ...
  *count=0;
  if ((text == (char *) NULL) || (*text == '\0'))
    return((unsigned long *) NULL);
  length=strlen(text)+MaxTextExtent;
  unicode=(unsigned long *) AcquireMagickMemory(length*sizeof(*unicode));
...
#define FT_OPEN_PATHNAME  ft_open_pathname
  ...
  */
  ...
  for (i=0; i < (long) length; i++)
  {
    ...
    if ((point.x+bitmap->bitmap.width-offset->x) > metrics->width)
      metrics->width=point.x+bitmap->bitmap.width-offset->x;
