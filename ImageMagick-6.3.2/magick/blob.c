  ...
  size_t *length,ExceptionInfo *exception)
{
  ...
  if (file == -1)
    {
      ...
      message=GetExceptionMessage(errno);
      (void) ThrowMagickException(exception,GetMagickModule(),BlobError,
        "UnableToOpenFile","`%s': %s",filename,message);
      message=(char *) RelinquishMagickMemory(message);
  ...
  if ((offset < 0) || (offset != (MagickOffsetType) ((ssize_t) offset)))
    {
      ...
      */
      quantum=(size_t) (fstat(file,&file_info) < 0 ? 0 : file_info.st_size);
      if ((quantum == 0) || (quantum > MagickMaxBufferSize))
        quantum=MagickMaxBufferSize;
      blob=(unsigned char *) AcquireMagickMemory(quantum*sizeof(*blob));
      for (i=0; blob != (unsigned char *) NULL; i+=count)
      {
        ...
        blob=(unsigned char *) ResizeMagickMemory(blob,(i+quantum)*
          sizeof(*blob));
  ...
  *length=MagickMin((size_t) offset,extent);
  blob=(unsigned char *) AcquireMagickMemory(CheckOverflowException(*length,
    MaxTextExtent)*sizeof(*blob));
  if (blob == (unsigned char *) NULL)
    {
      file=close(file)-1;
      (void) ThrowMagickException(exception,GetMagickModule(),
      ...
      if (i < *length)
        {
          ...
          message=GetExceptionMessage(errno);
          (void) ThrowMagickException(exception,GetMagickModule(),BlobError,
            "UnableToReadBlob","`%s': %s",filename,message);
          message=(char *) RelinquishMagickMemory(message);
          return((unsigned char *) NULL);
        }
    }
  ...
  switch (image->blob->type)
  {
    ...
    case FileStream:
    {
      length=(MagickSizeType) (fstat(fileno(image->blob->file),
        &file_info) < 0 ? 0 : file_info.st_size);
      break;
    }
    case StandardStream:
    case PipeStream:
    ...
    {
#if defined(HasZLIB)
      length=(MagickSizeType) (stat(image->filename,&file_info) < 0 ? 0 :
        file_info.st_size);
#endif
      break;
    }
  ...
  if (GetMagickBlobSupport(magick_info) != MagickFalse)
    {
      ...
      */
      blob_info->length=0;
      blob_info->blob=(void *) AcquireMagickMemory(DefaultBlobQuantum*
      ...
      else
        {
          ...
          *image->filename='\0';
          ...
          if ((status == MagickFalse) || (image->blob->length == 0))
            ...
            {
              image->blob->data=(unsigned char *) ResizeMagickMemory(
      ...
      if (file == -1)
        {
          ...
          message=GetExceptionMessage(errno);
          (void) ThrowMagickException(exception,GetMagickModule(),BlobError,
            "UnableToWriteBlob","`%s': %s",image->filename,message);
          message=(char *) RelinquishMagickMemory(message);
        }
      else
        {
  ...
  ExceptionInfo *exception)
{
  ...
  if ((quantum == 0) || (quantum > MagickMaxBufferSize))
    quantum=MagickMaxBufferSize;
  buffer=(unsigned char *) AcquireMagickMemory(quantum*sizeof(*buffer));
  ...
  length=0;
  for (i=0; (count=ReadBlob(image,quantum,buffer)) > 0; )
  {
    length=(size_t) count;
    for (i=0; i < length; i+=count)
    {
      ...
      if (file == -1)
        {
          ...
          message=GetExceptionMessage(errno);
          (void) ThrowMagickException(exception,GetMagickModule(),BlobError,
            "UnableToWriteBlob","`%s': %s",images->filename,message);
          message=(char *) RelinquishMagickMemory(message);
        }
      else
        {
  ...
  Image *image,const char *format)
{
  ...
  */
  ...
  for (i=0; ; i+=count)
  {
    ...
    status=WriteBlob(image,(size_t) count,buffer) == count ? MagickTrue :
      MagickFalse;
  }
  file=close(file)-1;
  (void) RelinquishUniqueFileResource(filename);
    ...
    if ((image->blob->type == FileStream) && (*type == 'r'))
      {
        ...
        if ((magick_info != (const MagickInfo *) NULL) &&
          ...
          if ((file_info.st_size > (off_t) MinBlobExtent) &&
              (file_info.st_size == (off_t) ((ssize_t) file_info.st_size)))
            {
              size_t
                length;
...
MagickExport ssize_t WriteBlobString(Image *image,const char *string)
{
  ...
  return(WriteBlob(image,strlen(string),(const unsigned char *) string));
}
