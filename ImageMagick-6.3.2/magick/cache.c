          ...
          (((y+v) < 0) || ((y+v) >= (long) cache_info->rows)) || (length == 0))
        {
          ...
          */
          ...
          switch (cache_info->virtual_pixel_method)
          {
            ...
            {
              p=AcquireCacheNexus(image,DitherX(cache_info->columns,x+u),
                DitherY(cache_info->rows,y+v),1UL,1UL,image_nexus,exception);
              break;
            }
            ...
            {
              p=AcquireCacheNexus(image,RandomX(cache_info->columns),
                RandomY(cache_info->rows),1UL,1UL,image_nexus,exception);
              break;
            }
...
%  safe.  In a threaded environment, use AcquireCacheView() or GetCacheView()
%  instead.
%
  ...
  const unsigned long rows,ExceptionInfo *exception)
{
  ...
  if (cache_info->methods.acquire_pixel_handler == (AcquirePixelHandler) NULL)
    return((const PixelPacket *) NULL);
  pixels=cache_info->methods.acquire_pixel_handler(image,x,y,columns,rows,
    exception);
  return(pixels);
}

...
static MagickBooleanType ClipCacheNexus(Image *image,const unsigned long nexus)
{
  ...
  */
  ...
  r=AcquireCacheNexus(image->clip_mask,nexus_info->x,nexus_info->y,
    nexus_info->columns,nexus_info->rows,clip_nexus,&image->exception);
...
static inline void AcquireNexusPixels(NexusInfo *nexus_info)
{
  ...
  if (nexus_info->length > MinBlobExtent)
    {
...
MagickExport Cache DestroyCacheInfo(Cache cache)
{
  ...
  if (cache_resources != (SplayTreeInfo *) NULL)
    (void) RemoveNodeByValueFromSplayTree(cache_resources,cache_info);
...
static MagickBooleanType MaskCacheNexus(Image *image,const unsigned long nexus)
{
  ...
  */
  ...
  q=nexus_info->pixels;
  nexus_indexes=nexus_info->indexes;
  r=AcquireCacheNexus(image->mask,nexus_info->x,nexus_info->y,
    nexus_info->columns,nexus_info->rows,clip_nexus,&image->exception);
  ...
  for (i=0; i < (long) (nexus_info->columns*nexus_info->rows); i++)
  {
    if ((p == (PixelPacket *) NULL) || (r == (const PixelPacket *) NULL))
      break;
    SetMagickPixelPacket(p,indexes+i,&alpha);
    SetMagickPixelPacket(q,nexus_indexes+i,&beta);
    MagickPixelCompositeOver(&beta,(MagickRealType) PixelIntensityToQuantum(r),
...
static MagickBooleanType ModifyCache(Image *image,const unsigned long nexus)
{
  ...
  (void) SetImageVirtualPixelMethod(image,clone_info->virtual_pixel_method);
...
%    o image: The image.
%
%    o method: choose the type of virtual pixel.
%
*/
