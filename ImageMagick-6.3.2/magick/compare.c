  ...
  const MetricType metric,double *distortion,ExceptionInfo *exception)
{
  ...
  */
  ...
  {
    p=AcquireCacheView(image_view,0,y,image->columns,1,exception);
    q=AcquireCacheView(reconstruct_view,0,y,reconstruct_image->columns,1,
      exception);
    ...
    reconstruct_indexes=GetCacheViewIndexes(reconstruct_view);
  ...
  const Image *reconstruct_image,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      SetMagickPixelPacket(p,indexes+x,&image_pixel);
      SetMagickPixelPacket(q,reconstruct_indexes+x,&reconstruct_pixel);
  ...
  const Image *reconstruct_image)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    p=AcquireCacheView(image_view,0,y,image->columns,1,&image->exception);
    q=AcquireCacheView(reconstruct_view,0,y,reconstruct_image->columns,1,
      &image->exception);
    if ((p == (const PixelPacket *) NULL) || (q == (const PixelPacket *) NULL))
      break;
  ...
  image->error.normalized_mean_error=(double)
    (QuantumScale*QuantumScale*mean_error/area);
