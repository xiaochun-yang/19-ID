  ...
  const long x,const long y)
{
  ...
  fill_color=AcquireOnePixel(pattern,x+pattern->origin.x,
    y+pattern->origin.y,&pattern->exception);
  ...
  const long x,const long y)
{
  ...
  stroke_color=AcquireOnePixel(pattern,x+pattern->origin.x,y+pattern->origin.y,
    &pattern->exception);
  if (pattern->matte == MagickFalse)
