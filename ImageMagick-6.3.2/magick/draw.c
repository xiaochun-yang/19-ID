  ...
  if (draw_info->dash_pattern != (double *) NULL)
    {
      ...
      clone_info->dash_pattern=(double *) AcquireMagickMemory((size_t) (x+1)*
  ...
  if (draw_info->gradient.stops != (StopInfo *) NULL)
    {
      ...
      clone_info->gradient.stops=(StopInfo *) AcquireMagickMemory((size_t)
...
MagickExport DrawInfo *DestroyDrawInfo(DrawInfo *draw_info)
{
  ...
  if (draw_info->clip_mask != (char *) NULL)
    draw_info->clip_mask=(char *) RelinquishMagickMemory(draw_info->clip_mask);
  ...
  */
  ...
  for (y=(long) (edge.y1+0.5); y <= (long) (edge.y2+0.5); y++)
  {
    ...
    if (inverse_edge.x2 < inverse_edge.x1)
      continue;
    q=GetCacheView(image_view,(long) (inverse_edge.x1+0.5),y,(unsigned long)
      ((long) (inverse_edge.x2+0.5)-(long) (inverse_edge.x1+0.5)+1),1);
    ...
    for (x=(long) (inverse_edge.x1+0.5); x <= (long) (inverse_edge.x2+0.5); x++)
    {
      ...
      point.y=(double) x*inverse_affine.rx+y*inverse_affine.sy+
        inverse_affine.ty;
      pixel=InterpolatePixelColor(source,source_view,image->interpolate,point.x,
        point.y,&image->exception);
      SetMagickPixelPacket(q,indexes+x_offset,&composite);
      MagickPixelCompositeOver(&pixel,pixel.opacity,&composite,
        composite.opacity,&composite);
      SetPixelPacket(&composite,q,indexes+x_offset);
      x_offset++;
      q++;
    }
  ...
  const DrawInfo *draw_info,const char *name)
{
  ...
  value=GetImageProperty(image,clip_mask);
  ...
  (void) QueryColorDatabase("none",&image->clip_mask->background_color,
    &image->exception);
  image->clip_mask->background_color.opacity=TransparentOpacity;
  ...
  const PrimitiveInfo *primitive_info,Image *image)
{
  ...
  dash_polygon=(PrimitiveInfo *)
    AcquireMagickMemory((size_t) (2*number_vertices+1)*sizeof(*dash_polygon));
...
#define RenderImageTag  "Render/Image"
  ...
  */
  ...
  (void) SetImageProperty(image,"[MVG]",primitive);
  n=0;
  /*
  ...
  */
  ...
  number_points=2047;
  primitive_info=(PrimitiveInfo *) AcquireMagickMemory((size_t) number_points*
  ...
  if (graphic_context[n]->fill_pattern != (Image *) NULL)
    (void) SetImageVirtualPixelMethod(graphic_context[n]->fill_pattern,
      TileVirtualPixelMethod);
  if (graphic_context[n]->stroke_pattern != (Image *) NULL)
    (void) SetImageVirtualPixelMethod(graphic_context[n]->stroke_pattern,
      TileVirtualPixelMethod);
        ...
        if (LocaleCompare("decorate",keyword) == 0)
          {
            ...
            GetMagickToken(q,&q,token);
            decorate=ParseMagickOption(MagickDecorationOptions,MagickFalse,
              token);
            if (decorate == -1)
              {
                status=MagickFalse;
        ...
        if (LocaleCompare("fill",keyword) == 0)
          {
            ...
            if (GetImageProperty(image,pattern) != (const char *) NULL)
        ...
        if (LocaleCompare("image",keyword) == 0)
          {
            ...
            GetMagickToken(q,&q,token);
            compose=ParseMagickOption(MagickCompositeOptions,MagickFalse,token);
            if (compose == -1)
              {
                status=MagickFalse;
                break;
        ...
        if (LocaleCompare("opacity",keyword) == 0)
          {
            GetMagickToken(q,&q,token);
            factor=strchr(token,'%') != (char *) NULL ? 0.01 : 1.0;
            graphic_context[n]->opacity=(Quantum) ((MagickRealType)
              QuantumRange*(1.0-((1.0-graphic_context[n]->opacity/
              QuantumRange)*factor*atof(token)))+0.5);
        ...
        if (LocaleCompare("push",keyword) == 0)
          {
            ...
            if (LocaleCompare("clip-path",token) == 0)
              {
                ...
                (void) SetImageProperty(image,name,token);
                GetMagickToken(q,&q,token);
                break;
              }
            if (LocaleCompare("gradient",token) == 0)
              {
                ...
                (void) SetImageProperty(image,key,token);
                (void) FormatMagickString(key,MaxTextExtent,"[%s-geometry]",
                  name);
                (void) FormatMagickString(geometry,MaxTextExtent,"%gx%g%+f%+f",
                  MagickMax(fabs(bounds.x2-bounds.x1+1.0),1.0),
                  MagickMax(fabs(bounds.y2-bounds.y1+1.0),1.0),
                  bounds.x1,bounds.y1);
                (void) SetImageProperty(image,key,geometry);
                GetMagickToken(q,&q,token);
                break;
              }
            if (LocaleCompare("pattern",token) == 0)
            ...
            if (LocaleCompare("graphic-context",token) == 0)
              {
                n++;
                graphic_context=(DrawInfo **) ResizeMagickMemory(
            ...
            if (IsPoint(q) != MagickFalse)
              {
                ...
                graphic_context[n]->dash_pattern=(double *) AcquireMagickMemory(
                  (size_t) (2*x+1)*sizeof(*graphic_context[n]->dash_pattern));
                if (graphic_context[n]->dash_pattern == (double *) NULL)
                  {
                    (void) ThrowMagickException(&image->exception,
                      GetMagickModule(),ResourceLimitError,
        ...
        if (LocaleCompare("viewbox",keyword) == 0)
          {
            ...
            if (*token == ',')
              GetMagickToken(q,&q,token);
            graphic_context[n]->viewbox.height=(unsigned long)
              (atof(token)+0.5);
            break;
          }
        status=MagickFalse;
  ...
  const DrawInfo *draw_info,const char *name,Image **pattern)
{
  ...
  path=GetImageProperty(image,property);
  if (path == (const char *) NULL)
    return(MagickFalse);
  (void) FormatMagickString(property,MaxTextExtent,"[%s-geometry]",name);
  geometry=GetImageProperty(image,property);
  ...
  MagickRealType *stroke_opacity)
{
  ...
  *stroke_opacity=0.0;
  subpath_opacity=0.0;
  p=polygon_info->edges;
  for (j=0; j < (long) polygon_info->number_edges; j++)
  {
    if ((MagickRealType) y <= (p->bounds.y1-mid-0.5))
      break;
    if ((MagickRealType) y > (p->bounds.y2+mid+0.5-MagickEpsilon))
      {
        (void) DestroyEdge(polygon_info,(unsigned long) j);
        p++;
        continue;
      }
    if ((MagickRealType) x <= (p->bounds.x1-mid-0.5))
      {
        p++;
        continue;
      }
    if ((MagickRealType) x > (p->bounds.x2+mid+0.5-MagickEpsilon))
      {
        p++;
        continue;
      }
    ...
    {
      if ((MagickRealType) y <= (p->points[i-1].y-mid-0.5))
        break;
      if ((MagickRealType) y > (p->points[i].y+mid+0.5-MagickEpsilon))
        continue;
      if (p->scanline != (MagickRealType) y)
        {
          p->scanline=(MagickRealType) y;
      ...
      */
      ...
      dx=(q+1)->x-q->x,
      dy=(q+1)->y-q->y;
      beta=dx*((MagickRealType) x-q->x)+dy*((MagickRealType) y-q->y);
      if (beta < 0.0)
        {
          dx=(MagickRealType) x-q->x;
          dy=(MagickRealType) y-q->y;
      ...
      else
        {
          ...
          if (beta > alpha)
            {
              dx=(MagickRealType) x-(q+1)->x;
              dy=(MagickRealType) y-(q+1)->y;
              distance=dx*dx+dy*dy;
            }
          else
            {
              alpha=1.0/alpha;
              beta=dx*((MagickRealType) y-q->y)-dy*((MagickRealType) x-q->x);
  ...
  */
  ...
  for (j=0; j < (long) polygon_info->number_edges; j++)
  {
    if ((MagickRealType) y <= p->bounds.y1)
      break;
    if ((MagickRealType) y > p->bounds.y2)
      {
        p++;
        continue;
      }
    if ((MagickRealType) x <= p->bounds.x1)
      {
        p++;
        continue;
      }
    if ((MagickRealType) x > p->bounds.x2)
      {
    ...
    i=(long) MagickMax((double) p->highwater,1.0);
    for ( ; i < (long) p->number_points; i++)
      if ((MagickRealType) y <= p->points[i].y)
        break;
    q=p->points+i-1;
    dx=(q+1)->x-q->x;
    dy=(q+1)->y-q->y;
  ...
  if (primitive_info->coordinates == 1)
    {
      ...
      */
      ...
      {
        x=start;
        q=GetCacheView(image_view,x,y,(unsigned long) (stop-x+1),1);
        if (q == (PixelPacket *) NULL)
          break;
        for ( ; x <= stop; x++)
        {
  ...
  for (y=(long) (bounds.y1+0.5); y <= (long) (bounds.y2+0.5); y++)
  {
    ...
    for ( ; x <= stop; x++)
    {
      ...
      */
      ...
      stroke_opacity=QuantumRange-stroke_opacity*(QuantumRange-
  ...
  const DrawInfo *draw_info,const PrimitiveInfo *primitive_info)
{
  ...
  switch (primitive_info->primitive)
  {
    case PointPrimitive:
    {
      ...
      q=GetCacheView(image_view,x,y,1,1);
      if (q == (PixelPacket *) NULL)
        break;
      fill_color=GetFillColor(draw_info,x,y);
    ...
    case MattePrimitive:
    {
      ...
      switch (primitive_info->method)
      {
        ...
        case FillToBorderMethod:
        {
          ...
          (void) MatteFloodfillImage(image,target,
            GetFillColor(draw_info,x,y).opacity,x,y,primitive_info->method);
          break;
        }
        case ResetMethod:
        {
  ...
  const unsigned long number_coordinates)
{
  ...
  */
  ...
  coefficients=(MagickRealType *) AcquireMagickMemory((size_t)
    number_coordinates*sizeof(*coefficients));
  points=(PointInfo *) AcquireMagickMemory((size_t) control_points*
    sizeof(*points));
  ...
  const PrimitiveInfo *primitive_info)
{
  ...
  */
  ...
  path_p=(PointInfo *)
    AcquireMagickMemory((size_t) max_strokes*sizeof(*path_p));
  path_q=(PointInfo *)
    AcquireMagickMemory((size_t) max_strokes*sizeof(*path_q));
  polygon_primitive=(PrimitiveInfo *) AcquireMagickMemory((size_t)
    (number_vertices+2)*sizeof(*polygon_primitive));
    ...
    if (q >= (long) (max_strokes-6*BezierQuantum-360))
      {
         ...
         path_p=(PointInfo *)
           ResizeMagickMemory(path_p,(size_t) max_strokes*sizeof(*path_p));
         path_q=(PointInfo *)
           ResizeMagickMemory(path_q,(size_t) max_strokes*sizeof(*path_q));
         if ((path_p == (PointInfo *) NULL) || (path_q == (PointInfo *) NULL))
           {
  ...
  */
  stroke_polygon=(PrimitiveInfo *)
    AcquireMagickMemory((size_t) (p+q+2*closed_path+2)*sizeof(*stroke_polygon));
