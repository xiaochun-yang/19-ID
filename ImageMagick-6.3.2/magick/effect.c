...
#define AdaptiveBlurImageTag  "Convolve/Image"
  ...
  */
  ...
  {
    kernel[i]=(double *) AcquireMagickMemory((size_t) (width-i)*(width-i)*
      sizeof(**kernel));
...
#define TauGaussian  ScaleCharToQuantum(20)
  ...
  switch (noise_type)
  {
    ...
    case PoissonNoise:
    {
      ...
      noise=(MagickRealType)
        ScaleCharToQuantum((unsigned char) (i/SigmaPoisson));
      break;
    }
  }
  ...
  */
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      ...
      if (((channel & OpacityChannel) != 0) && (image->matte != MagickFalse))
        q->opacity=GenerateNoise(p->opacity,noise_type);
      if (((channel & IndexChannel) != 0) &&
          (image->colorspace == CMYKColorspace))
...
#define KernelRank 3
  ...
  */
  ...
  for (i=(-bias); i <= bias; i++)
  {
    alpha=exp((-((double) (i*i))/(double)
      (2.0*KernelRank*KernelRank*sigma*sigma)));
...
#define BlurImageTag  "Blur/Image"
  ...
  */
  ...
  for (x=0; x < (long) blur_image->columns; x++)
  {
    p=AcquireCacheView(image_view,x,-((long) width/2L),1,blur_image->rows+width,
      exception);
...
#define KernelRank 3
  ...
  */
  ...
  {
    alpha=exp((-((double) (i*i))/
      (double) (2.0*KernelRank*KernelRank*sigma*sigma)));
  ...
  const ChannelType channel,const double angle,ExceptionInfo *exception)
{
  ...
  */
  ...
  n=(unsigned long) fabs(4*DegreesToRadians(angle)*sqrt((double) blur_radius)+
    2);
  theta=DegreesToRadians(angle)/(MagickRealType) (n-1);
  cos_theta=(MagickRealType *) AcquireMagickMemory((size_t) n*
    sizeof(*cos_theta));
...
#define SharpenImageTag  "Sharpen/Image"
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      if ((channel & RedChannel) != 0)
        {
          pixel.red=(MagickRealType) p->red-(MagickRealType) q->red;
      ...
      if ((channel & GreenChannel) != 0)
        {
          pixel.green=(MagickRealType) p->green-(MagickRealType) q->green;
      ...
      if ((channel & BlueChannel) != 0)
        {
          pixel.blue=(MagickRealType) p->blue-(MagickRealType) q->blue;
      ...
      if (((channel & OpacityChannel) != 0) && (image->matte != MagickFalse))
        {
          pixel.opacity=(MagickRealType) p->opacity-(MagickRealType) q->opacity;
          if (fabs(2.0*pixel.opacity) < quantum_threshold)
            pixel.opacity=(MagickRealType) p->opacity;
          else
            pixel.opacity=(MagickRealType) p->opacity+(pixel.opacity*amount);
          q->opacity=RoundToQuantum(pixel.opacity);
        }
      if (((channel & IndexChannel) != 0) &&
          (image->colorspace == CMYKColorspace))
        {
          pixel.index=(MagickRealType) sharp_indexes[x]-(MagickRealType)
            indexes[x];
          if (fabs(2.0*pixel.index) < quantum_threshold)
            pixel.index=(MagickRealType) sharp_indexes[x];
          else
            pixel.index=(MagickRealType) sharp_indexes[x]+(pixel.index*amount);
