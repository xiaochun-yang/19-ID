...
#define NormalizeImageTag  "Normalize/Image"
  ...
  */
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  histogram=(MagickPixelPacket *) AcquireMagickMemory((MaxMap+1)*
    sizeof(*histogram));
  normalize_map=(MagickPixelPacket *) AcquireMagickMemory((MaxMap+1)*
...
#define LinearStretchImageTag  "LinearStretch/Image"
  ...
  */
  assert(image != (Image *) NULL);
  assert(image->signature == MagickSignature);
  histogram=(MagickRealType *) AcquireMagickMemory((MaxMap+1)*
    sizeof(*histogram));
  ...
  /*
    Form histogram.
  */
  ResetMagickMemory(histogram,0,(MaxMap+1)*sizeof(*histogram));
  for (y=0; y < (long) image->rows; y++)
  {
    p=AcquireImagePixels(image,0,y,image->columns,1,&image->exception);
    if (p == (const PixelPacket *) NULL)
