...
MagickExport FxInfo *DestroyFxInfo(FxInfo *fx_info)
{
  ...
  fx_info->expression=(char *) RelinquishMagickMemory(fx_info->expression);
...
#define EvaluateImageTag  "Constant/Image "
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      ...
      if (((channel & OpacityChannel) != 0) && (image->matte != MagickFalse))
        q->opacity=QuantumRange-ApplyEvaluateOperator(QuantumRange-q->opacity,
          op,value);
...
%        const ChannelType channel,MagickRealType *alpha,
%        Exceptioninfo *exception)
...
static MagickRealType
  FxEvaluateSubexpression(FxInfo *,const char *,MagickRealType *,
    ExceptionInfo *);

static inline MagickRealType FxMax(FxInfo *fx_info,const char *expression,
  ExceptionInfo *exception)
{
  MagickRealType
    alpha,
  ...
  alpha=FxEvaluateSubexpression(fx_info,expression,&beta,exception);
  return((MagickRealType) MagickMax((double) alpha,(double) beta));
}

static inline MagickRealType FxMin(FxInfo *fx_info,const char *expression,
  ExceptionInfo *exception)
{
  MagickRealType
    alpha,
...
static MagickRealType FxGetSymbol(FxInfo *fx_info,const char *expression,
  ExceptionInfo *exception)
{
  char
    *q,
          ...
          if (*p == '[')
            {
              ...
              *q='\0';
              alpha=FxEvaluateSubexpression(fx_info,subexpression,&beta,
                exception);
              i=(long) (alpha+0.5);
              p++;
            }
  ...
  pixel=InterpolatePixelColor(image,image_view,image->interpolate,(double) x,
    (double) y,exception);
  ...
  if (*symbol == '\0')
    {
      switch (fx_info->channel)
      {
        ...
        case OpacityChannel: return(QuantumScale*(QuantumRange-pixel.opacity));
        case IndexChannel:
        {
      ...
      if (LocaleCompare(symbol,"a") == 0)
        {
          if (image->matte == MagickFalse)
            return(0.0);
          return(QuantumScale*(QuantumRange-pixel.opacity));
        }
      break;
    }
    case 'B':
      ...
      if (LocaleCompare(symbol,"hue") == 0)
        {
          ...
          TransformHSB(RoundToQuantum(pixel.red),RoundToQuantum(pixel.green),
      ...
      if (LocaleCompare(symbol,"z") == 0)
        {
          ...
          depth=(MagickRealType) GetImageChannelDepth(image,fx_info->channel,
            fx_info->exception);
          return(depth);
        }
      break;
  ...
  if (p != (const char *) NULL)
    {
      (void) CopyMagickString(subexpression,expression,(size_t)
        (p-expression+1));
      alpha=FxEvaluateSubexpression(fx_info,subexpression,beta,exception);
      switch ((unsigned char) *p)
      {
        case '~':
        {
          *beta=FxEvaluateSubexpression(fx_info,++p,beta,exception);
        ...
        case '^':
        {
          *beta=pow((double) alpha,(double) FxEvaluateSubexpression(fx_info,++p,
            beta,exception));
          return(*beta);
        }
        case '*':
        ...
        case LeftShiftOperator:
        {
          gamma=FxEvaluateSubexpression(fx_info,++p,beta,exception);
        ...
        case '?':
        {
          ...
          rho=FxEvaluateSubexpression(fx_info,p,beta,exception);
          sigma=FxEvaluateSubexpression(fx_info,q,beta,exception);
          return(alpha != 0.0 ? rho : sigma);
        }
        case '=':
        {
          char
          ...
          *beta=FxEvaluateSubexpression(fx_info,++p,beta,exception);
          ...
          (void) RemoveNodeFromSplayTree(fx_info->symbols,subexpression);
        ...
        default:
          return(alpha*FxEvaluateSubexpression(fx_info,p,beta,exception));
      }
    }
  if (strchr("(",(int) *expression) != (char *) NULL)
    {
      ...
      return(FxEvaluateSubexpression(fx_info,subexpression,beta,exception));
    }
  switch (*expression)
  {
    case '+':
    {
      return(FxEvaluateSubexpression(fx_info,expression+1,beta,exception));
    }
    case '-':
    {
      return(-1.0*FxEvaluateSubexpression(fx_info,expression+1,beta,exception));
    }
    case '~':
    {
      gamma=FxEvaluateSubexpression(fx_info,expression+1,beta,exception);
      alpha=(MagickRealType) (~(unsigned long) (gamma+0.5));
      return(alpha);
    }
    case 'A':
    case 'a':
    {
      if (LocaleNCompare(expression,"abs",3) == 0)
        {
          alpha=FxEvaluateSubexpression(fx_info,expression+3,beta,exception);
          return((MagickRealType) fabs((double) alpha));
        }
      if (LocaleNCompare(expression,"acos",4) == 0)
        {
          alpha=FxEvaluateSubexpression(fx_info,expression+4,beta,exception);
      ...
      if (LocaleNCompare(expression,"atan2",5) == 0)
        {
          alpha=FxEvaluateSubexpression(fx_info,expression+5,beta,exception);
      ...
      if (LocaleCompare(expression,"a") == 0)
        return(FxGetSymbol(fx_info,expression,exception));
      break;
    }
    case 'B':
    case 'b':
      ...
      if (LocaleNCompare(expression,"debug",5) == 0)
        {
          ...
          if (fx_info->images->colorspace == CMYKColorspace)
            switch (fx_info->channel)
            {
              case RedChannel: channel="cyan"; break;
              case GreenChannel: channel="magenta"; break;
              case BlueChannel: channel="yellow"; break;
              case OpacityChannel: channel="opacity"; break;
              case IndexChannel: channel="black"; break;
              default: channel="unknown"; break;
            }
          else
            switch (fx_info->channel)
            {
              case RedChannel: channel="red"; break;
              case GreenChannel: channel="green"; break;
              case BlueChannel: channel="blue"; break;
              case OpacityChannel: channel="opacity"; break;
              default: channel="unknown"; break;
            }
          ...
          (void) fprintf(stderr,"%s[%ld,%ld].%s: %s=%g\n",
            fx_info->images->filename,(long) fx_info->y,(long) fx_info->x,
            channel,subexpression,(double) alpha);
          return(0.0);
        }
      break;
    }
    ...
    case 'l':
    {
      if (LocaleNCompare(expression,"ln",2) == 0)
        {
          alpha=FxEvaluateSubexpression(fx_info,expression+2,beta,exception);
    ...
    case 'm':
    {
      ...
      if (LocaleNCompare(expression,"max",3) == 0)
        return(FxMax(fx_info,expression+3,exception));
      if (LocaleNCompare(expression,"min",3) == 0)
        return(FxMin(fx_info,expression+3,exception));
...
MagickExport MagickBooleanType FxEvaluateExpression(FxInfo *fx_info,
  MagickRealType *alpha,ExceptionInfo *exception)
{
  return(FxEvaluateChannelExpression(fx_info,GrayChannel,alpha,exception));
}

MagickExport MagickBooleanType FxEvaluateChannelExpression(FxInfo *fx_info,
  const ChannelType channel,MagickRealType *alpha,ExceptionInfo *exception)
{
  MagickRealType
    beta;

  fx_info->channel=channel;
  *alpha=FxEvaluateSubexpression(fx_info,fx_info->expression,&beta,exception);
...
#define FxImageTag  "Fx/Image"
  ...
  */
  ...
  for (y=0; y < (long) fx_image->rows; y++)
  {
    ...
    fx_info->y=(MagickRealType) y;
    for (x=0; x < (long) fx_image->columns; x++)
    {
      fx_info->x=(MagickRealType) x;
      if ((channel & RedChannel) != 0)
        {
          status=FxEvaluateChannelExpression(fx_info,RedChannel,&alpha,
            exception);
          q->red=RoundToQuantum((MagickRealType) QuantumRange*alpha);
        }
      if ((channel & GreenChannel) != 0)
        {
          status=FxEvaluateChannelExpression(fx_info,GreenChannel,&alpha,
            exception);
          q->green=RoundToQuantum((MagickRealType) QuantumRange*alpha);
        }
      if ((channel & BlueChannel) != 0)
        {
          status=FxEvaluateChannelExpression(fx_info,BlueChannel,&alpha,
            exception);
          q->blue=RoundToQuantum((MagickRealType) QuantumRange*alpha);
        }
      if ((channel & OpacityChannel) != 0)
        {
          status=FxEvaluateChannelExpression(fx_info,OpacityChannel,&alpha,
            exception);
      ...
      if (((channel & IndexChannel) != 0) && (indexes != (IndexPacket *) NULL))
        {
          status=FxEvaluateChannelExpression(fx_info,IndexChannel,&alpha,
            exception);
          indexes[x]=(IndexPacket) RoundToQuantum((MagickRealType)
            QuantumRange*alpha);
        }
      q++;
    }
...
#define ImplodeImageTag  "Implode/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    q=GetCacheView(implode_view,0,y,implode_image->columns,1);
    if (q == (PixelPacket *) NULL)
      break;
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      ...
      */
      ...
      if (distance >= (radius*radius))
        {
          p=AcquireCacheView(image_view,x,y,1,1,exception);
      ...
      else
        {
          ...
          */
          ...
          pixel=InterpolatePixelColor(image,image_view,image->interpolate,
            (double) (factor*delta.x/scale.x+center.x),
            (double) (factor*delta.y/scale.y+center.y),exception);
        }
  ...
  const double angle,ExceptionInfo *exception)
{
  ...
  */
  ...
  if (value != (const char *) NULL)
    {
      ...
      */
      ...
      if (status == MagickFalse)
        ...
        {
          (void) QueryColorDatabase("#f0f0ff",&caption_image->background_color,
            exception);
  ...
  (void) QueryColorDatabase("#f0f0ff",&picture_image->background_color,
    exception);
  ...
  bend_image=WaveImage(picture_image,0.01*picture_image->columns,2.0*
    picture_image->rows,exception);
  picture_image=DestroyImage(picture_image);
  ...
  for (y=0; y < (long) recolor_image->rows; y++)
  {
    ...
    for (x=0; x < (long) recolor_image->columns; x++)
    {
      ...
      switch (order)
      {
        ...
        case 1:
        {
          pixel.red=k[0]*pixel.red;
          break;
        }
        case 2:
        {
          pixel.red=k[0]*pixel.red+k[1]*pixel.green;
          pixel.green=k[2]*pixel.red+k[3]*pixel.green;
          break;
        }
        case 3:
        {
          pixel.red=k[0]*pixel.red+k[1]*pixel.green+k[2]*pixel.blue;
          pixel.green=k[3]*pixel.red+k[4]*pixel.green+k[5]*pixel.blue;
          pixel.blue=k[6]*pixel.red+k[7]*pixel.green+k[8]*pixel.blue;
          break;
        }
        case 4:
        {
          pixel.red=k[0]*pixel.red+k[1]*pixel.green+k[2]*pixel.blue+
            k[12]*QuantumRange;
          pixel.green=k[4]*pixel.red+k[5]*pixel.green+k[6]*pixel.blue+
            k[13]*QuantumRange;
          pixel.blue=k[8]*pixel.red+k[9]*pixel.green+k[10]*pixel.blue+
            k[14]*QuantumRange;
          break;
        }
        case 5:
        {
          ...
          pixel.green=k[5]*pixel.red+k[6]*pixel.green+k[7]*pixel.blue+
            k[8]*(QuantumRange-pixel.opacity)+k[21]*QuantumRange;
          pixel.blue=k[10]*pixel.red+k[11]*pixel.green+k[12]*pixel.blue+
            k[13]*(QuantumRange-pixel.opacity)+k[22]*QuantumRange;
          if (image->matte != MagickFalse)
            pixel.opacity=QuantumRange-k[15]*pixel.red+k[16]*pixel.green+
              k[17]*pixel.blue+k[18]*(QuantumRange-pixel.opacity)+
              k[23]*QuantumRange;
          break;
        }
        default:
        {
          pixel.red=k[0]*pixel.red+k[1]*pixel.green+k[2]*pixel.blue+
            k[3]*pixel.index+k[4]*(QuantumRange-pixel.opacity)+
            k[30]*QuantumRange;
          pixel.green=k[6]*pixel.red+k[7]*pixel.green+k[8]*pixel.blue+
            k[9]*pixel.index+k[10]*(QuantumRange-pixel.opacity)+
            k[31]*QuantumRange;
          pixel.blue=k[12]*pixel.red+k[13]*pixel.green+k[14]*pixel.blue+
            k[15]*pixel.index+k[16]*(QuantumRange-pixel.opacity)+
            k[32]*QuantumRange;
          if (image->colorspace == CMYKColorspace)
            pixel.index=k[18]*pixel.red+k[19]*pixel.green+k[20]*pixel.blue+
              k[21]*pixel.index+k[22]*(QuantumRange-pixel.opacity)+
              k[33]*QuantumRange;
          if (image->matte != MagickFalse)
            pixel.opacity=QuantumRange-k[24]*pixel.red+k[25]*pixel.green+
              k[26]*pixel.blue+k[27]*pixel.index+k[28]*(QuantumRange-
              pixel.opacity)+k[34]*QuantumRange;
          break;
        }
      }
      ...
      if (image->colorspace == CMYKColorspace)
        recolor_indexes[x]=RoundToQuantum(pixel.index);
      p++;
      q++;
    }
  ...
  ExceptionInfo *exception)
{
  ...
  if (fabs(sigma) <= MagickEpsilon)
    shadow_image=CloneImage(border_image,0,0,MagickTrue,exception);
  else
    shadow_image=BlurImageChannel(border_image,AlphaChannel,0.0,sigma,
      exception);
  ...
  const double sigma,const double angle,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) random_image->rows; y++)
  {
    ...
    {
      pixel.red=QuantumRange*GetRandomValue();
      pixel.green=pixel.red;
      pixel.blue=pixel.red;
      if (image->colorspace == CMYKColorspace)
...
#define SwirlImageTag  "Swirl/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    q=GetCacheView(swirl_view,0,y,swirl_image->columns,1);
    if (q == (PixelPacket *) NULL)
      break;
      ...
      if (distance >= (radius*radius))
      ...
      else
        {
          ...
          */
          ...
          pixel=InterpolatePixelColor(image,image_view,image->interpolate,
            (double) ((cosine*delta.x-sine*delta.y)/scale.x+center.x),
            (double) ((sine*delta.x+cosine*delta.y)/scale.y+center.y),
            exception);
        }
...
#define WaveImageTag  "Wave/Image"
  ...
  */
  ...
  /*
    Allocate sine map.
  */
  sine_map=(MagickRealType *) AcquireMagickMemory((size_t) wave_image->columns*
    sizeof(*sine_map));
  ...
  */
  ...
  for (y=0; y < (long) wave_image->rows; y++)
  {
    ...
    {
      pixel=InterpolatePixelColor(image,image_view,image->interpolate,
        (double) x,(double) (y-sine_map[x]),exception);
      SetPixelPacket(&pixel,q,indexes+x);
      q++;
    }
  ...
  (void) SetImageVirtualPixelMethod(image,method);
