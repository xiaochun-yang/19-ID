...
MagickExport HashmapInfo *DestroyHashmap(HashmapInfo *hashmap_info)
{
  ...
  if (hashmap_info->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"...");
  AcquireSemaphoreInfo(&hashmap_info->semaphore);
  ...
  hashmap_info->semaphore=DestroySemaphoreInfo(hashmap_info->semaphore);
  ...
  void *(*relinquish_value)(void *))
{
  ...
  RelinquishSemaphoreInfo(list_info->semaphore);
  list_info->semaphore=DestroySemaphoreInfo(list_info->semaphore);
...
MagickExport size_t HashStringType(const void *string)
{
  ...
  UpdateSignature(&signature_info,(const unsigned char *) string,
...
MagickExport size_t HashStringInfoType(const void *string)
{
  ...
  GetSignatureInfo(&signature_info);
  UpdateSignature(&signature_info,GetStringInfoDatum((const StringInfo *)
    string),GetStringInfoLength((const StringInfo *) string));
  FinalizeSignature(&signature_info);
  hash=0;
  for (i=0; i < 8; i++)
    hash^=signature_info.digest[i];
  ...
  void *(*relinquish_key)(void *),void *(*relinquish_value)(void *))
{
  ...
  hashmap_info->map=(LinkedListInfo **) AcquireMagickMemory((size_t)
    (capacity+1)*sizeof(*hashmap_info->map));
  ...
  hashmap_info->semaphore=(SemaphoreInfo *) NULL;
  hashmap_info->signature=MagickSignature;
  return(hashmap_info);
}

...
#define MaxCapacities  20
  ...
  */
  ...
  map=(LinkedListInfo **) AcquireMagickMemory((size_t) (capacity+1)*
    sizeof(*map));
