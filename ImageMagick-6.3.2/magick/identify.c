...
#define IdentifyFormat "    %s:\n      Min: %lu (%g)\n      Max: %lu (%g)\n" \
  ...
  */
  ...
  (void) fprintf(file,"  Geometry: %lux%lu%+ld%+ld\n",image->columns,
    image->rows,image->origin.x,image->origin.y);
  if ((image->magick_columns != 0) || (image->magick_rows != 0))
    if ((image->magick_columns != image->columns) ||
        (image->magick_rows != image->rows))
      (void) fprintf(file,"  Base geometry: %lux%lu\n",image->magick_columns,
        image->magick_rows);
  (void) fprintf(file,"  Type: %s\n",MagickOptionToMnemonic(MagickImageOptions,
    (long) type));
      ...
      if (image->colors <= 1024)
        {
          ...
          {
            SetMagickPixelPacket(p,(IndexPacket *) NULL,&pixel);
            (void) QueryMagickColorname(image,&pixel,SVGCompliance,MagickFalse,
              color,&image->exception);
            (void) QueryMagickColorname(image,&pixel,SVGCompliance,MagickTrue,
              hex,&image->exception);
            (void) fprintf(file,"  %8ld: %s %s\n",i,hex,color);
            p++;
          }
        }
    }
