  ...
  unsigned long *number_messages,ExceptionInfo *exception)
{
  ...
  *number_messages=0;
  ...
  if (p == (const LocaleInfo *) NULL)
    return((const LocaleInfo **) NULL);
  messages=(const LocaleInfo **) AcquireMagickMemory((size_t)
    (GetNumberOfNodesInSplayTree(locale_list)+1)*sizeof(*messages));
  ...
  unsigned long *number_messages,ExceptionInfo *exception)
{
  ...
  *number_messages=0;
  ...
  AcquireSemaphoreInfo(&locale_semaphore);
  RelinquishSemaphoreInfo(locale_semaphore);
  messages=(char **) AcquireMagickMemory((size_t)
    (GetNumberOfNodesInSplayTree(locale_list)+1)*sizeof(*messages));
      ...
      (instantiate_locale == MagickFalse))
    {
      ...
      if ((locale_list == (SplayTreeInfo *) NULL) &&
          (instantiate_locale == MagickFalse))
        {
          ...
          locale=(char *) RelinquishMagickMemory(locale);
...
static void *DestroyLocaleNode(void *locale_info)
{
  ...
  if (p->message != (char *) NULL)
    p->message=(char *) RelinquishMagickMemory(p->message);
  return(RelinquishMagickMemory(p));
}

static MagickBooleanType TraverseLocaleMap(const char *filename,
  XMLTreeInfo **nodes,XMLTreeInfo *node,const unsigned long depth,
  ExceptionInfo *exception)
{
  MagickBooleanType
    status;
    ...
    if (node != (XMLTreeInfo *) NULL)
      {
        ...
        if (strcmp(GetXMLTreeTag(node),"message") == 0)
          {
            ...
            *tag='\0';
            ...
            {
              (void) ConcatenateMagickString(tag,GetXMLTreeTag(nodes[i]),
                MaxTextExtent);
            ...
            if (attribute != (const char *) NULL)
              {
                (void) ConcatenateMagickString(tag,attribute,MaxTextExtent);
                (void) ConcatenateMagickString(tag,"/",MaxTextExtent);
              }
            ...
            *message='\0';
            ...
            if (content != (const char *) NULL)
              {
                (void) CopyMagickString(message,content,MaxTextExtent);
                StripString(message);
              }
            locale_info->message=ConstantString(message);
            status=AddValueToSplayTree(locale_list,ConstantString(
              locale_info->tag),locale_info);
        ...
        if (child != (XMLTreeInfo *) NULL)
          status=TraverseLocaleMap(filename,nodes,child,depth+1,exception);
        sibling=GetXMLTreeOrdered(node);
        if (sibling != (XMLTreeInfo *) NULL)
          status=TraverseLocaleMap(filename,nodes,sibling,depth,exception);
      }
  return(status);
}
  ...
  const char *locale,const unsigned long depth,ExceptionInfo *exception)
{
  ...
  */
  ...
  if (locale_list == (SplayTreeInfo *) NULL)
    {
      locale_list=NewSplayTree(CompareSplayTreeString,RelinquishMagickMemory,
        DestroyLocaleNode);
      if (locale_list == (SplayTreeInfo *) NULL)
        {
  ...
  nodes=(XMLTreeInfo **) AcquireMagickMemory(MaxRecursionDepth*sizeof(*nodes));
  if (nodes == (XMLTreeInfo **) NULL)
    {
  ...
  status=TraverseLocaleMap(filename,nodes,locale_map,0,exception);
  nodes=(XMLTreeInfo **) RelinquishMagickMemory(nodes);
  locale_map=DestroyXMLTree(locale_map);
  return(status);
}

