  ...
  for (p=log_info->format; *p != '\0'; p++)
  {
    ...
    if ((size_t) (q-text+MaxTextExtent) >= extent)
      {
        ...
        text=(char *) ResizeMagickMemory(text,(extent+MaxTextExtent)*
          sizeof(*text));
        if (text == (char *) NULL)
          return((char *) NULL);
        q=text+strlen(text);
    ...
    */
    ...
    switch (*p)
    {
      case 'c':
      {
        (void) CopyMagickString(q,GetClientName(),extent);
        q+=strlen(GetClientName());
        break;
      }
      case 'd':
      {
        (void) CopyMagickString(q,domain,extent);
        q+=strlen(domain);
        break;
      }
      case 'e':
      {
        (void) CopyMagickString(q,event,extent);
        q+=strlen(event);
        break;
      }
      case 'f':
      {
        (void) CopyMagickString(q,function,extent);
        q+=strlen(function);
        break;
      }
      case 'g':
      ...
      case 'm':
      {
        ...
        (void) CopyMagickString(q,p,extent);
        q+=strlen(p);
        break;
      }
      case 'n':
      {
        (void) CopyMagickString(q,GetLogName(),extent);
        q+=strlen(GetLogName());
        break;
      }
      case 'p':
      ...
      case 'v':
      {
        (void) CopyMagickString(q,MagickLibVersionText,extent);
  ...
  for (p=log_info->filename; *p != '\0'; p++)
  {
    *q='\0';
    if ((size_t) (q-filename+MaxTextExtent) >= extent)
      {
        extent+=MaxTextExtent;
        filename=(char *) ResizeMagickMemory(filename,(extent+MaxTextExtent)*
          sizeof(*filename));
