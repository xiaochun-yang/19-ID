...
static void *DestroyMimeElement(void *mime_info)
{
  ...
  if (p->pattern != (char *) NULL)
    p->pattern=(char *) RelinquishMagickMemory(p->pattern);
  if (p->description != (char *) NULL)
    p->description=(char *) RelinquishMagickMemory(p->description);
  if (p->type != (char *) NULL)
    p->type=(char *) RelinquishMagickMemory(p->type);
  if (p->path != (char *) NULL)
  ...
  */
  ...
  while (p != (const MimeInfo *) NULL)
  {
    ...
    if ((p->pattern != (char *) NULL) && (filename != (char *) NULL))
      {
        if (GlobExpression(filename,p->pattern) != MagickFalse)
          mime_info=p;
        p=(const MimeInfo *) GetNextValueInLinkedList(mime_list);
        continue;
      }
  ...
  unsigned long *number_aliases,ExceptionInfo *exception)
{
  ...
  *number_aliases=0;
  ...
  if (p == (const MimeInfo *) NULL)
    return((const MimeInfo **) NULL);
  aliases=(const MimeInfo **) AcquireMagickMemory((size_t)
    (GetNumberOfElementsInLinkedList(mime_list)+1)*sizeof(*aliases));
  ...
  */
  ...
  for (i=0; p != (const MimeInfo *) NULL; )
  {
    if ((p->stealth == MagickFalse) &&
        (GlobExpression(p->type,pattern) != MagickFalse))
      aliases[i++]=p;
    p=(const MimeInfo *) GetNextValueInLinkedList(mime_list);
  }
  RelinquishSemaphoreInfo(mime_semaphore);
  ...
  unsigned long *number_aliases,ExceptionInfo *exception)
{
  ...
  *number_aliases=0;
  ...
  if (p == (const MimeInfo *) NULL)
    return((char **) NULL);
  aliases=(char **) AcquireMagickMemory((size_t)
    (GetNumberOfElementsInLinkedList(mime_list)+1)*sizeof(*aliases));
