  ...
  Image **image,const int argc,char **argv)
{
  ...
  */
  ...
  length=GetMagickModulePath(name,MagickFilterModule,path,&(*image)->exception);
  if (length == 0)
    return(MagickFalse);
  /*
    Open the module.
  ...
  unsigned long *number_modules,ExceptionInfo *exception)
{
  ...
  */
  ...
  length=GetMagickModulePath(filename,MagickCoderModule,module_path,
    exception);
  if (length == 0)
    return((char **) NULL);
...
# error MagickCoderModulesPath or __WINDOWS__ must be defined when UseInstalledMagick is defined
#endif
#else
  {
    char
      ...
      if ((module_list == (SplayTreeInfo *) NULL) &&
          (instantiate_module == MagickFalse))
        {
          ...
          status=AddValueToSplayTree(module_list,
            ConstantString(module_info->tag),module_info);
          if (status == MagickFalse)
            {
              char
                *message;
  ...
  ExceptionInfo *exception)
{
  ...
  *path='\0';
  length=GetMagickModulePath(filename,MagickCoderModule,path,exception);
  if (length == 0)
    {
      RelinquishSemaphoreInfo(module_semaphore);
      return(MagickFalse);
  ...
  ExceptionInfo *exception)
{
  ...
  if (module_list == (SplayTreeInfo *) NULL)
    return((const ModuleInfo *) NULL);
  status=AddValueToSplayTree(module_list,ConstantString(module_info->tag),
    module_info);
  if (status == MagickFalse)
