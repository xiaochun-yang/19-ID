      ...
      if ((icc_profile != (const StringInfo *) NULL) &&
          (CompareStringInfo(icc_profile,profile) == 0))
        {
          ...
          if (LocaleCompare(value,"1") != 0)
            {
              (void) CreatesRGBImageProfile(image);
              (void) DeleteImageProperty(image,"EXIF:ColorSpace");
            }
          value=GetImageProperty(image,"EXIF:InteroperabilityIndex");
          if (LocaleCompare(value,"R98.") != 0)
            {
              (void) CreatesRGBImageProfile(image);
              (void) DeleteImageProperty(image,"EXIF:InteroperabilityIndex");
            }
          icc_profile=GetImageProfile(image,"icc");
        }
      if ((icc_profile != (const StringInfo *) NULL) &&
...
#endif
          source_profile=cmsOpenProfileFromMem((unsigned char *)
            GetStringInfoDatum(icc_profile),(DWORD)
            GetStringInfoLength(icc_profile));
          target_profile=cmsOpenProfileFromMem((unsigned char *)
            GetStringInfoDatum(profile),(DWORD) GetStringInfoLength(profile));
...
static void WriteProfileLong(const EndianType endian,const unsigned long value,
  unsigned char *p)
{
  unsigned char
    buffer[4];
