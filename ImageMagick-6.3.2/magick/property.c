  ...
  const Image *clone_image)
{
  ...
  image->origin=clone_image->origin;
  ...
  if (clone_image->properties == (void *) NULL)
    return(MagickTrue);
  ResetImagePropertyIterator(clone_image);
  for (property=GetNextImageProperty(clone_image); property != (char *) NULL; )
  {
    value=GetImageProperty(clone_image,property);
    if (value != (const char *) NULL)
      (void) SetImageProperty(image,property,value);
    property=GetNextImageProperty(clone_image);
  }
  return(MagickTrue);
}

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  DeleteImageProperty() deletes an key from the image map.
%
%  The format of the DeleteImageProperty method is:
%
%      MagickBooleanType DeleteImageProperty(Image *image,
%        const char *key)
%
%  A description of each parameter follows:
%
  ...
  const char *property)
{
  ...
  if (image->properties == (void *) NULL)
    return(MagickFalse);
  return(RemoveNodeFromSplayTree((SplayTreeInfo *) image->properties,property));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
static int ReadPropertyByte(unsigned char **p,size_t *length)
{
  int
    c;
...
static unsigned long ReadPropertyMSBLong(unsigned char **p,size_t *length)
{
  int
    c;
...
static unsigned short ReadPropertyMSBShort(unsigned char **p,size_t *length)
{
  int
    c;
      ...
      if ((all != 0) || (tag == (unsigned long) tag_value))
        {
          ...
          switch (format)
          {
            ...
            {
              value=(char *) AcquireMagickMemory((size_t) (number_bytes+1)*
                sizeof(*value));
              if (value != (char *) NULL)
                {
                  register long
      ...
      if (LocaleNCompare("pixel:",property,6) == 0)
        {
          ...
          status=FxEvaluateChannelExpression(fx_info,RedChannel,&alpha,
            exception);
          pixel.red=QuantumRange*alpha;
          status|=FxEvaluateChannelExpression(fx_info,GreenChannel,&alpha,
            exception);
          pixel.green=QuantumRange*alpha;
          status|=FxEvaluateChannelExpression(fx_info,BlueChannel,&alpha,
            exception);
          pixel.blue=QuantumRange*alpha;
          if (image->matte != MagickFalse)
            {
              status|=FxEvaluateChannelExpression(fx_info,OpacityChannel,&alpha,
                exception);
              pixel.opacity=QuantumRange*(1.0-alpha);
            }
          if (image->colorspace == CMYKColorspace)
            {
              status|=FxEvaluateChannelExpression(fx_info,BlackChannel,&alpha,
                exception);
              pixel.index=QuantumRange*alpha;
            }
          fx_info=DestroyFxInfo(fx_info);
          if (status != MagickFalse)
            {
  ...
  for (q=interpret_text; *p != '\0'; p++)
  {
    *q='\0';
    if ((size_t) (q-interpret_text+MaxTextExtent) >= extent)
      {
        extent+=MaxTextExtent;
        interpret_text=(char *) ResizeMagickMemory(interpret_text,(extent+
    ...
    switch (*p)
    {
      case 'b':
      {
        ...
        */
        ...
        (void) ConcatenateMagickString(q,format,extent);
        q+=strlen(format);
        break;
      }
      case 'c':
      {
        ...
        */
        ...
        if ((size_t) (q-interpret_text+length+1) >= extent)
          {
            extent+=length;
            interpret_text=(char *) ResizeMagickMemory(interpret_text,
        ...
        */
        ...
        switch (*p)
        {
          case 'd':
          {
            ...
            */
            GetPathComponent(image->magick_filename,HeadPath,filename);
            (void) CopyMagickString(q,filename,extent);
            q+=strlen(filename);
            break;
          }
          case 'e':
      ...
      case 'i':
      {
        /*
          Image filename.
        */
        (void) CopyMagickString(q,image->filename,extent);
      ...
      case 'm':
      {
        /*
          Image format.
        */
        (void) CopyMagickString(q,image->magick,extent);
      ...
      case 'M':
      {
        ...
        */
        (void) CopyMagickString(q,image->magick_filename,extent);
      ...
      case 'o':
      {
        ...
        */
        (void) CopyMagickString(q,text_info->filename,extent);
      ...
      case '[':
      {
        ...
        */
        ...
        if (value != (const char *) NULL)
          ...
          if (IsGlob(property) != MagickFalse)
            {
              /*
              ...
              */
              ...
              {
                if (GlobExpression(property,key) != MagickFalse)
                  {
                    ...
                    if (value != (const char *) NULL)
                      {
                        ...
                        if ((size_t) (q-interpret_text+length+1) >= extent)
                          {
                            extent+=length;
                            interpret_text=(char *) ResizeMagickMemory(
        ...
        value=GetMagickProperty(text_info,image,property);
        if (value != (const char *) NULL)
          {
            length=strlen(value);
            if ((size_t) (q-interpret_text+length+1) >= extent)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  RemoveImageProperty() removes an property from the image map.
%
%  The format of the RemoveImageProperty method is:
%
%      MagickBooleanType RemoveImageProperty(Image *image,
%        const char *property)
%
%  A description of each parameter follows:
%
...
%
*/
MagickExport MagickBooleanType RemoveImageProperty(Image *image,
  const char *property)
{
  assert(image != (Image *) NULL);
  assert(image->signature == MagickSignature);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ResetImagePropertyIterator() resets the image values iterator.  Use it
  ...
  const char *property,const char *value)
{
  ...
  switch (*property)
  {
    ...
    case 'c':
    {
      if (LocaleCompare(property,"comment") == 0)
        {
          status=AddValueToSplayTree((SplayTreeInfo *) image->properties,
            ConstantString(property),InterpretImageProperties((ImageInfo *)
            NULL,image,value));
          break;
        }
      break;
    ...
    case 'l':
    {
      if (LocaleCompare(property,"label") == 0)
        {
          status=AddValueToSplayTree((SplayTreeInfo *) image->properties,
            ConstantString(property),InterpretImageProperties((ImageInfo *)
            NULL,image,value));
    ...
    case 'o':
    {
      if (LocaleCompare(property,"origin") == 0)
        {
          char
            *geometry;
