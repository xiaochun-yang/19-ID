  ...
  const CubeInfo *cube_info,const PixelPacket *pixel)
{
  ...
  alpha=QuantumScale*(QuantumRange-(MagickRealType) pixel->opacity);
  ...
  if ((cube_info->quantize_info->number_colors == 2) &&
      (cube_info->quantize_info->colorspace == GRAYColorspace))
    {
      ...
      */
      ...
      {
        intensity=(Quantum) (PixelIntensity(q) < (QuantumRange/2.0) ? 0 : 
          QuantumRange);
        q->red=intensity;
        q->green=intensity;
        q->blue=intensity;
...
#define ClassifyImageTag  "Classify/Image"
  ...
  */
  ...
  /*
    More than maximim colors;  classify to the cube_info->depth tree depth.
  */
  ...
  const NodeInfo *node_info)
{
  ...
  */
  number_children=IsAssociatedAlpha(image,cube_info) == MagickFalse ? 8L : 16L;
  ...
  if (node_info->number_unique != 0)
    {
      ...
      */
      ...
      if (IsAssociatedAlpha(image,cube_info) == MagickFalse)
        {
          q->red=RoundToQuantum(alpha*QuantumRange*node_info->total_color.red);
          q->green=RoundToQuantum(alpha*QuantumRange*
            node_info->total_color.green);
          q->blue=RoundToQuantum(alpha*QuantumRange*
      ...
      else
        {
          ...
          opacity=alpha*QuantumRange*node_info->total_color.opacity;
          q->opacity=RoundToQuantum(opacity);
          if (q->opacity == OpaqueOpacity)
            {
              q->red=RoundToQuantum(alpha*QuantumRange*
          ...
          else
            {
              ...
              gamma=QuantumScale*(QuantumRange-(MagickRealType) q->opacity);
              gamma=1.0/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
              q->red=RoundToQuantum(alpha*gamma*QuantumRange*
                node_info->total_color.red);
              q->green=RoundToQuantum(alpha*gamma*QuantumRange*
                node_info->total_color.green);
              q->blue=RoundToQuantum(alpha*gamma*QuantumRange*
                node_info->total_color.blue);
  ...
  if ((p->x >= 0) && (p->x < (long) image->columns) &&
      (p->y >= 0) && (p->y < (long) image->rows))
    {
      /*
        Distribute error.
      */
      q=GetCacheView(image_view,p->x,p->y,1,1);
      if (q == (PixelPacket *) NULL)
        return(MagickFalse);
      indexes=GetCacheViewIndexes(image_view);
      pixel=AssociateAlphaPixel(image,cube_info,q);
      ...
      */
      ...
      p->error[i].red=pixel.red-color.red;
      p->error[i].green=pixel.green-color.green;
      p->error[i].blue=pixel.blue-color.blue;
      if (IsAssociatedAlpha(image,cube_info) != MagickFalse)
        p->error[i].opacity=pixel.opacity-color.opacity;
      if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
...
static MagickBooleanType DitherImage(Image *image,CubeInfo *cube_info)
{
  ...
  */
  (void) ResetMagickMemory(cube_info->error,0,ExceptionQueueLength*
...
%
%      CubeInfo GetCubeInfo(const Image *image,
%        const QuantizeInfo *quantize_info,const unsigned long depth,
...
%
*/
static CubeInfo *GetCubeInfo(const Image *image,
  const QuantizeInfo *quantize_info,const unsigned long depth,
  const unsigned long maximum_colors)
{
  CubeInfo
  ...
  */
  length=(size_t) (1 << ((IsAssociatedAlpha(image,cube_info) == MagickFalse ?
    3 : 4)*(8-CacheShift)));
  cube_info->cache=(long *) AcquireMagickMemory(length*
  ...
  */
  ...
  for (i=0; i < ExceptionQueueLength; i++)
  {
    ...
    weight*=exp(log(((double) QuantumRange+1.0))/(ExceptionQueueLength-1.0));
  }
  /*
  ...
  if (cube_info->free_nodes == 0)
    {
      ...
      */
      ...
      nodes->nodes=(NodeInfo *) AcquireMagickMemory(NodesInAList*
        sizeof(*nodes->nodes));
...
MagickExport MagickBooleanType GetImageQuantizeError(Image *image)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      ...
      if (image->matte != MagickFalse)
        {
          alpha=QuantumScale*(QuantumRange-(MagickRealType) p->opacity);
          beta=QuantumScale*(QuantumRange-(MagickRealType)
            image->colormap[index].opacity);
        }
      distance=(MagickRealType) fabs((double) (alpha*p->red-beta*
        image->colormap[index].red));
      mean_error_per_pixel+=distance;
      mean_error+=distance*distance;
      if (distance > maximum_error)
        maximum_error=distance;
      distance=(MagickRealType) fabs((double) (alpha*p->green-beta*
        image->colormap[index].green));
      mean_error_per_pixel+=distance;
      mean_error+=distance*distance;
      if (distance > maximum_error)
        maximum_error=distance;
      distance=(MagickRealType) fabs((double) (alpha*p->blue-beta*
  ...
  const MagickBooleanType dither)
{
  ...
  */
  ...
  cube_info=GetCubeInfo(image,&quantize_info,MaxTreeDepth,
  ...
  const unsigned long levels,const MagickBooleanType dither)
{
  ...
  */
  ...
  if (posterize_image == (Image *) NULL)
    return(MagickFalse);
  l=1;
  while ((l*l*l) < MagickMin((long) levels*levels*levels,MaxColormapSize+1))
    l++;
    ...
    for (j=0; j < l; j++)
      for (k=0; k < l; k++)
      {
        ...
        posterize_image->colormap[n].green=(Quantum)
          (QuantumRange*j/MagickMax(l-1,1));
  ...
  Image *image)
{
  ...
  /*
    Initialize color cube.
  */
  cube_info=GetCubeInfo(image,quantize_info,depth,maximum_colors);
  if (cube_info == (CubeInfo *) NULL)
    ThrowBinaryException(ResourceLimitError,"MemoryAllocationFailed",
      image->filename);
  ...
  Image *images)
{
  ...
  /*
    Initialize color cube.
  */
  cube_info=GetCubeInfo(images,quantize_info,depth,maximum_colors);
  if (cube_info == (CubeInfo *) NULL)
    {
