...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  AdaptiveResizeImage() adaptively resize image with data dependent
%  triangulation.
%
  ...
  for (y=0; y < (long) resize_image->rows; y++)
  {
    ...
    for (x=0; x < (long) resize_image->columns; x++)
    {
      ...
      pixel=InterpolatePixelColor(image,image_view,image->interpolate,
        offset.x-0.5,offset.y-0.5,exception);
      SetPixelPacket(&pixel,q,resize_indexes+x);
      q++;
    }
...
#define ResizeImageTag  "Resize/Image"
  ...
  */
  ...
  if (support > 0.5)
    {
      if (SetImageStorageClass(destination,DirectClass) == MagickFalse)
        {
          InheritException(exception,&destination->exception);
          return(MagickFalse);
        }
    }
  else
  ...
  for (x=0; x < (long) destination->columns; x++)
  {
    ...
    stop=(long) (MagickMin(center+support,(double) source->columns)+0.5);
    density=0.0;
          ...
          if (source->matte == MagickFalse)
            {
              for (i=0; i < n; i++)
              {
                j=y*(contribution[n-1].pixel-contribution[0].pixel+1)+
                  (contribution[i].pixel-contribution[0].pixel);
                alpha=contribution[i].weight;
                pixel.index+=alpha*source_indexes[j];
              }
              indexes[y]=(IndexPacket) RoundToQuantum(pixel.index);
            }
          else
            {
              gamma=0.0;
  ...
  return(x == (long) destination->columns ? MagickTrue : MagickFalse);
}
  ...
  const MagickSizeType span,MagickOffsetType *quantum,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) destination->rows; y++)
  {
    ...
    start=(long) (MagickMax(center-support,0.0)+0.5);
    stop=(long) (MagickMin(center+support,(double) source->rows)+0.5);
    density=0.0;
    for (n=0; n < (stop-start); n++)
    {
      contribution[n].pixel=start+n;
          ...
          if (source->matte == MagickFalse)
            {
              ...
              indexes[x]=(IndexPacket) RoundToQuantum(pixel.index);
            }
          else
            {
              for (i=0; i < n; i++)
  ...
  return(y == (long) destination->rows ? MagickTrue : MagickFalse);
}
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  contribution=(ContributionInfo *) AcquireMagickMemory((size_t)
    (2.0*MagickMax(support,0.5)+3)*sizeof(*contribution));
...
#define SampleImageTag  "Sample/Image"
  ...
  */
  pixels=(PixelPacket *) AcquireMagickMemory((size_t) image->columns*
    sizeof(*pixels));
  x_offset=(long *) AcquireMagickMemory((size_t) sample_image->columns*
    sizeof(*x_offset));
  y_offset=(long *) AcquireMagickMemory((size_t) sample_image->rows*
    sizeof(*y_offset));
...
#define ScaleImageTag  "Scale/Image"
  ...
  */
  ...
  /*
    Allocate memory.
  */
  x_vector=(MagickPixelPacket *) AcquireMagickMemory((size_t) image->columns*
    sizeof(*x_vector));
  scanline=x_vector;
  if (image->rows != scale_image->rows)
    scanline=(MagickPixelPacket *) AcquireMagickMemory((size_t) image->columns*
      sizeof(*scanline));
  scale_scanline=(MagickPixelPacket *) AcquireMagickMemory((size_t)
    scale_image->columns*sizeof(*scale_scanline));
  y_vector=(MagickPixelPacket *) AcquireMagickMemory((size_t) image->columns*
    sizeof(*y_vector));
