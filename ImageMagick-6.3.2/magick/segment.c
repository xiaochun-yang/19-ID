...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Classify() defines on ore more classes.  Each pixel is thresholded to
...
static MagickBooleanType Classify(Image *image,short **extrema,
  const MagickRealType cluster_threshold,
  const MagickRealType magick_unused(weighting_exponent),
  const MagickBooleanType verbose)
{
#define SegmentImageTag  "Segment/Image"
        ...
        if (head != (Cluster *) NULL)
          {
            cluster->next=(Cluster *)
              AcquireMagickMemory(sizeof(*cluster->next));
  ...
  */
  squares=(MagickRealType *) AcquireMagickMemory(513*sizeof(*squares));
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  {
    histogram[i]=(long *) AcquireMagickMemory(256*sizeof(**histogram));
    extrema[i]=(short *) AcquireMagickMemory(256*sizeof(**histogram));
  ...
  const unsigned long number_crossings)
{
  ...
  */
  list=(IntervalTree **) AcquireMagickMemory(TreeLength*sizeof(*list));
            ...
            if (node == head)
              {
                node->child=(IntervalTree *)
                  AcquireMagickMemory(sizeof(*node->child));
                node=node->child;
              }
            else
              {
                node->sibling=(IntervalTree *)
                  AcquireMagickMemory(sizeof(*node->sibling));
                node=node->sibling;
              }
  ...
  short *extrema)
{
  ...
  */
  ...
  zero_crossing=(ZeroCrossing *)
    AcquireMagickMemory((size_t) count*sizeof(*zero_crossing));
  ...
  */
  derivative=(MagickRealType *) AcquireMagickMemory(256*sizeof(*derivative));
  second_derivative=(MagickRealType *)
    AcquireMagickMemory(256*sizeof(*second_derivative));
  if ((derivative == (MagickRealType *) NULL) ||
      (second_derivative == (MagickRealType *) NULL))
    ThrowMagickFatalException(ResourceLimitFatalError,
      "UnableToAllocateDerivatives","OptimalTau");
  i=0;
...
%  A description of each parameter follows.
%
%    o histogram: Specifies an array of MagickRealTypes representing the number of
  ...
  MagickRealType *scale_histogram)
{
  ...
  gamma=(MagickRealType *) AcquireMagickMemory(256*sizeof(*gamma));
  if (gamma == (MagickRealType *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,
      "UnableToAllocateGammaMap","ScaleSpace");
  ...
  const double cluster_threshold,const double smooth_threshold)
{
  ...
  */
  ...
  {
    histogram[i]=(long *) AcquireMagickMemory(256*sizeof(**histogram));
    extrema[i]=(short *) AcquireMagickMemory(256*sizeof(**extrema));
    if ((histogram[i] == (long *) NULL) || (extrema[i] == (short *) NULL))
      {
        for (i-- ; i >= 0; i--)
        {
