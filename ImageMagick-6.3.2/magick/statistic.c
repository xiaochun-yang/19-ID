  ...
  ExceptionInfo *exception)
{
  ...
  for (i=0; i < AllChannels; i++)
  {
    channel_statistics[AllChannels].depth=MagickMax(
      channel_statistics[AllChannels].depth,channel_statistics[i].depth);
    channel_statistics[AllChannels].minima=(Quantum) MagickMin(
      channel_statistics[AllChannels].minima,channel_statistics[i].minima);
    channel_statistics[AllChannels].maxima=(Quantum) MagickMax(
  ...
  const MagickBooleanType constrain)
{
  ...
  if (constrain != MagickFalse)
    depth=MagickMin(depth,QuantumDepth);
  return(depth);
}

/*
  ...
  const ChannelType channel,const unsigned long depth)
{
  ...
  assert(image->signature == MagickSignature);
  y=(long) image->rows;
  if (GetImageDepth(image,&image->exception) > MagickMin(depth,QuantumDepth))
    {
      /*
        Scale pixels to desired depth.
      */
      scale=QuantumRange/(MagickRealType) (QuantumRange >> (QuantumDepth-
        MagickMin(depth,QuantumDepth)));
      for (y=0; y < (long) image->rows; y++)
      {
        ...
        {
          if ((channel & RedChannel) != 0)
            q->red=RoundToQuantum(scale*(q->red/scale));
          if ((channel & GreenChannel) != 0)
            q->green=RoundToQuantum(scale*(q->green/scale));
          if ((channel & BlueChannel) != 0)
            q->blue=RoundToQuantum(scale*(q->blue/scale));
          if (((channel & OpacityChannel) != 0) &&
              (image->matte != MagickFalse))
            q->opacity=RoundToQuantum(scale*(q->opacity/scale));
          if (((channel & IndexChannel) != 0) &&
              (image->colorspace == CMYKColorspace))
            indexes[x]=RoundToQuantum(scale*(indexes[x]/scale));
          q++;
        }
