  ...
  if (string_info->length != 0)
    {
      ...
      extent=CheckOverflowException(string_info->length,MaxTextExtent);
      string_info->datum=(unsigned char *) AcquireMagickMemory(extent*
  ...
  const StringInfo *source)
{
  ...
  extent=CheckOverflowException(string_info->length,source->length);
  SetStringInfoLength(string_info,extent);
...
MagickExport StringInfo *ConfigureFileToStringInfo(const char *filename)
{
  ...
  extent=CheckOverflowException(length,MaxTextExtent);
  string=(char *) AcquireMagickMemory(extent*sizeof(*string));
  if (string == (char *) NULL)
    {
      file=close(file)-1;
      return((StringInfo *) NULL);
  ...
  const size_t length)
{
  ...
  if (string_info->datum == (unsigned char *) NULL)
    ...
    string_info->datum=(unsigned char *) ResizeMagickMemory(string_info->datum,
      extent*sizeof(*string_info->datum));
  if (string_info->datum == (unsigned char *) NULL)
    {
      char
        *message;
  ...
  if (*p == '\0')
    {
      ...
      */
      ...
      for (p=text; *p != '\0'; p++)
        if (*p == '\n')
          lines++;
      textlist=(char **) AcquireMagickMemory((size_t) (lines+1)*
        sizeof(*textlist));
      ...
      for (i=0; i < (long) lines; i++)
      {
        for (q=(char *) p; *q != '\0'; q++)
          if ((*q == '\r') || (*q == '\n'))
            break;
        textlist[i]=(char *) AcquireMagickMemory(((q-p)+MaxTextExtent)*
          sizeof(*textlist));
  ...
  else
    {
      ...
      */
      ...
      for (i=0; i < (long) lines; i++)
      {
        textlist[i]=(char *) AcquireMagickMemory(2*MaxTextExtent*
          sizeof(*textlist));
        if (textlist[i] == (char *) NULL)
          ThrowMagickFatalException(ResourceLimitFatalError,
            "UnableToConvertText",text);
  ...
  const char *search,const char *replace)
{
  ...
  if (match == (char *) NULL)
    return(MagickFalse);
  extent=CheckOverflowException(strlen(source),MaxTextExtent);
  result=(char *) AcquireMagickMemory(extent*sizeof(*result));
  if (result == (char *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,"UnableToAcquireString",
      search);
  *result='\0';
  ...
  for (search_length=strlen(search); match != (char *) NULL; )
  {
    ...
    if (copy_length != 0)
      {
        ...
        if ((result_length+MaxTextExtent) >= extent)
          {
            extent+=copy_length;
            result=(char *) ResizeMagickMemory(result,(extent+MaxTextExtent)*
              sizeof(*result));
            if (result == (char *) NULL)
              ThrowMagickFatalException(ResourceLimitFatalError,
    ...
    */
    ...
    (void) ConcatenateMagickString(result+destination_offset,replace,
      (extent+MaxTextExtent)*sizeof(*result));
  ...
  */
  ...
  (void) ConcatenateMagickString(result+destination_offset,source,(size_t)
    (extent+MaxTextExtent-destination_offset)*sizeof(*result));
