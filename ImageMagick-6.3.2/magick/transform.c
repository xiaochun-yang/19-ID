  ...
  */
  ...
  for (i=0; i < (long) GetImageListLength(images); i+=4)
  {
    ...
    for (y=0; y < (long) images->rows; y++)
    {
      ...
      {
        q->red=QuantumRange-PixelIntensityToQuantum(p);
        p++;
        q++;
      }
    ...
    for (y=0; y < (long) images->rows; y++)
    {
      ...
      for (x=0; x < (long) images->columns; x++)
      {
        q->green=QuantumRange-PixelIntensityToQuantum(p);
        p++;
        q++;
      }
      if (SyncImagePixels(cmyk_image) == MagickFalse)
    ...
    for (y=0; y < (long) images->rows; y++)
    {
      ...
      for (x=0; x < (long) images->columns; x++)
      {
        q->blue=QuantumRange-PixelIntensityToQuantum(p);
        p++;
        q++;
      }
      if (SyncImagePixels(cmyk_image) == MagickFalse)
    ...
    for (y=0; y < (long) images->rows; y++)
    {
      ...
      for (x=0; x < (long) images->columns; x++)
      {
        indexes[x]=QuantumRange-PixelIntensityToQuantum(p);
        p++;
      }
      if (SyncImagePixels(cmyk_image) == MagickFalse)
        break;
...
#define CropImageTag  "Crop/Image"
  ...
  if (((bounding_box.x-page.x) >= (long) page.width) ||
      ...
      ((page.y-bounding_box.y) > (long) image->rows))
    {
      ...
      if (crop_image == (Image *) NULL)
        return((Image *) NULL);
      crop_image->background_color.opacity=TransparentOpacity;
  ...
  */
  ...
  {
    p=AcquireCacheView(image_view,page.x,page.y+y,crop_image->columns,1,
      exception);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ExtentImage() extends the image as defined by the geometry and gravity.
%
