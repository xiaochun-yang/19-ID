  ...
  char *destination)
{
  ...
  quantum=(size_t) (fstat(source_file,&file_info) < 0 ? 0 :
    file_info.st_size);
  ...
  for (length=0; ; )
  {
    count=read(source_file,buffer,quantum);
    if (count <= 0)
      break;
    length=(size_t) count;
    count=write(destination_file,buffer,length);
...
MagickExport unsigned char *Base64Decode(const char *source,size_t *length)
{
  ...
  *length=0;
  quantum=3*strlen(source)/4+1;
  decode=(unsigned char *) AcquireMagickMemory(quantum*sizeof(*decode));
  ...
  const size_t blob_length,size_t *encode_length)
{
  ...
  *encode_length=0;
  ...
  encode=(char *) AcquireMagickMemory(quantum*sizeof(*encode));
  if (encode == (char *) NULL)
    return((char *) NULL);
  i=0;
  ...
  if ((*(path+1) == *DirectorySeparator) || (*(path+1) == '\0'))
    {
      ...
      */
      (void) CopyMagickString(expanded_path,".",MaxTextExtent);
      ...
      if (home != (char *) NULL)
        {
          (void) CopyMagickString(expanded_path,home,MaxTextExtent);
          (void) ConcatenateMagickString(expanded_path,path+1,MaxTextExtent);
          home=(char *) RelinquishMagickMemory(home);
        }
    }
  else
    {
  ...
  if (IsEventLogging() != MagickFalse)
    {
      ...
      command_line=(char *) RelinquishMagickMemory(command_line);
    }
  *argc=(int) count;
  *argv=vector;
  return(MagickTrue);
  ...
  *number_components=1;
  for (p=path; *p != '\0'; p++)
    if (IsBasenameSeparator(*p))
      (*number_components)++;
  components=(char **)
    AcquireMagickMemory((size_t) (*number_components+1)*sizeof(*components));
  if (components == (char **) NULL)
    {
      ...
      ThrowMagickFatalException(ResourceLimitFatalError,
        "memory allocation failed `%s'",message);
  ...
  for (i=0; i < (long) *number_components; i++)
  {
    for (q=(char *) p; *q != '\0'; q++)
      if (IsBasenameSeparator(*q))
        break;
    components[i]=(char *) AcquireMagickMemory(((q-p)+MaxTextExtent)*
      sizeof(*components));
  ...
  if (file == (FILE *) NULL)
    {
      ...
      wchar_t
        wide_mode[MaxTextExtent],
        wide_path[MaxTextExtent];

      /*
        Perhaps this is a UTF-8 encoded filename.
      */
