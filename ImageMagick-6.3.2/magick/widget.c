  ...
  if (event == (XEvent *) NULL)
    {
      ...
      */
      ...
      selection_info=(XWidgetInfo *)
        AcquireMagickMemory(number_selections*sizeof(*selection_info));
      if (selection_info == (XWidgetInfo *) NULL)
        {
          char
            *message;
            ...
            if (anomaly == MagickFalse)
              {
                ...
                */
                ...
                for (i=0; i < (int) number_formats; i++)
                  formats[i]=(char *) RelinquishMagickMemory(formats[i]);
...
#define ResetButtonText  "Reset"
  ...
  */
  listhead=fontlist;
  fontlist=(char **) AcquireMagickMemory(fonts*sizeof(*fontlist));
  if (fontlist == (char **) NULL)
    {
