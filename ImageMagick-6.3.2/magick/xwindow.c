...
/*
  X defines.
*/
#define XBlueGamma(color) ((Quantum) (blue_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) QuantumScale*(color)),1.0/(double) blue_gamma)* \
  QuantumRange)+0.5)))
#define XGammaPixel(map,color)  (unsigned long) (map->base_pixel+ \
  ...
  ((ScaleQuantumToShort(XBlueGamma((color)->blue))*map->blue_max/65535L)* \
    map->blue_mult))
#define XGreenGamma(color) ((Quantum) (green_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) QuantumScale*(color)),1.0/(double) green_gamma)* \
  QuantumRange)+0.5)))
#define XRedGamma(color) ((Quantum) (red_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) QuantumScale*(color)),1.0/(double) red_gamma)* \
  QuantumRange)+0.5)))
...
static char **FontToList(char *font)
{
  ...
  */
  ...
  for (p=font; *p != '\0'; p++)
    if ((*p == ':') || (*p == ';') || (*p == ','))
      fonts++;
  fontlist=(char **) AcquireMagickMemory((fonts+1)*sizeof(*fontlist));
  ...
  for (i=0; i < (int) fonts; i++)
  {
    for (q=p; *q != '\0'; q++)
      if ((*q == ':') || (*q == ';') || (*q == ','))
        break;
    fontlist[i]=(char *) AcquireMagickMemory((size_t) (q-p+1));
      ...
      if (fontlist != (char **) NULL)
        {
          for (i=0; fontlist[i] != (char *) NULL; i++)
          {
            ...
            fontlist[i]=(char *) RelinquishMagickMemory(fontlist[i]);
          }
  ...
  if (query_server != MagickFalse)
    {
      ...
      */
      colors=(XColor *) AcquireMagickMemory(number_colors*sizeof(*colors));
...
MagickExport void XDestroyResourceInfo(XResourceInfo *resource_info)
{
  ...
  if (resource_info->name != (char *) NULL)
    resource_info->name=(char *) RelinquishMagickMemory(resource_info->name);
...
static void XDitherImage(Image *image,XImage *ximage)
{
  ...
  */
  ...
  for (y=0; y < (int) image->rows; y++)
  {
    ...
    {
      color.red=(Quantum)
        red_map[i][j][(int) ScaleQuantumToChar(p->red)] << 8;
      color.green=(Quantum)
        green_map[i][j][(int) ScaleQuantumToChar(p->green)] << 8;
      color.blue=(Quantum)
        blue_map[i][j][(int) ScaleQuantumToChar(p->blue)] << 8;
  ...
  const XResourceInfo *resource_info,Image *image,XPixelInfo *pixel)
{
  ...
  */
  ...
  pixel->pixels=(unsigned long *)
    AcquireMagickMemory(packets*sizeof(pixel->pixels));
  if (pixel->pixels == (unsigned long *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,"UnableToGetPixelInfo",
      image->filename);
  /*
  ...
  */
  ...
  status=XParseColor(display,colormap,resource_info->foreground_color,
    &pixel->foreground_color);
  if (status == False)
    ThrowMagickFatalException(XServerError,"ColorIsNotKnownToServer",
  ...
  const char *client_name)
{
  ...
  */
  ...
  (void) FormatMagickString(filename,MaxTextExtent,"%s%c%s",ApplicationDefaults,
    c,client_name+1);
  ...
  */
  ...
  (void) FormatMagickString(filename,MaxTextExtent,"%s%src",PreferencesDefaults,
    client_name);
  ...
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  resource_value=XGetResourceClass(database,client_name,"confirmEdit","False");
  resource_info->confirm_edit=IsMagickTrue(resource_value);
  resource_value=XGetResourceClass(database,client_name,"delay","1");
  ...
  resource_value=XGetResourceClass(database,client_name,"usePixmap","True");
      ...
      */
      ...
      if (window_info == (WindowInfo *) NULL)
        window_info=(WindowInfo *)
          AcquireMagickMemory(max_windows*sizeof(*window_info));
      else
        window_info=(WindowInfo *)
          ResizeMagickMemory(window_info,max_windows*sizeof(*window_info));
    }
  if (window_info == (WindowInfo *) NULL)
    {
      char
            ...
            if (p == (ColormapInfo *) NULL)
              {
                ...
                */
                colors=(XColor *)
                  AcquireMagickMemory(number_colors*sizeof(*colors));
                if (colors == (XColor *) NULL)
                  {
                    XDestroyImage(ximage);
                    return((Image *) NULL);
      ...
      */
...
#if defined(HasSharedMemory)
      {
        ...
        if (window->segment_info == (void *) NULL)
          window->segment_info=AcquireMagickMemory(2*sizeof(*segment_info));
  ...
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  if (windows == (XWindows *) NULL)
    {
      ...
      message=GetExceptionMessage(errno);
      ThrowXWindowException(XServerFatalError,"MemoryAllocationFailed",
        message);
      message=(char *) RelinquishMagickMemory(message);
  ...
  windows->pixel_info=(XPixelInfo *)
    AcquireMagickMemory(sizeof(*windows->pixel_info));
  windows->icon_pixel=(XPixelInfo *)
    AcquireMagickMemory(sizeof(*windows->icon_pixel));
  windows->icon_resources=(XResourceInfo *)
    AcquireMagickMemory(sizeof(*windows->icon_resources));
  if ((windows->pixel_info == (XPixelInfo *) NULL) ||
  ...
  *windows->icon_resources=(*resource_info);
  ...
  if ((windows->visual_info == (XVisualInfo *) NULL) ||
      (windows->icon_visual == (XVisualInfo *) NULL))
    ThrowMagickFatalException(XServerFatalError,"UnableToGetVisual",
  ...
  if ((source == (Pixmap) NULL) || (mask == (Pixmap) NULL))
    {
      ...
      message=GetExceptionMessage(errno);
      ThrowXWindowException(XServerError,"UnableToCreatePixmap",message);
      message=(char *) RelinquishMagickMemory(message);
      return((Cursor) NULL);
    }
      ...
      if ((width != (unsigned int) window->image->columns) ||
          (height != (unsigned int) window->image->rows))
        {
          ...
          */
          ...
          display_width=(unsigned int) XDisplayWidth(display,window->screen);
          display_height=(unsigned int) XDisplayHeight(display,window->screen);
          if (window->pixel_info->colors != 0)
            ...
            resize_image=ResizeImage(window->image,width,height,
              window->image->filter,window->image->blur,&image->exception);
...
MagickExport void XMakeMagnifyImage(Display *display,XWindows *windows)
{
  ...
  */
  ...
  GetMagickPixelPacket(windows->image.image,&pixel);
  ...
  (void) QueryColorname(windows->image.image,&color,X11Compliance,tuple,
    &windows->image.image->exception);
  (void) FormatMagickString(text,MaxTextExtent," %+d%+d  %s ",
    windows->magnify.x,windows->magnify.y,tuple);
  ...
  y+=height;
  GetColorTuple(&pixel,MagickTrue,text);
  (void) XDrawImageString(display,windows->magnify.pixmap,
    windows->magnify.annotate_context,x,y,text,(int) strlen(text));
  /*
  ...
  XStandardColormap *map_info,XPixelInfo *pixel)
{
            ...
            if (q != (PixelPacket *) NULL)
              {
                for (i=0; i < (long) number_colors; i++)
                {
                  q->red=0;
                  if (map_info->red_max != 0)
                    q->red=ScaleAnyToQuantum(1UL*i/map_info->red_mult,
                      map_info->red_max);
                  q->green=0;
                  if (map_info->green_max != 0)
                    q->green=ScaleAnyToQuantum((1UL*i/map_info->green_mult) %
                      (map_info->green_max+1),map_info->green_max);
                  q->blue=0;
                  if (map_info->blue_max != 0)
                    q->blue=ScaleAnyToQuantum(1UL*i % map_info->green_mult,
                      map_info->blue_max);
                  q->opacity=TransparentOpacity;
                  q++;
                }
  ...
  */
  ...
  if (colormap == (Colormap) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,"UnableToCreateColormap",
      image->filename);
  /*
  ...
  switch (visual_info->klass)
  {
    ...
    case StaticColor:
    {
      ...
      */
      ...
      colors=(XColor *)
        AcquireMagickMemory(visual_info->colormap_size*sizeof(*colors));
      if (colors == (XColor *) NULL)
        ThrowMagickFatalException(ResourceLimitFatalError,
      ...
      if (colormap_type == SharedColormap)
        {
          ...
          */
          diversity=(DiversityPacket *)
            AcquireMagickMemory(image->colors*sizeof(*diversity));
          ...
          */
          qsort((void *) diversity,image->colors,sizeof(*diversity),
            IntensityCompare);
          for (i=0; i < (long) image->colors; i+=MagickMax((long) (image->colors >> 4),2))
          ...
          */
          ...
          /*
            Read X server colormap.
          */
          server_colors=(XColor *) AcquireMagickMemory(
...
#define MinWindowSize  64
  ...
  */
  ...
  if (size_hints == (XSizeHints *) NULL)
    ThrowMagickFatalException(XServerFatalError,"UnableToMakeXWindow",argv[0]);
  ...
  if (window_info->id == (Window) NULL)
    ThrowMagickFatalException(XServerFatalError,"UnableToCreateWindow",
      window_info->name);
  status=XStringListToTextProperty(&window_info->name,1,&window_name);
  if (status == False)
    ThrowMagickFatalException(XServerFatalError,"UnableToCreateTextProperty",
      window_info->name);
...
MagickExport const char *GetLocaleMonitorMessage(const char *tag)
{
  char
  ...
  if (display == (Display *) NULL)
    {
      ...
      message=GetExceptionMessage(errno);
      ThrowXWindowException(XServerError,"UnableToOpenXServer",message);
      message=(char *) RelinquishMagickMemory(message);
      return(MagickFalse);
    }
  ...
  if (pixmap == (Pixmap) NULL)
    {
      ...
      message=GetExceptionMessage(errno);
      ThrowXWindowException(XServerError,"UnabletoCreateBitmap",message);
      message=(char *) RelinquishMagickMemory(message);
      return;
    }
...
#define MinimumCropArea  (unsigned int) 9
  ...
  */
  target_cursor=XMakeCursor(display,root_window,XDefaultColormap(display,
    XDefaultScreen(display)),"white","black");
  ...
  if (status != GrabSuccess)
    {
      ...
      message=GetExceptionMessage(errno);
      ThrowXWindowException(XServerError,"UnableToGrabMouse",message);
      message=(char *) RelinquishMagickMemory(message);
      return((Window) NULL);
    }
  /*
...
MagickExport void XUserPreferences(XResourceInfo *resource_info)
{
  ...
  */
  ...
  (void) FormatMagickString(filename,MaxTextExtent,"%s%src",
    PreferencesDefaults,GetClientName());
  ExpandFilename(filename);
