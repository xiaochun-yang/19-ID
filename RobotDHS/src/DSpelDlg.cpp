// DSpelDlg.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDSpelDlg, CDialog)
CDSpelDlg::CDSpelDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CDSpelDlg::IDD, pParent)
{
}
...
BOOL CDSpelDlg::Initialize( RobotEpson* RobotPtr )
{

	// Create modeless, invisible dialog
	if (!Create(IDD_DIALOG1))
		return FALSE; 
...
void CDSpelDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDSpelDlg, CDialog)
END_MESSAGE_MAP()


// CDSpelDlg message handlers
BEGIN_EVENTSINK_MAP(CDSpelDlg, CDialog)
	ON_EVENT(CDSpelDlg, IDC_SPELCOMCTRL1, 2, EventReceivedSpelcomctrl1, VTS_I4 VTS_BSTR)
END_EVENTSINK_MAP()

void CDSpelDlg::EventReceivedSpelcomctrl1(long EventNumber, LPCTSTR EventMessage)
{
