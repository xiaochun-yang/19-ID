...
void DcsMessageService::ReceiveMessageFromDCSS( )
{
	...
	DcsMessage* pMsg = m_DCSSConnection.CreateAndReceiveDcsMessage( 10 );
	...
	if ( pMsg )
	{
		...
		if (!ProcessEvent( pMsg ))
		{
			...
			if (pMsg->IsAbortAll( ))
			{
				//LOG_FINEST( "this is abort all message" );
			}
			else
			{
				...
				if (pMsg->IsOperation( ))
				{
    				DcsMessage* pReply = DcsMessageManager::GetObject( ).NewOperationCompletedMessage( pMsg, "Not implemented" );
	    			ConsumeDcsMessage( pReply );
				}
			}
...
BOOL DcsMessageService::ConsumeDcsMessage( DcsMessage* pMsg )
{
	if (pMsg)
	{
		if (!m_MsgQueue.Enqueue( pMsg ))
		{
			LOG_WARNING( "DcsMessageService Message Queue FULL" );
...
void DcsMessageService::reset( )
{
	...
	while ((pMsg = m_MsgQueue.Dequeue( )) != NULL)
	{
		LOG_WARNING1( "message discarded in DcsMessageService::reset: %s", pMsg->GetText( ) );
