...
BOOL CNTService::ParseStandardArgs( const char arg[] )
{
    if (_stricmp(arg, "-v") == 0) {
...
BOOL CNTService::Install()
{
    ...
    char szFilePath[_MAX_PATH];
    ::GetModuleFileName(NULL, szFilePath, sizeof(szFilePath));
	char dependency_array[256] = {"EventSystem\0Eventlog\0Tcpip\0\0\0\0"};
    ...
    LOG_INFO1( "service %s installed", m_szServiceName);
...
BOOL CNTService::Uninstall()
{
    ...
    if (hService) {
        if (::DeleteService(hService)) {
            LOG_INFO1( "service %s removed", m_szServiceName);
            bResult = TRUE;
        } else {
			LOG_WARNING1( "service %s cannot be removed", m_szServiceName);
        }
        ::CloseServiceHandle(hService);
    }
...
BOOL CNTService::StartService()
{
	...
	DebugMsg("Returned from StartServiceCtrlDispatcher():%d", b);
    return b;
}
...
void CNTService::ServiceMain(DWORD dwArgc, LPTSTR* lpszArgv)
{
    ...
    if (pService->m_hServiceStatus == NULL) {
        LOG_SEVERE( "service handle cannot be registered" );
        return;
    }
...
BOOL CNTService::Initialize()
{
    ...
    m_Status.dwWaitHint = 0;
    if (!bResult) {
        LOG_SEVERE( "service initalize failed" );
