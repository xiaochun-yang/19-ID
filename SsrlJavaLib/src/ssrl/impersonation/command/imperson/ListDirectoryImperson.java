...
public class ListDirectoryImperson implements ImpersonCommand {
	private final ListDirectory listDir;
	
	public ListDirectoryImperson( ListDirectory cmd ) {
		listDir = cmd;
	}
	...
	public String buildImpersonUrl(String host, int port, AuthSession authSession) {
		...
		url.append("GET /" );
		url.append( "listDirectory?impSessionID=" + authSession.getSessionId()
				+ "&impUser=" + authSession.getUserName() + "&impDirectory=" + dirName2);

		if ( isShowDetails() )
			url.append("&impShowDetails=true");
		else
			url.append("&impShowDetails=false");

		url.append("&impSortType="+getSortType().toString());
		
		if ( getFileType() != null )
			url.append("&impFileType="+getFileType());
		if ( getMaxDepth() != null)
			url.append("&impMaxDepth=" + getMaxDepth() );
		url.append(" HTTP/1.1\r\n");
	...
	static public class ResultReader implements ResultExtractor<List<FileStatus>> {
		...
		static public FileStatus parseLine (String line, String splitString) {
			...
			fileStatus.setFileType(CheckFileStatusImperson.convertFileType(values[1]));
