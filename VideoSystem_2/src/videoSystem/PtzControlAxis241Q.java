...
public class PtzControlAxis241Q extends PtzControlAxis2400 {
	...
	public List getPresetList () throws Exception {
    	List res= new Vector();    
   		...
   		String presetUrl = "http://" + getHostConfig().getHost()+":" + getHostConfig().getPort() +getPresetRequestServlet() + "&camera="+ getCameraChannel()+" ";

   		
		HttpClient axisSocket = new HttpClient(); 
		HttpMethod method = new GetMethod(presetUrl);

		axisSocket.getHttpConnectionManager().getParams().setSoTimeout(getHostConfig().getTimeout());
