...
if ($#argv != 1) then
	echo `date +"%T"` " Wrong number of commandline arguments for con_process.csh" >>& process.out
	echo `date +"%T"` " Usage: con_process.csh <input file>" >>& process.out
	exit 1
endif
...
if (-e control.txt) then
	echo `date +"%T"` " ERROR: Control file (control.txt) already exists in $workDir." >>& process.out
endif

# Run the actual script in the appropriate dir
$WEBICE_SCRIPT_DIR/run_process.csh >>& process.out &
