...
<body class="mainBody">
<%	Vector beamlines = client.getAvailableBeamlines(); // may contain both real and sim beamlines
	String selectedBeamline = client.getBeamline(); // may be either real or sim beamline
	Vector realBeamlines = client.getAllRealBeamlines(); // only real beamlines
	Vector simBeamlines = client.getAllSimBeamlines(); // only sim beamlines
	String site = ServerConfig.getInstallation();
	
	if (beamlines.size() == 0) { %>

User <%= client.getUser() %> does not have a permission to access any beamline at this time.
...
<% 	} // for beamlines %>
<% if (!hasBeamline) { %>
User <%= client.getUser() %> does not have a permission to access a beamline at this time.
<% } %>
</div>
</td>
<td valign="top">
