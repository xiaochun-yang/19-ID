...
<xsl:variable name="helpUrl" select="$param10"/>
<xsl:variable name="mode" select="$param11"/>
...
<form name="silForm" action="handleSilCommand.do" target="_parent" method="GET">
...
<xsl:element name="a">
	...
	<xsl:attribute name="href">sortCrystal.do?column=<xsl:value-of select="@name"/></xsl:attribute>
...
<xsl:if test="@name='SystemWarning'">
<xsl:text> </xsl:text><a id="help" target="WebIceHelp" href="{$helpUrl}/Troubleshooting.html#system-errors">i</a>
</xsl:if></xsl:element>
</xsl:for-each>
</xsl:template>
...
<xsl:if test="(($numRows = 1) and ($selectedRow = @row)) or ($numRows = 'all')">
<xsl:element name="TR">
<xsl:choose>
...
<xsl:param name="crystalNode" select="null"/>
<xsl:choose>
<xsl:when test="$mode = 'multi'">
<TD><input type="checkbox" name="row_{$crystalNode/@row}" value="{$crystalNode/@row}"/><xsl:value-of select="$crystalNode/Port"/></TD>
</xsl:when>
<xsl:otherwise>
<xsl:choose>
 <xsl:when test="$selectedRow = $crystalNode/@row">
 <TD><input type="radio" name="row" value="{$crystalNode/@row}" 
 			checked="true" onclick="row_onclick(this)" />
   <xsl:value-of select="$crystalNode/Port"/></TD>
 </xsl:when>
 <xsl:otherwise>
 <TD><input type="radio" name="row" value="{$crystalNode/@row}" onclick="row_onclick(this)" />
 		<xsl:value-of select="$crystalNode/Port" /></TD>
 </xsl:otherwise>
 </xsl:choose>
 </xsl:otherwise>
...
<xsl:param name="crystalNode" select="null"/>
 <TD><xsl:choose>
    ...
    <xsl:otherwise><xsl:value-of select="translate(string($crystalNode/CrystalID), '{}','()')"/></xsl:otherwise>
  </xsl:choose></TD>
</xsl:template>
...
<xsl:template match="Images">
  <xsl:choose>
    <xsl:when test="($selectedRow = ../@row) and ($showImages = 'show')">
      <table border="1">
    	<tr><th>Group</th>
    	<td>1</td>
    	<td>2</td>
...
<xsl:param name="crystalNode" select="null"/>
 <TD>
  <xsl:if test="(string-length($crystalNode/Resolution) > 0) and ($crystalNode/Resolution != 'N/A')">
   <xsl:value-of select="format-number($crystalNode/Resolution, '0.00')"/>
   <xsl:text> &#197;</xsl:text>
  </xsl:if>
 </TD>
</xsl:template>
...
<xsl:param name="crystalNode" select="null"/>
<TD>
<xsl:if test="string($crystalNode/Move) != ''">
<xsl:variable name="direction">
  <xsl:choose>
    <xsl:when test="contains($crystalNode/Move, 'from')">
       from
    </xsl:when>
    <xsl:otherwise>
       to
...
<xsl:variable name="tmp1"><xsl:value-of select="substring-after($crystalNode/Move, 'sil=')"/></xsl:variable>
<xsl:variable name="otherSilId"><xsl:value-of select="substring-before($tmp1, ',')"/></xsl:variable>
<xsl:variable name="tmp1"><xsl:value-of select="substring-after($crystalNode/Move, 'row=')"/></xsl:variable>
<xsl:variable name="otherRow"><xsl:value-of select="substring-before($tmp1, ',')"/></xsl:variable>
<xsl:variable name="tmp1"><xsl:value-of select="substring-after($crystalNode/Move, 'Port=')"/></xsl:variable>
<xsl:variable name="otherPort"><xsl:value-of select="substring-before($tmp1, ',')"/></xsl:variable>
<xsl:variable name="otherCrystalId"><xsl:value-of select="substring-after($crystalNode/Move, 'CrystalID=')"/></xsl:variable>
<a target="_top" href="setSilDisplayMode.do?silId={$otherSilId}&amp;mode=silOverview&amp;owner={$owner}&amp;row={$otherRow}"><xsl:value-of select="$direction"/>
<xsl:value-of select="$otherSilId"/>/<xsl:value-of select="$otherPort"/>/<xsl:value-of select="$otherCrystalId"/></a>
</xsl:if>
</TD>
</xsl:template>
