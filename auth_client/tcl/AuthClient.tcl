...
# Use httpsmb.tcl instead for now.
...
# public
	...
	constructor { host port {timeout 5000} } {
		...
		set m_servletHost $host
		set m_servletPort $port
		set m_timeout $timeout
	...
	public method validateSession { sessionid } {}
...
# Private 
	...
	private variable m_appName "SMBTest"
	private variable m_servletHost ""
	private variable m_servletPort ""
	private variable m_timeout 5000
...
#	set encodedStr [string replace $encodedStr "=" "%3D"]
	...
	if { [catch {
		...
		if { $m_timeout <= 0 } {
			...
			set httpObjName [httpsmb::geturl $url]
		} else {
			# Block until we get a response or timeout
			set httpObjName [httpsmb::geturl $url -timeout $m_timeout]
		}

		# Convert the return value from httpsmb::geturl to a local array variable.
...
::itcl::body AuthClient::endSession { { sessionId "" } } {
	...
	if { [string length $m_sessionId] < 1 } {
		return error -code "ERROR in AuthClient::updateSession: invalid sessionid"
	}
...
#
# ===================================================
::itcl::body AuthClient::validateSession { sessionid } {
...
::itcl::body AuthClient::isTrue { aString } {
    ...
    if [string compare -nocase $aString "t"] {
    	return 1
    } elseif [string compare -nocase $aString "true"] {
    	return 1
    } elseif [string compare -nocase $aString "y"] {
    	return 1
    } elseif [string compare -nocase $aString "yes"] {
    	return 1
    }
