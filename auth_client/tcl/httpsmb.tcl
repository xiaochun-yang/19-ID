# Modified from http.tcl in tcl 8.4.3
#
# http.tcl --
#
...
#
# RCS: @(#) $Id: httpsmb.tcl,v 1.1 2003/07/21 17:25:38 penjitk Exp $

# Rough version history:
# 1.0	Old http_get interface
# 2.0	httpsmb:: namespace and httpsmb::geturl
...
# 2.4	Added -binary option to httpsmb::geturl and charset element
#	to the state array.
...
namespace eval httpsmb {
    variable http
    array set http {
	-accept */*
	-proxyhost {}
	-proxyport {}
	-proxyfilter httpsmb::ProxyRequired
    }
...
proc httpsmb::register {proto port command} {
    variable urlTypes
...
proc httpsmb::Finish { token {errormsg ""} {skipCB 0}} {
    variable $token
    upvar 0 $token state
...
proc httpsmb::reset { token {why reset} } {
    variable $token
    upvar 0 $token state
...
proc httpsmb::geturl { url args } {
	...
	set state(after) [after $state(-timeout) \
		[list httpsmb::reset $token timeout]]
	set async -async
    } else {
	set async ""
    }
    ...
    if {$state(-timeout) > 0} {
	fileevent $s writable [list httpsmb::Connect $token]
	httpsmb::wait $token
	...
	if {$isQuery || $isQueryChannel} {
	    ...
	    fileevent $s writable [list httpsmb::Write $token]
	} else {
	    puts $s ""
	    flush $s
	    fileevent $s readable [list httpsmb::Event $token]
	}
...
proc httpsmb::data {token} {
    variable $token
    upvar 0 $token state
    return $state(body)
}
proc httpsmb::status {token} {
    variable $token
    upvar 0 $token state
    return $state(status)
}
proc httpsmb::code {token} {
    variable $token
    upvar 0 $token state
    return $state(http)
}
proc httpsmb::ncode {token} {
    variable $token
    upvar 0 $token state
...
proc httpsmb::size {token} {
    variable $token
    upvar 0 $token state
    return $state(currentsize)
}

proc httpsmb::error {token} {
    variable $token
    upvar 0 $token state
    if {[info exists state(error)]} {
	return $state(error)
...
#
# Arguments
#	token	The token returned from httpsmb::geturl
#
# Side Effects
#	unsets the state array

proc httpsmb::cleanup {token} {
    variable $token
    upvar 0 $token state
...
proc httpsmb::CopyStart {s token} {
    variable $token
    upvar 0 $token state
    if {[catch {
	fcopy $s $state(-channel) -size $state(-blocksize) -command \
	    [list httpsmb::CopyDone $token]
    } err]} {
	Finish $token $err
    }
}
...
proc httpsmb::CopyDone {token count {error {}}} {
    variable $token
    upvar 0 $token state
    set s $state(sock)
...
#
#	See documentaion for details.
#	Call httpsmb::formatQuery with an even number of arguments, where 
...
proc httpsmb::ProxyRequired {host} {
    variable http
