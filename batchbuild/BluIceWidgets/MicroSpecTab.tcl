...
class SimpleImageView {
    ...
    itk_option define -snapshot   snapshot   Snapshot   ""        {
        set m_retryCounter 0
        update
    }
    ...
    private variable m_retryCounter 0

    private common MAX_RETRY 10
...
class MicroSpecLightWidget {
    ...
    private variable m_enable2Light 0

    private variable m_objSpectroWrap ""
    private variable m_objSwitchString ""
    private variable m_ctsSwitchString ""
    private variable m_objLightString ""
    private variable m_ctsLightString ""
    private variable m_objLightString2 ""
    private variable m_ctsLightString2 ""
    private variable m_objBulbString ""
    private variable m_ctsBulbString ""
    
    private variable m_onImage  ""
    private variable m_offImage ""
    private variable m_uvOnImage  ""
    private variable m_uvOffImage ""

    public method onlyShowCheckbox { } {
        pack forget $itk_component(on) $itk_component(off)
        grid forget $itk_component(visStatus) $itk_component(uvStatus)
    }

    public method selectLight { } {
        set newBulb [$itk_component(light_choice) get]
        $m_objSpectroWrap startOperation set_bulb $newBulb
    }

    public method getOK4On { } {
        set contents [getLightStringContents]

        if {[catch {dict get $contents HALOGEN} visStatus]} {
            set visStatus off
        }
        if {[catch {dict get $contents DEUTERIUM} uvStatus]} {
            set uvStatus off
        }
        set bulb [lindex $m_ctsBulbString 5]
        switch -exact -- $bulb {
            UV {
                if {$uvStatus != "on"} {
                    return 1
    ...
    public method getOK2OpenShutter { } {
        set contents [getLightStringContents]

        if {[catch {dict get $contents SHUTTER} shutterState]} {
            return 1
        }
    ...
    public method getOK2CloseShutter { } {
        set contents [getLightStringContents]
    ...
    public method handleSwitchStringUpdate { - ready_ - contents_ -} {
        if {!$ready_} {
            return
        }
        set m_ctsSwitchString $contents_

        ### low level already delt with m_enable2Light
        updateLightDisplay
    }

    public method handleLightStringUpdate { - ready_ - contents_ -} {
        if {!$ready_} {
            return
        }

        set m_ctsLightString $contents_

        updateLightDisplay
    }
    public method handleLightString2Update { - ready_ - contents_ -} {
        if {!$ready_} {
            return
        }

        set m_ctsLightString2 $contents_

        ### low level already delt with m_enable2Light
        updateLightDisplay
    }
    private method getLightStringContents { } {
        if {!$m_enable2Light} {
            return $m_ctsLightString
        }

        set bulb $m_ctsSwitchString
        #set bulb [lindex $m_ctsBulbString 5]
        ...
        switch -exact -- $bulb {
            ...
            default {
                ### new box
                return $m_ctsLightString2
            }
        }
    }
    private method updateLightDisplay { } {
        updateRegisteredComponents OKTurnOn
        updateRegisteredComponents OKTurnOff
        updateRegisteredComponents OKToOpen
        updateRegisteredComponents OKToClose
        ...
        if {[catch {dict get $contents SHUTTER} shutterStatus]} {
            set shutterStatus unknown
        }

        if {$visStatus == "on"} {
            set vis_image $m_onImage
        } else {
            set vis_image $m_offImage
        }
        if {$uvStatus == "on"} {
            set uv_image $m_uvOnImage
        } else {
            set uv_image $m_uvOffImage
        }
        
        $itk_component(visStatus) configure -image $vis_image
        $itk_component(uvStatus)  configure -image $uv_image

        $itk_component(shutterF) configure \
        -labeltext "Shutter Status: $shutterStatus"
    }
    public method handleBulbStringUpdate { - ready_ - contents_ -} {
        if {!$ready_} {
            return
        }
    ...
    public method openShutter { } {
        $m_objLightString  sendContentsToServer "SHUTTER open"
        if {$m_enable2Light} {
            $m_objLightString2 sendContentsToServer "SHUTTER open"
        }
    }
    public method closeShutter { } {
        $m_objLightString  sendContentsToServer "SHUTTER closed"
        if {$m_enable2Light} {
            $m_objLightString2 sendContentsToServer "SHUTTER closed"
        }
    }
    public method turnLight { on_off } {
        if {!$on_off} {
            $m_objLightString  sendContentsToServer "HALOGEN off DEUTERIUM off"
            if {$m_enable2Light} {
                $m_objLightString2 sendContentsToServer \
                "HALOGEN off DEUTERIUM off"
            }
            return
        }
        set bulb [lindex $m_ctsBulbString 5]
        switch -exact -- $bulb {
            UV {
                set contents "DEUTERIUM on HALOGEN off"
            }
            UV_VIS {
                set contents "DEUTERIUM on HALOGEN on"
            }
            NONE {
                set contents "DEUTERIUM off HALOGEN off"
            }
            VIS -
            default {
                set contents "DEUTERIUM off HALOGEN on"
            }
        }
        ###TODO: change to operation call
        puts "sending $contents"
        $m_objLightString  sendContentsToServer $contents
        if {$m_enable2Light} {
            $m_objLightString2 sendContentsToServer $contents
        }
    }

    ## there are cases that both on and off buttons need to be enabled.
    constructor { includeShutter args } {
        ::DCS::Component::constructor {
            OKTurnOff { getOK4Off }
            OKTurnOn  { getOK4On }
            OKToOpen  { getOK2OpenShutter }
            OKToClose { getOK2CloseShutter }
        }
    } {
        global BLC_IMAGES
        ...
        set m_uvOnImage  [image create photo \
        -file "$BLC_IMAGES/uvbulb_on.gif" \
        -palette "8/8/8"]

        set m_uvOffImage [image create photo \
        -file "$BLC_IMAGES/uvbulb_off.gif" \
        -palette "8/8/8"]

        set m_enable2Light [::config getInt microspec.2light 0]

        set deviceFactory [DCS::DeviceFactory::getObject]
        set m_objSpectroWrap \
        [$deviceFactory createOperation spectrometerWrap]

        set m_objSwitchString \
        [$deviceFactory createString opticalFiberSwitch]

        set m_objLightString \
        [$deviceFactory createString microspecLightControl]
        set m_objLightString2 \
        [$deviceFactory createString microspecLightControl2]

        set m_objBulbString \
        [$deviceFactory createString spectroWrap_config]

        $m_objBulbString createAttributeFromField bulb 5

        itk_component add light_choice {
            DCSRadiobox $itk_interior.uv \
            -stateList [list VIS UV_VIS] \
            -buttonLabels [list "Visible Light Only" "Visible + UV"] \
            -shadowReference 1 \
            -selectcolor red\
        ...
        itk_component add visStatus {
            label $chcSite.vis_status \
            -image $m_offImage \
        } {
        }
        itk_component add uvStatus {
            label $chcSite.uv_status \
            -image $m_uvOffImage \
        } {
        }

        set buttonSite [frame $chcSite.buttonF]

        itk_component add on {
            DCS::Button $buttonSite.on \
            -text "On" \
            -width 7 \
        ...
        pack $itk_component(off) -side right

        grid $itk_component(visStatus) -row 0 -column 1
        grid $itk_component(uvStatus)  -row 1 -column 1
        grid $buttonSite -columnspan 2 -row 2 -column 0 -sticky news

        $itk_component(on) addInput \
        "$this OKTurnOn 1 {light already on}"
        $itk_component(off) addInput \
        "$this OKTurnOff 1 {light already off}"

        itk_component add shutterF {
            ::iwidgets::labeledframe $itk_interior.shutterF \
            -labeltext "Shutter Status: unknown"
        } {
        }
        set shutterSite [$itk_component(shutterF) childsite]

        itk_component add open {
            DCS::Button $shutterSite.open \
            -text "Open" \
            -width 8 \
            -command "$this openShutter"
        } {
        }

        itk_component add close {
            DCS::Button $shutterSite.close \
            -text "Close" \
            -width 8 \
            -command "$this closeShutter"
        } {
        }
        pack $itk_component(open)  -side left
        pack $itk_component(close) -side right

        $itk_component(open) addInput \
        "$this OKToOpen 1 {shutter already open}"
        $itk_component(close) addInput \
        "$this OKToClose 1 {shutter already closed}"
        
        pack $itk_component(light_choice) -fill x -side top
        if {$includeShutter} {
            pack $itk_component(shutterF) -fill x -side top
        }

        eval itk_initialize $args
        announceExist

        $m_objSwitchString register $this contents handleSwitchStringUpdate
        $m_objLightString register $this contents handleLightStringUpdate
        $m_objLightString2 register $this contents handleLightString2Update
        $m_objBulbString  register $this contents handleBulbStringUpdate
    }
    destructor {
        $m_objSwitchString unregister $this contents handleSwitchStringUpdate
        $m_objLightString unregister $this contents handleLightStringUpdate
        $m_objLightString2 unregister $this contents handleLightString2Update
        $m_objBulbString  unregister $this contents handleBulbStringUpdate
    }
}
...
class MicroSpecModeWidget {
    ...
    constructor { args } {
        ...
        set upPane   [$itk_component(pw_h) childsite 0]
        set downSite [$itk_component(pw_h) childsite 1]
        $itk_component(pw_h) fraction 70 30

        itk_component add sf {
            iwidgets::scrolledframe $upPane.sf \
            -vscrollmode static \
            -hscrollmode none \
        } {
        }
        set upSite [$itk_component(sf) childsite]
        ...
        itk_component add light_control {
            MicroSpecLightWidget $upSite.lightControl 0 \
            -labeltext "Light Selection" \
        } {
        }
        $itk_component(light_control) onlyShowCheckbox
        ...
        grid rowconfigure $upSite 5 -weight 10
        ...
        grid $itk_component(light_control)         -row 4 -column 0 -sticky nws
        grid $controlSite                          -row 5 -column 0 -sticky new

        pack $itk_component(snapshot) -side bottom -expand 1 -fill both

        pack $itk_component(sf) -side top -expand 1 -fill both
        ...
        "$m_objWrapStatus refValid 1 {need take reference first on Hardware Tab}"
        $itk_component(startButton) addInput \
        "$m_objWrapStatus darkValid 1 {need take dark first on Hardware Tab}"
    }
}
body MicroSpecModeWidget::handleModeSelect { } {
    ...
    switch -exact -- $currentMode {
        ...
        time_scan {
            set text "Measure Changes in Absorbance as a Function of Time"
        ...
        dose_scan {
            set text "Measure Changes in Absorbance as a Function of X-Ray Exposure"
...
body MicroSpecModeWidget::updateParameters { } {
    ...
    if {$fileRoot != ""} {
        ...
        set subDir  [$obj getFieldByIndex 4]
...
class MicroSpecPlotView {
    ...
    itk_option define -loadCommand loadCommand LoadCommand ""
    ...
    public method setResult { mtrName phi title rList aList tList fName {t0 0}}
    ...
    public method loadFromGui { } {
        set cmd $itk_option(-loadCommand)
        if {$cmd == ""} {
            log_error no load command defined, skip.
            return
        }
        set types { { MicroSpec .yaml} {{All Files} *} }
...
body MicroSpecPlotView::constructor { args } {
    ...
    $itk_component(graph) addToMainMenu loadScan "load Scan" \
    "$this loadFromGui"
...
body MicroSpecPlotView::updateLog { } {
    ...
    foreach w $wList r $rList a $aList t $tList {
        set line \
        "[format $fmtW $w] [format $fmtR $r] [format $fmtA $a] [format $fmtT $t]"
...
body MicroSpecPlotView::setWavelengthList { wList } {
    ...
    if {$m_numElmOverlay != $numElement} {
        ...
        puts "zoomOutAll because diff xVector"
    }
}
...
body MicroSpecPlotView::setResult { motorName p title \
rList aList tList fileName {t0 0}} {
    set m_cleared 0
...
class MicroSpecHardwareView {
    ...
    constructor { args} {
        ...
        itk_component add lightControl {
            MicroSpecLightWidget $itk_component(canvas).light_control 0 \
        ...
        place $itk_component(lightControl)  -x 95 -y 475
...
class MicroSpecDataControl {
    ...
    public method loadResultByName { path } {
    ...
    constructor { mode plotWidget videoWidget args } {
        ...
        $m_plot configure \
        -loadCommand "$this loadResultByName"
...
body MicroSpecDataControl::parseFileName { path } {
    set fDir  [file dir $path]
    set fExt  [file ext $path]
    set fName [file tail $path]
    set fName [file root $fName]

    ### system file
    if {[string first dark $fName] == 0 \
    ||  [string first reference $fName] == 0} {
        return [list $path ""]
    }

    set ll    [string length $fName]
    set index [string last _ $fName]
    ### header file
    if {$index < 0 || $ll == $index + 7} {
        return [list $path ""]
    }

    set header [string range $fName 0 [expr $index - 1]]
    set index  [string range $fName [expr $index + 1] end]
    set headerPath [file join $fDir $header$fExt]
