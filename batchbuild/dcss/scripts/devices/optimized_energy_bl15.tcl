...
proc smartOptimize { new_energy } {
   ...
   puts $handle "[time_stamp] Attenuation: $attenuation"
	...
	while { 1 } { 
		...
		if { [catch {set tableNew [optimizeTest_start table_vert $table_vert i2 20 0.05 0.1]} errorResult] } {
...
proc beamGood {} {
	...
	if { [catch {
		set op [start_waitable_operation getPV beam_steered]
		set result [lindex [wait_for_operation $op] 2]
	} errorResult] } {
	...
	read_ion_chambers 0.1 i0
	
	# wait for ion chamber read to complete
	wait_for_devices i0
	
	#
	set counts [get_ion_chamber_counts i0]

	if { $counts < 100 } {
		log_warning "Is there beam in the hutch?  Please check stoppers. ."
		return 0
	}
...
proc wait_for_good_beam {} {
	...
	#this is needed so that we can abort the procedure
	set lastAbortCount $gDevice(abortCount)
	
	if { [catch {
		#check if spear for steer status
		set op [start_waitable_operation waitPV beam_steered 1 int 0 1000]
		set result [wait_for_operation $op]
	} errorResult] } {
		log_error "Error talking to spear: $errorResult"
	}

	set counts 0
	#check i0 to see if stoppers are closed 
 	while { $counts < 100 } {
		...
		#this is needed so that we can abort the procedure
		if { $::gDevice(abortCount) != $lastAbortCount } {
			error "getStableIonCounts was aborted"
		}
