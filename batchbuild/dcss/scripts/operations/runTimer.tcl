...
variable ::datatime::wait_for_good_beam
...
if { $dType == "Q315CCD" } {
  ...
  set image_overhead_time 0
}

# wait_for_good_beam adds 1 sec at 12-2 before the first image in every wedge

set wait_for_good_beam 0
if { $beamlineID == "BL12-2" } {
  set wait_for_good_beam 1
}
...
# ... then time for wait_for_good_beam (before shutter opens)
...
lappend messages "wait_for_good_beam. ."
...
# add time for Pilatus, before save_energy_move_times
       if { [lindex $detectorType 0] == "PILATUS6" } {
          set energyCfg [::config getStr energy.config]
          if {$energyCfg != ""} {
...
# add 10 sec for BL11-1 mono
    if { $beamlineID == "BL11-1" } {
       set tempMoveEnergy [expr $tempMoveEnergy + 10 ]
    }
# add (6 + 1) sec for BL12-2 wait_for_good_beam after energy move, attenuation move
    if { $beamlineID == "BL12-2" } {
       set tempMoveEnergy [expr $tempMoveEnergy + 7 ]
    }
...
# add 6 sec for BL12-2 wait_for_good_beam
      if { $beamlineID == "BL12-2" } {
         set tempMoveEnergy [expr $tempMoveEnergy + 6 ]
      }
...
# wait_for_good_beam delay (1 sec) before each shutter opening
...
# first check if beamline is 12-2
if { $beamlineID != "BL12-2" } {
     return 0
}
...
if { $next_frame < $fragmentStartIndex } {
  ...
  set openings [expr [lindex $fullWedgeFramesPerLevel 2] / [lindex $fullWedgeFramesPerLevel 0] - \
                     [lindex $completedLevelsInWedge 1] * ($inverse + 1) - \
                     [lindex $completedLevelsInWedge 2] ]
  if { [lindex $fragmentWedgeFramesPerLevel 0] != 0 } {
       set openings [expr $openings + [lindex $fragmentWedgeFramesPerLevel 2] / [lindex $fragmentWedgeFramesPerLevel 0]]
  } 
} else {
# case in last fragment
  set openings [expr [lindex $fragmentWedgeFramesPerLevel 2] / [lindex $fragmentWedgeFramesPerLevel 0] - \
                     [lindex $completedLevelsInWedge 1] * ($inverse + 1) - \
                     [lindex $completedLevelsInWedge 2] ]

}

set wait_time [expr $openings * 1]
