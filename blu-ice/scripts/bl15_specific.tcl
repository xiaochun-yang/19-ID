...
proc initialize_components {} {
	...
	if { $gBeamline(beamlineId) != "bl15" } {
		...
		create_pseudo_motor mirror_chin_gap 	mcg 	mirror 	" mm $gFont(micron) "
	}
	...
	if { $gBeamline(beamlineId) != "bl15" } {
		...
		create_pseudo_motor	energy						e		default		"eV keV $gFont(angstrom)"
	}
	...
	if { $gBeamline(doubleMono) } {
		...
		if { $gBeamline(beamlineId) == "bl15" } {
			#define toroid
			create_mdw_document toroid  "Toroid"  630 310  construct_15_toroid_window
			add_component_menu_entry Toroid toroid

			create_real_motor 	mono_theta			mot	default		{ deg mrad }
			create_real_motor 	mono_encoder		mot	default		{ deg mrad }
			create_real_motor 	mono_pitch			mop	default		{ deg mrad }
			#create_pseudo_motor  mono_theta_corr   mtc   default 	{ deg mrad }

			create_pseudo_motor toroid_vert			tov 	toroid	" mm $gFont(micron) "
			create_pseudo_motor toroid_horz			toh 	toroid	" mm $gFont(micron) "

			create_real_motor toroid_vert_1			toh 	toroid	" mm $gFont(micron) "
			create_real_motor toroid_vert_2			toh 	toroid	" mm $gFont(micron) "
			create_real_motor toroid_horz_1			toh 	toroid	" mm $gFont(micron) "
			create_real_motor toroid_horz_2			toh 	toroid	" mm $gFont(micron) "

			create_real_motor 	mono_slit_vert		mosv	default		" mm $gFont(micron) "
			create_real_motor mono_slit_horz			toh 	default	" mm $gFont(micron) "
			create_real_motor mono_slit_gap_vert			toh 	default	" mm $gFont(micron) "
			create_real_motor mono_slit_gap_horz			toh 	default	" mm $gFont(micron) "
			create_real_motor mono_pitch_coarse		toh 	default	" mm $gFont(micron) "

			create_pseudo_motor toroid_pitch		top	toroid	{ deg mrad }
			create_pseudo_motor toroid_yaw			toy	toroid	{ deg mrad }

			create_pseudo_motor	toroid_v1_z		tv1z	default	" mm $gFont(micron) "
			create_pseudo_motor	toroid_v2_z		tv2z	default  " mm $gFont(micron) "
			create_pseudo_motor	toroid_h1_z		th1z	default	" mm $gFont(micron) "
			create_pseudo_motor	toroid_h2_z		th2z	default  " mm $gFont(micron) "
			create_pseudo_motor	toroid_pivot_z	tpz	default	" mm $gFont(micron) "

			create_real_motor toroid_bend			tob	default	" mm "


			create_pseudo_motor  table_vert_1_offset tso  default " mm $gFont(micron) "
			create_pseudo_motor  table_vert_2_offset tso  default " mm $gFont(micron) "
			create_pseudo_motor  table_horz_1_offset tso  default " mm $gFont(micron) "
			create_pseudo_motor  table_horz_2_offset tso  default " mm $gFont(micron) "

			create_pseudo_motor  mono_piezo_offset tso  default " V "
			create_pseudo_motor  mono_piezo tso  default " V "
			create_pseudo_motor  linear_dac tso  default " V "
		...
		create_pseudo_motor  table_vert_2_offset tso  default " mm $gFont(micron) "
		create_pseudo_motor  table_horz_2_offset tso  default " mm $gFont(micron) "
		create_pseudo_motor  snapPhi sPhi  default " deg "
...
proc construct_beamline_window { parent } {
	...
	# create views for each motor
	motorView $canvas beamline_vert_1 120 110 s 140 45 13
	motorView $canvas beamline_vert_2 560 110 s 140 45 13
	motorArrow $canvas beamline_vert_1 120 113 {} 120 163
	motorArrow $canvas beamline_vert_2 570 113 {} 570 163
	}
...
proc joyUp {args} {
	...
	if { $joyspeed(xy_last) != 0 } {
		if { $joyspeed(xyUp) == 0 } {	
			#stop vector move
			vector_stop_move sample_x sample_y
	...
	if { $joyspeed(xy_last) != 0 } {
		if { $joyspeed(xyUp) != 0 } {	
			#change vector speed
			vector_change_speed sample_x sample_y $joyspeed(xyUp)
...
proc joyDown {args} {
	...
	if { $joyspeed(xy_last) != 0 } {
		if { $joyspeed(xyDown) != 0 } {	
			#change vector speed
			vector_change_speed sample_x sample_y $joyspeed(xyDown)
			#			log_note "change vector speed"
		}
	}
...
proc joyLeft {args} {
	...
	if { $joyspeed(z_last) != 0 } {
		if { $joyspeed(zLeft) == 0 } {	
			#stop vector move
			vector_stop_move sample_z NULL
			#log_note "stop vector move"
			update
			after 500 {}
			}
	...
	if { $joyspeed(z_last) != 0 } {
		if { $joyspeed(zLeft) != 0 } {	
			#change vector speed
			vector_change_speed sample_z NULL $joyspeed(zLeft)
#			log_note "change vector speed"
			}
		}
...
proc joyRight {args} {
	...
	if { $joyspeed(z_last) != 0 } {
		if { $joyspeed(zRight) != 0 } {	
			#change vector speed
			vector_change_speed sample_z NULL $joyspeed(zRight)
#			log_note "change vector speed"
			}
		}
...
#	}
	...
	move_vector_no_parse $motor1 $motor2 $comp_x $comp_y $speed
}
...
proc zminushandler {speed} {
	...
	move_vector_no_parse sample_z NULL $gDevice(sample_z,scaledLowerLimit) 0 $speed
}
...
proc zplushandler {speed} {
	...
	move_vector_no_parse sample_z NULL $gDevice(sample_z,scaledUpperLimit) 0 $speed
}
...
proc construct_15_toroid_window { parent } {
	...
	# create views for each motor
	motorView $canvas toroid_vert_1 190  90 s
	motorView $canvas toroid_vert 	 330  90 s 
	motorView $canvas toroid_vert_2 470  90 s
	motorView $canvas toroid_horz_1 135 197 n
	motorView $canvas toroid_horz 	 275 197 n
	motorView $canvas toroid_horz_2 415 197 n
	motorView $canvas toroid_yaw 	 143 137 e 105 45 7
	motorView $canvas toroid_pitch  507 137 w 105 45 7
	
	# create arrow for each motor
	motorArrow $canvas toroid_vert_1 210  90 {} 210 140 222 94 222 140
	motorArrow $canvas toroid_vert   330  90 {} 330 140	342 94 342 140
	motorArrow $canvas toroid_vert_2 450  90 {} 450 140 462 95 462 140
	motorArrow $canvas toroid_horz_1  190 170 {} 158 197 198 172 176 196
	motorArrow $canvas toroid_horz   305 170 {} 272 197 313 172 291 196
	motorArrow $canvas toroid_horz_2 430 170 {} 403 197 442 177 422 196
	motorArrow $canvas toroid_yaw    165 165 {125 137} 200 135 154 172 186 125
	motorArrow $canvas toroid_pitch  470  110 {500 120 500 160} 470 170 486 104 486 176
