...
int count; /* Number of times called */
{
	...
	if (marstatusfp != NULL) {
		...
		if (marstatusfp == NULL) {
			perror("Error opening marstatusfile:");
			sprintf(buf,"adx_ccd_control: Update status: Cannot open status file: %s\n", marstatusfile);
			emess(buf);
			return;
		}
	}
...
{
	char	ai_buf[2048], tbuf[20];
	int	probe_res, ru_res, i, j;
	int	ai_which;
	float	ai_energy;
	float	ai_wavelength;
	
	probe_res = probe_port_raw_with_timeout(ai_fd, 100);
	if(probe_res == 0)
		return;		/* no data */
	if(probe_res == -1)	/* error or disconnect */
	{
		close(ai_fd);
		ai_fd = -1;
		return;
	...
	ru_res = read_until(ai_fd, ai_buf, 2048, "end_of_ai\n");
	if(ru_res == -1 || ru_res == 0)
	{
		close(ai_fd);
		ai_fd = -1;
		return;
	}
	for(i = 0, j = 0; i < ru_res; i++)
	{
		for(j = i; j < ru_res; j++)
			if(ai_buf[j] == '\n')
			{
		...
		if(NULL != (char *) strstr(&ai_buf[i], "end_of_ai"))
		{
			write(ai_fd, END_OF_AI, strlen(END_OF_AI));
			break;
		}
		/*
		 * process command
		 */
		if(NULL != (char *) strstr(&ai_buf[i], "ai_set_energies"))
		{
		...
		XtManageChild(madDialog);
		fprintf(stdout,"input string; %s\n", &ai_buf[i]);
		sscanf(&ai_buf[i], "%d %f", &ai_which, &ai_energy);
		ai_wavelength = 12398.5 / ai_energy;
		fprintf(stdout,"ai_set_energies: %d %f %f\n", ai_which, ai_energy, ai_wavelength);

		switch(ai_which)
		{
			case 1:
				XmToggleButtonSetState(enable_wavelength1_toggleButton,True,False);
				sprintf(tbuf, "%.2f", ai_energy);
				XmTextFieldSetString(energy1_textField, tbuf);
				sprintf(tbuf, "%.6f", ai_wavelength);
				XmTextFieldSetString(wavelength1_textField, tbuf);
			...
			case 2:
				...
				sprintf(tbuf, "%.2f", ai_energy);
				XmTextFieldSetString(energy2_textField, tbuf);
				sprintf(tbuf, "%.6f", ai_wavelength);
				XmTextFieldSetString(wavelength2_textField, tbuf);
			...
			case 3:
				...
				sprintf(tbuf, "%.2f", ai_energy);
				XmTextFieldSetString(energy3_textField, tbuf);
				sprintf(tbuf, "%.6f", ai_wavelength);
				XmTextFieldSetString(wavelength3_textField, tbuf);
			...
			case 4:
				...
				sprintf(tbuf, "%.2f", ai_energy);
				XmTextFieldSetString(energy4_textField, tbuf);
				sprintf(tbuf, "%.6f", ai_wavelength);
				XmTextFieldSetString(wavelength4_textField, tbuf);
			...
			case 5:
				...
				sprintf(tbuf, "%.2f", ai_energy);
				XmTextFieldSetString(energy5_textField, tbuf);
				sprintf(tbuf, "%.6f", ai_wavelength);
				XmTextFieldSetString(wavelength5_textField, tbuf);
...
#define	BEAMLINE_EXEC_ENVIRON	"BEAMLINE_LINK_FROM_ADX"

void
beamline_activateCallback(w, client_data, call_data)
 Widget w;
 XtPointer client_data;
 XtPointer call_data;
{
    ...
    if(NULL == (exec_name = (char *) getenv(BEAMLINE_EXEC_ENVIRON)))
    {
	fprintf(stderr,"beamline_activateCallback: NO ENVIRONMENT NAME %s\n", BEAMLINE_EXEC_ENVIRON);
	fprintf(stderr,"    ---> THIS IS USED TO EXECUTE THE LINK TO THE PAL BEAMLINE CONTROL PROGRAMS\n");
	fprintf(stderr,"    ---> No action taken on button press\n");
	return;
    }
