...
char	*retstat_ok = "OK";
char	*retstat_retry = "RETRY";
char	*retstat_error = "ERROR";
		...
		if(-1 != (eobuf = string_found(blbuf,blindex,looking_for)))
		  {
			...
			fprintf(stdout, "ccd_bl_gen: REPLIED: ");
			strcpy(retbuf, bl_returned_string);

			switch(returned_status)
			{
			    case 0:
				retstat_string = retstat_ok;
				break;
			    case 1:
				retstat_string = retstat_retry;
				break;
			    case 2:
				retstat_string = retstat_error;
				break;
			}
			strcat(retbuf, retstat_string);
			strcat(retbuf, looking_for);
			fprintf(stdout, "%s\n", retstat_string);
