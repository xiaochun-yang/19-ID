	...
	while(1)
	  {
	    ...
	    if(j == 0)
	      {
		fprintf(stderr,"ccd_bl_gen: %s is an unrecognized command\n",
			tok1);
	    ...
	    if(i == MDC_COM_ABORT)
	      {
		fprintf(stderr,"ccd_bl_gen: ABORT SEEN\n");
		fprintf(stderr,"ccd_bl_gen: NO operation in progress.  Ignore.\n");
	      }
		...
		case MDC_COM_OMEGA_CONT:
		case MDC_COM_PHI_CONT:
		case MDC_COM_OMEGA_STOP:
		case MDC_COM_PHI_STOP:
		...
		case MDC_COL_MODE:
		  ...
		  if(0 == strcmp("continuous_dc",tok2))
		    {
			mdc_queue[next].cmd_col_mode = 6;
			break;
		    }
		...
		case MDC_COL_REMARK:
		  ...
		  if(NULL == (mdc_queue[next].cmd_col_remarkv[mdc_queue[next].cmd_col_remarkc] =
		    ...
		    {
			    fprintf(stderr,"ccd_bl_gen: error calloc memory for remark command\n");
			    cleanexit(BAD_STATUS);
		    }
 ...
 *
 *	When it is finished, its return will signal to ccd_bl_gen that
 *	data collection may resume.
 *
 *	"buf" is a null terminated string containing information passed
 *	to ccd_bl_gen via the control GUI.  It is of the form:
 *
...
int	local_beamline_control(buf)
char	*buf;
  {
	int	i,j,k,l,m,n;
	int	cmdno;
	...
	for(n = 0; blcmdtypes[n] != NULL; n++)
	  if(0 == strcmp(blcmdtypes[n],ccdparse_subv[0][0]))
	    {
		cmdno = n;
		break;
	    }
	...
	switch(cmdno)
	  {
		...
		inlen = strlen(inbuf);
		mdc_process_input(0);
		returned_status = ccd_bl_generic_cmd(0);
		break;

	    case BL_BEFORE:
		fprintf(stdout,"ccd_bl_gen: BEFORE seen\n");
		returned_status = 0;
		break;

	    case BL_DURING:
		fprintf(stdout,"ccd_bl_gen: DURING seen\n");
		returned_status = 0;
		break;

	    case BL_AFTER:
		fprintf(stdout,"ccd_bl_gen: AFTER seen\n");
		returned_status = 0;
		break;
	  }
