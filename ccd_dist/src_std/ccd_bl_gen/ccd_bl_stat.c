...
int	fd;
  {
	...
	for(i = 0; i < 3; i++)
		stat_ion[i] = (i / 10.) + .03 * ((double) random()) / ((double) RAND_MAX);
	...
	if(stat_onetime[MDC_COM_XL_HS_MOVE])
	{
	...
	{
		sprintf(tbuf,"xl_hs %d", -1);
		pad_to_n(tbuf,80,obuf);
		if(strlen(obuf) != st_write(fd,obuf,strlen(obuf))) return;
	}

	if(stat_onetime[MDC_COM_XL_VS_MOVE])
	{
