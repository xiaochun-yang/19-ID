...
merge_headers(char *det_header, char *extra_header, char *merged_header)
{
	int	det_hlen;
	int	i, j, n;
	char	field[256], value[256], value2[256];
...
strip_redundant_entries(char *hp)
{
	int	hdlen;
	...
	gethdl(&hdlen, hp);
	if(NULL == (new_hd = (char *) malloc(hdlen)))
	{
	...
	for(i = 0; ; i++)
	{
		...
		puthd(field, value, new_hd);
	}

	sprintf(value, "%d", last_size1);
	puthd("SIZE1", value, new_hd);
	sprintf(value, "%d", last_size2);
	puthd("SIZE2", value, new_hd);
	sprintf(value, "%d", last_saturated);
	puthd("CCD_IMAGE_SATURATION", value, new_hd);
...
int	save_image(int which_kind,int  buffer_kind, int compress)
  {
	...
	char		*cptr,*hdptr;
	unsigned short	*uptr;
	int		sizehd,sizedata,i, j;
	char		*tstr;
	float		*flptr;
	struct passwd	*pwp;
	char		header_logname[256];
 ...
 */
	...
	header_logname[0] = '\0';
	gethd("LOGNAME", header_logname, cptr);
	if(header_logname[0] != '\0')
	{
		if(0 != (pwp = getpwnam(header_logname)))
		{
 ...
 */
	...
	header_logname[0] = '\0';
	gethd("LOGNAME", header_logname, hdptr);
	if(header_logname[0] != '\0')
	{
		if(0 != (pwp = getpwnam(header_logname)))
		{
