...
public:
	...
	DcsMessage* CreateAndReceiveDcsMessage( unsigned int timeout = 10 );

	bool SendDcsMessage( const char* pText, const void *pBinary, size_t binarySize, unsigned int timeout = 10 );
	bool SendAndDeleteDcsMessage( DcsMessage* pMsg, unsigned int timeout = 10 );	//this will also delete the message
...
private:
	...
	bool ReceiveDcsConnectionMessage( unsigned int timeout = 10 );
	bool SendDcsConnectionMessage( unsigned int timeout = 10 );
