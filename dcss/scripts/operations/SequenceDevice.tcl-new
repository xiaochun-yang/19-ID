...
#yangx    private common ADD_BEAM_INFO_TO_JPEG 1
...
private method doGrabberWarmCycle { cycle } {}
...
#yif {!$m_dismountRequested} {
#y    if {[catch {
#y        ::nScripts::correctPreCheckMotors 1
#y    } errMsg]} {
#y        log_error failed to correct motors $errMsg
#y        set screening_msg "failed to correct motors before run: $errMsg"
#y        return -code error $errMsg
#y    }
#y}
puts "lock sil"
...
#yangx The following lines were in the "if" switch statement
#      when I comment out anyline (for example #LoopAlignment)
#      the "switch" think "#" is part of the value to be compared
#      So I can't make comment out "#" in the switch statement 

#y        MountNextCrystal {
#y            set m_lockNextCrystal 1
#y            eval doMountNextCrystal $params
#y            set m_lockNextCrystal 0
#y            clearImages
#y            setStrategyFileName disMnted
#y        }
#y        Pause { doPause }
#y        OptimizeTable { doOptimizeTable }
#y	  LoopAlignment { eval doLoopAlignment }
#y        VideoSnapshot { eval doVideoSnapshot $params }
#y        CollectImage { eval doCollectImage $params }
#y        ExcitationScan { eval doExcitation $params }
#y        Rotate { eval doRotate $params }
#y        ReOrient { doReOrient }
#y        RunQueueTask { sleep 2000 }
#y        test { sleep 5000 }
...
#y        ReOrient -
#y        RunQueueTask -
#y        Rotate { log_warning "skipped $actionClass for not mounted sample [getCurrentPortID]" }
#y        test { sleep 5000 }
...
if {!$m_skipThisSample} {
	...
	GrabberWarmCycle { eval doGrabberWarmCycle $params }
        RunQueueTask { sleep 2000 }
        test { sleep 5000 }
        default { puts "ERROR SequenceDevice::runAction actionClass $actionClass not supported yangx" }
    }
} else {
    switch -exact -- $actionClass {
        MountNextCrystal {
...
::itcl::body SequenceDevice::loadCrystalList { } {
    ...
    set cassetteList [lindex $m_cassetteInfo 2]
    set cassette [lindex $cassetteList $index]
#puts "yangx user=$user index=$index cassetteList=$cassetteList cassette=$cassette"
...
#puts "yangx m_cassetteInfo=$m_cassetteInfo cassette_index=$cassette_index cassette_status=$cassette_status"
 ...
 #   puts "yangx index=$index m_PortIndex=$m_PortIndex m_currentCassetteStatus=$m_currentCassetteStatus"

 #  puts "yangx m_numRowParsed=$m_numRowParsed  m_SelectedIndex=$m_SelectedIndex  crystalList=$crystalList "
...
if { $currentPortOK && $nextPortOK } {
	...
	#yangx need to make sure to set screen mode off" after mount next crystal.
	# this is set in the itcl::body SampleMountingDevice::mountNextCrystal
...
#yangx I don't understand this. Changed to 4 to 0        set mt_status [lindex $errorText 4]
	set mt_status [lindex $errorText 0]
...
#yangx add
	if { $op_result_l > 1 } {
		if { [lindex $errorText 1] == "sample_ok" } {
			log_error "Set Sample number $errorText"
			return
		}
	}

	puts "SequenceDevice::doMountNextCrystal() done $errorText errorFlag=$errorFlag "
        if { $errorFlag || $op_status != "normal" || $op_result_l < 4 } {
            puts "ERROR SequenceDevice::doMountNextCrystal() $errorText"
            log_error "Screening mountNextCrystal $errorText"
            set screening_msg "error: $errorText"
...
puts "portIndex=$portIndex m_indexMap=$m_indexMap spreadsheet_index=$spreadsheet_index"
...
#puts "xyang casIndex=$casIndex CIndex=$CIndex RIndex=$RIndex"
    ...
    set cassette_status [lindex $robot_cassette $cassette_index]
#yangx add. because the folowing calculations might be base on stanford 
# sample configuration. So in my case, it should be 4 less pucks sample
# for middle page and 8 less pucks for the right page.

    if { $CIndex >= 8 } {
	set CIndex [expr $CIndex - 8]
    } elseif { $CIndex >= 4 } {
	set CIndex [expr $CIndex - 4]
    }
