#################################################
# take both 0 and 90 degree at all grid nodes
#################################################
...
proc xraySampleSearch_start { username args } {
   ...
   variable $gMotorBeamHeight
    ...
    set save_x0 $sample_x
    set save_y0 $sample_y
    set save_z0 $sample_z
    set save_phi0 $gonio_phi
   ...
   set beamSizeMicrons "[expr int($beamSize_x * 1000)]X[expr int($beamSize_y * 1000)]"
   ...
   log_note "Starting xray search."
	...
	if { [catch {
      ...
      move $gMotorBeamWidth to $beamSize_x
      move $gMotorBeamHeight to $beamSize_y
      ...
      set deltaXPlane [expr -$scanWidth /2 + $beamSize_x /2]
      set deltaYPlane [expr -$scanHeight /2 + $beamSize_y /2]
      ...
      for { set x 0} { $x < $totalColumns } {incr x} {
         ...
         for { set y 0} { $y < $totalRows } { incr y} {
            ...
            set deltaSampleXmm [calcDeltaSampleX $phi [expr $y * $beamSize_y]]
            set deltaSampleYmm [calcDeltaSampleY $phi [expr $y * $beamSize_y]] 
            set deltaSampleZmm [expr $beamSize_x * $x] 
            ...
            doVideoSnapshot $username $sessionID $directory ${filenameWithIndex}_002
            ...
            doVideoSnapshot $username $sessionID $directory ${filenameWithIndex}_001
			   set operationHandle [start_waitable_operation collectFrame \
										 0 \
											 ${filenameWithIndex}_002 \
											 $directory \
											 $username \
											 gonio_phi \
											 shutter \
   ...
   move sample_x to $save_x0
   move sample_y to $save_y0
   move sample_z to $save_z0
   move gonio_phi to $save_phi0
   wait_for_devices sample_x sample_y sample_z gonio_phi
}
...
proc getVideoUrl {} {
   ...
   set videoServerUrl "http://smb.slac.stanford.edu"

   switch -exact -- $beamlineID {
      BL1-5 {
         set  sampleVideoPath "/BluIceVideo/bl15/video1/axis-cgi/jpg/image.cgi?camera=2"
      }
      BL7-1 {
