...
//    double newPosition;
    ...
    sscanf (message, "%*s %200s %200s", deviceName, &newPosition );
    ...
    if(newPos=atof(newPosition))
    {
    	get_circle_corrected_destination( device, newPos, & newPos );
	sprintf(newPosition,"%lf",newPos);
    }

    /* send the move command to the appropriate hardware client */
    sprintf( buffer, "stoh_start_motor_move %s %s", deviceName, newPosition );

    LOG_INFO2("yang send stoh_start_motor_move %s %s",deviceName, newPosition);
...
client_profile_t * user ) {
    ...
    LOG_INFO("xiaochun gtos_read_ion_chambers: exit\n");
