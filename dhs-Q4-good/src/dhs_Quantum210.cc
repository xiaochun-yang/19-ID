...
xos_result_t quantum210Configuration( );
xos_result_t handleMessagesQ210( xos_thread_t	*pThread );
xos_result_t handleResetRunQ210( xos_socket_t * commandQueue, char * message );
xos_result_t requestSubFrameQ210( xos_socket_t * commandSocket,
...
xos_result_t create_header_q210( char * buffer  );
...
XOS_THREAD_ROUTINE imageWriterThreadRoutineQ210( void *args );
xos_result_t connectDataPortQ210 ( xos_socket_t * moduleSocketAddress );
char *header_string_Q210(char *keyword, char *header, int last=0);
double header_value_Q210(char *keyword, char *header, int last=0);
...
static xos_boolean_t mStoredDarks;
...
// *************************************************************
// Quantum210Thread: This is the function that is called by DHS once
...
XOS_THREAD_ROUTINE Quantum210Thread( void * parameter)
	...
	// initialize devices
	if ( quantum210Configuration() == XOS_FAILURE )
		{
		xos_semaphore_post( initData->semaphorePointer );
		LOG_SEVERE("Quantum210Thread: initialization failed" );
      xos_error_exit("Exit.");
		}
	...
	if ( xos_semaphore_create( & semaphore, 0 ) != XOS_SUCCESS )
		{
		LOG_SEVERE("Quantum210Thread: cannot create semaphore." );
      xos_error_exit("Exit.");
		}
	...
	if ( xos_thread_create( &ccdThread,
									Quantum210ControlThread,
									(void *)&semaphore) != XOS_SUCCESS )
		{
      LOG_SEVERE("Quantum210Thread: error creating internal message thread");
      xos_error_exit("Exit.");
      }	
	...
	handleMessagesQ210( mThreadHandle );
		
	LOG_WARNING("Quantum210Thread: error handling messages");
	XOS_THREAD_ROUTINE_RETURN;
	}


// *****************************************************************
// quantum210Configuration: connects to the configuration database
// and does the following based on the information found there:
// sets up directories.
// creates message queues for the Quantum210ControlThread and image assembler thread.
...
xos_result_t quantum210Configuration( )
   ...
   mSerialNumber = gConfig.getStr("quantum210.serialNumber");
   mCommandHost =  gConfig.getStr("quantum210.commandHostname");
   mCommandPort =  gConfig.getInt(std::string("quantum210.commandPort"), 0);
   std::string dataHostnames= gConfig.getStr("quantum210.dataHostnameList");
   std::string dataPortList = gConfig.getStr("quantum210.dataPortList");
   std::string beamCenter = gConfig.getStr("quantum210.beamCenter");
	mDarkRefreshTime = gConfig.getInt(std::string("quantum210.darkRefreshTime"),7200);
	std::string darkExposureTolerance = gConfig.getStr("quantum210.darkExposureTolerance");
	std::string writeRaw = gConfig.getStr("quantum210.writeRawImages");
    std::string mStoredDarksString = gConfig.getStr("quantum210.storedDarks");
   ...
   if (mCommandHost == "") 
      ...
      printf("Example:\n");
      printf("quantum210.commandHostname=q210-01\n"); 
      xos_error_exit("Exit.");
      }
   ...
   if (mCommandPort == 0) 
      ...
      printf("Example:\n");
      printf("quantum210.commandPort=8041\n"); 
      xos_error_exit("Exit.");
      }

   if ( sscanf(dataHostnames.c_str(),"%s %s %s %s",
         mDataHostname[0], mDataHostname[1], mDataHostname[2],
         mDataHostname[3])  != NUM_MODULES)
   {
      LOG_SEVERE("Need 4 hostnames in config file. One for each module.\n");
      printf("====================CONFIG ERROR=================================\n");
      printf("Need 4 hostnames in config file. One for each module.\n");
      printf("Example:\n");
      printf("quantum210.dataHostnameList=q210-01 q210-02 q210-03 q210-04\n"); 
      xos_error_exit("Exit.");
   }

      
    LOG_WARNING1("%s",dataPortList.c_str());
   if ( sscanf(dataPortList.c_str(),"%d %d %d %d",
	      &mDataPortInt[0], &mDataPortInt[1], &mDataPortInt[2],
	      &mDataPortInt[3]) != NUM_MODULES )
   {
      LOG_SEVERE("Need 4 data ports in config file. One for each module.\n");
      printf("====================CONFIG ERROR=================================\n");
      printf("Need 4 data ports in config file. One for each module.\n");
      printf("Example:\n");
      printf("quantum210.dataPortList=9042 9042 9042 9042\n"); 
      xos_error_exit("Exit.");
   }
   ...
   if ( sscanf(beamCenter.c_str(),"%f %f", &mBeamCenterX, &mBeamCenterY ) != 2 )
   {
      ...
      printf("Example:\n");
      printf("quantum210.dataPortList=157.5 157.5\n"); 
      xos_error_exit("Exit");
   }
   ...
   if (mStoredDarksString != "" )
   {
        if (mStoredDarksString == "Y" || mStoredDarksString == "y" )
           mStoredDarks = TRUE;
        else
            mStoredDarks = FALSE;
   }
   ...
   {
           xos_error("quantum210Configuration: storedDarks not defined in config file.");
           mStoredDarks = FALSE;
   }

   if (mStoredDarks)
           printf("initialize_detector: using stored dark images.\n");
   else
           printf("initialize_detector: WARNING: Not using stored darks.\n");
   ...
   // 
    ...
    dhs_database_set_string(deviceIndex,"Q210CCD"); 
	...
	mDark.isValid = FALSE;
    if (mStoredDarks) mDark.isValid = TRUE;
...
// *********************************************************************
// handleMessagesQ210: handles messages from DCSS regarding 
// data collection.
...
xos_result_t handleMessagesQ210( xos_thread_t	*pThread )
   ...
   char dcssCommand[MAX_DCSS_RETURN_MESSAGE];
	...
	while (TRUE) 
		...
		//The Quantum210Control thread should not let us connect until
		// it is fully  initialized and connected to the CCD.

		if (commandSocketStatus == XOS_FAILURE) 
			{
			//try to connect
			LOG_INFO1("handleMessagesQ210: connecting to Quantum210Control thread on port %d.",
					 mCommandQueueListeningPort);
			while ( xos_socket_create_and_connect( & commandQueue,
																"localhost",
																mCommandQueueListeningPort ) != XOS_SUCCESS)
				{
				LOG_WARNING("handleMessagesQ210: error connecting to Quantum210ControlThread.");
				xos_thread_sleep(1000);
				continue;
				}
			
			LOG_INFO("handleMessagesQ210: connected to commandQueue. Waiting for 'ready'");

			// read reply from Quantum210Control thread.
			if ( xos_receive_dcs_message( &commandQueue, &replyMessage ) == XOS_FAILURE )
				{
				LOG_WARNING("Quantum210ControlThread: lost connection from message handler.");
				goto socket_error;
				}
			
			LOG_INFO("handleMessagesQ210: got 'ready' from Quantum210Control thread");
		...
		if ( xos_thread_message_receive( pThread, (xos_message_id_t *) &messageID,
			...
			{
			LOG_SEVERE("handleMessagesQ210: got error on message queue.");			
         xos_error_exit("Exit.");
			}
		...
		if ( messageID == DHS_CONTROLLER_MESSAGE_BASE )
			...
			switch (  ((dhs_card_message_t *) message)->CardMessageID )
				...
				case DHS_MESSAGE_KICK_WATCHDOG:
					...
					continue;
				default:
					LOG_WARNING1("handleMessagesQ210:  unhandled controller message %d", 
		...
		if ( messageID == DHS_MESSAGE_OPERATION_REGISTER )
			{
			LOG_INFO("handleMessagesQ210: registered operation\n");
		...
		if ( messageID == DHS_MESSAGE_OPERATION_START )
			...
			if ( strcmp(operationName, "detector_collect_image") == 0 ||
				...
				{
				if ( handleResetRunQ210( &commandQueue, operationPtr ) != XOS_SUCCESS )
					{
					// inform DCSS that the command failed
					xos_semaphore_post( semaphore );
					LOG_WARNING("handleMessagesQ210: error writing to command queue");
					goto socket_error;
					}
				xos_semaphore_post(semaphore);
				continue;
				}
			else if (strcmp(operationName,"Quantum210Controlthread_error") == 0) 
				{
				LOG_WARNING("handleMessagesQ4: Quantum210Controlthread reported error.");
				goto socket_error;
				}
			else
				{
				xos_semaphore_post( semaphore );
				LOG_WARNING1("handleMessagesQ210: unhandled operation %s", operationName );
				continue;
				}
			}
		...
		if ( messageID == DHS_MESSAGE_OPERATION_ABORT )
			{  
			LOG_INFO("handleMessagesQ210: got abort\n");
			//forward the message to the command thread
		...
		LOG_WARNING("handleMessagesQ210: error handling messages");
		xos_semaphore_post( semaphore );
		continue;
		...
		LOG_INFO("handleMessagesQ210: disconnecting from DCSS");
		...
		LOG_INFO("handleMessageQ210: destroying Quantum210Control socket");

		// close the Quamtum4Control queue socket.
		if ( xos_socket_destroy( &commandQueue ) != XOS_SUCCESS )
			LOG_WARNING("handleMessageQ210: error disconnecting from detector");	
		...
		while ( xos_thread_message_receive( mThreadHandle, (xos_message_id_t *) &messageID,
			...
			if ( messageID == DHS_MESSAGE_OPERATION_START )
				...
				if (strcmp(((dhs_start_operation_t *)message)->message,"stoh_start_operation flush_the_queue! dummyHandle") == 0) 
					{
					LOG_INFO("handleMessageQ210: Flushed the queue!");
					break;
					}
				}
			}
...
XOS_THREAD_ROUTINE Quantum210ControlThread( void * args )
	{
	// local variables
	xos_socket_t 		commandSocket; //socket for sending commands to ADSC Q210
	...
	if ( xos_semaphore_create( & semaphore, 0 ) != XOS_SUCCESS )
		{
		LOG_SEVERE("Quantum210ControlThread: cannot create semaphore." );
      xos_error_exit("Exit.");
		}

	LOG_INFO ("Quantum210ControlThread: initialize imageAssembler thread\n");
	// create a thread to assemble images from NUM_MODULES detector chips
	if ( xos_thread_create( &imageAssemblerThread,
									imageAssemblerRoutineQ210, 
									(void *)&semaphore ) != XOS_SUCCESS )
      {
		LOG_SEVERE("Quantum210ControlThread: error creating imageAssembler thread");
      xos_error_exit("Exit.");
      }
	...
	while ( xos_socket_create_server( &commandQueueServer, 0 ) != XOS_SUCCESS )
		{
		LOG_WARNING("Quantum210ControlThread: error creating socket for command queue.");
	...
	if ( xos_socket_start_listening( &commandQueueServer ) != XOS_SUCCESS ) 
      {
		LOG_SEVERE("Quantum210ControlThread: error listening for incoming connection.");
      xos_error_exit("Exit.");
      }	
	...
	while (TRUE)
		...
		while ( xos_socket_create_and_connect( & assemblerMessageQueue,
			...
			{
			LOG_WARNING("Quantum210ControlThread: error connecting to assembler thread.");
			xos_thread_sleep(1000);
			}

		//The Q210 can have a good 'command' socket while simultaneously having a bad 'data' socket.
		...
		LOG_INFO("QuantumQ210ControlThread: Connected to assembler thread. Waiting for 'ready' from assembler thread.");

		// read next command from message queue
		if ( xos_receive_dcs_message( &assemblerMessageQueue, &commandBuffer ) == XOS_FAILURE )
			{
			LOG_WARNING("Quantum210ControlThread: lost connection from assembler handler.");
			goto disconnect_Quantum210_command_socket;
			}

		LOG_INFO("QuantumQ210ControlThread: got 'ready' from assembler thread");
		
		// Now that we have a connection to the image assembler queue, and the assembler threads
		// has connections to all 4 modules, we can connect to the command port.
		while ( xos_socket_create_and_connect( & commandSocket,
															(char *)mCommandHost.c_str(),
															mCommandPort ) != XOS_SUCCESS )
			{
			LOG_WARNING("Quantum210ControlThread: error connecting to Q210 command socket.");
			xos_thread_sleep(1000);
			}
		...
		while ( xos_socket_accept_connection( &commandQueueServer,
			...
			{
			LOG_WARNING("Quantum210ControlThread: waiting for connection from message handler");
			}
		...
		if ( xos_send_dcs_text_message( &commandQueueSocket,
												  "ready!" ) != XOS_SUCCESS )
			{
			LOG_WARNING("Quantum210ControlThread: error writing to command queue");
			goto disconnect_Quantum210_command_socket;
			}

		LOG_INFO("Quantum210ControlThread: got connection from Message Handler.");


		while (TRUE)
			{
			LOG_INFO("Quantum210ControlThread: Reading next command from queue...");
			...
			if ( FD_ISSET(commandSocket.clientDescriptor,&vitalSockets) != 0 )
				{
				LOG_INFO("Quantum210ControlThread: Unexpected event on CCD command socket.");

				// read a character and break out of read loop if an error occurs
				if ( xos_socket_read( &commandSocket, &character , 1) != XOS_SUCCESS )
					{
					LOG_INFO("Quantum210ControlThread: Got error on CCD command socket.");
					goto disconnect_Quantum210_command_socket;
					}

				LOG_INFO1("Quantum210ControlThread: found extra character from CCD: '%c'\n", character );
				goto disconnect_Quantum210_command_socket;
				}
			...
			if ( FD_ISSET( assemblerMessageQueue.clientDescriptor, &vitalSockets) != 0 )
				{
				LOG_INFO("Quantum210ControlThread: Unexpected event on xform queue socket.");
			...
			LOG_INFO2("TIME: %f Quantum210ControlThread got_message_{%s}\n", TIMESTAMP(time_stamp), logBuffer);

			// LOG_INFO("Quantum210ControlThread: Reading next command from queue...");
			// read next command from message queue

		
			//LOG_INFO("Quantum210ControlThread: Got command:");
			...
			if ( strcmp( commandToken, "detector_collect_image" ) == 0 )
				...
				LOG_INFO("Quantum210ControlThread: checking file permissions.\n");
				if ( createWritableDirectory( (const char *)frame.userName,
														(const char *)frame.directory ) == XOS_FAILURE )
					{
					LOG_WARNING("Quantum210ControlThread: not authorized to write to that directory.");
				...
				LOG_INFO("Quantum210ControlThread: checking for endangered files.\n");
				...
				LOG_INFO2("Quantum210ControlThread: frame.exposureTime %f, darkExposureTime %f\n",
						 frame.exposureTime,
						 mDark.exposureTime );
				
				LOG_INFO1("Quantum210ControlThread: exposure diff %f\n", 
				...
				// kick off the CCD and the imageAssemblerRoutineQ210 thread
				if ( requestSubFrameQ210( &commandSocket,
			...
			// *************************************
				...
				if ( mDetectorExposing == TRUE )
					...
					if ( xos_socket_write( &commandSocket, "stop\nend_of_det\n",
												  strlen("stop\nend_of_det\n") )  != XOS_SUCCESS )
						{
						LOG_WARNING("Quantum210ControlThread: could not send stop to detector");
						result = XOS_FAILURE;
						break;
						}
					}
					...
					{
					LOG_WARNING("Quantum210ControlThread: WARNING!! mDetectorExposing is FALSE during exposure!");
					}

				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread sent_stop_to_ccd %s\n",TIMESTAMP(time_stamp), frame.filename  );
				...
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread stop_ok_from_ccd %s\n",TIMESTAMP(time_stamp), frame.filename);
				}
			// ***********************************************************
			...
			// ***********************************************************
				...
				{
				LOG_INFO("Quantum210ControlThread: received oscillation_ready\n");
				// inform CCD and the imageAssemblerRoutineQ210 thread
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread axis_motor_ready_for_oscillation %s\n",TIMESTAMP(time_stamp),frame.filename );
				...
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread requesting_oscillation_from_dcss %s\n",TIMESTAMP(time_stamp),frame.filename );
			...
			// **************************************************************************
				...
				{
				//detector_reset_run doesn't really do anything for the Q210
			...
			// *****************************************
				...
				if ( mDetectorExposing == TRUE )
					...
					if ( xos_socket_write( &commandSocket, "stop\nend_of_det\n",
												  strlen("stop\nend_of_det\n") )  != XOS_SUCCESS )
						{
						LOG_WARNING("Quantum210ControlThread: error writing to CCD");
						continue;
						}
					if ( waitForOk(&commandSocket) != XOS_SUCCESS )
						goto disconnect_Quantum210_command_socket;
				...
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread sending_flush_to_ccd %s\n",TIMESTAMP(time_stamp),frame.filename );
				...
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f Quantum210ControlThread flush_ok_from_ccd %s\n",TIMESTAMP(time_stamp),frame.filename );
		...
		disconnect_Quantum210_command_socket:

		mDark.isValid = FALSE;
        if( mStoredDarks ) mDark.isValid = TRUE;

		LOG_INFO("Quantum210ControlThread: closing connection to CCD command socket\n.");
		...
		if ( xos_socket_destroy( &assemblerMessageQueue ) != XOS_SUCCESS )
			LOG_WARNING("Quantum210ControlThread: error disconnecting from detector");
		...
		/* fill in message structure */
		LOG_INFO("Quantum210ControlThread: sending message back to message queue.");
		messageReset.deviceIndex		= 0;
		messageReset.deviceType		= DCS_DEV_TYPE_OPERATION;
		sprintf( messageReset.message, "stoh_start_operation Quantum210Controlthread_error dummyHandle");
...
// ********************************************************************
// requestSubFrame from a Q210 detector: 
// ********************************************************************
xos_result_t requestSubFrameQ210( xos_socket_t * commandSocket,
											 xos_socket_t * assemblerMessageQueue,
											 subframe_states_t & subFrameState,
	...
	if ( getOscillationTime( subFrameState, frame, oscillationTime ) != XOS_SUCCESS)
		{
		LOG_WARNING("requestSubFrameQ210: could not get oscillationTime");
		return XOS_FAILURE;
		}
	...
	clock_gettime( CLOCK_REALTIME, &time_stamp );
	LOG_INFO2("TIME: %f Quantum210ControlThread sending_start_to_ccd %s\n", TIMESTAMP(time_stamp), frame.filename );
		
	LOG_INFO("requestSubFrameQ210: Writing start command to CCD detector..." );
	
	//LOG_INFO1("requestSubFrameQ210: out -> CCD: %s\n", detectorCommand);
	// send the command to the CCD detector
	if ( xos_socket_write( commandSocket, detectorCommand, 
								  strlen(detectorCommand) )  != XOS_SUCCESS )
		{
		LOG_WARNING("requestSubFrameQ210: could not write to CCD detector.");
		return XOS_FAILURE;
		}
	...
	clock_gettime( CLOCK_REALTIME, &time_stamp );
	LOG_INFO2("TIME: %f Quantum210ControlThread start_ok_from_ccd %s\n", TIMESTAMP(time_stamp), frame.filename );
...
xos_result_t handleResetRunQ210( xos_socket_t * commandQueue, char * message )
	...
	if ( xos_send_dcs_text_message( commandQueue,
											  message ) != XOS_SUCCESS )
		{
		LOG_WARNING("handleResetRunQ210: error writing to command queue");
		return XOS_FAILURE;
		}
...
XOS_THREAD_ROUTINE imageAssemblerRoutineQ210( void *args )
	{
	/* local variables */
	...
	LOG_INFO("imageAssemblerRoutineQ210: create semaphore set\n");
	// create a set of semaphores
	if ( xos_semaphore_set_create( & moduleSemaphoreSet, 10 ) == XOS_FAILURE )
		{
		LOG_SEVERE( "imageAssemblerRoutineQ210:  error creating semaphore set");
      xos_error_exit("Exit.");
		//return XOS_FAILURE;
		}

	int xChipCoordMult[NUM_MODULES] = { 0, 1,
										0, 1 };
	int yChipCoordMult[NUM_MODULES] = { 0, 0,
										1, 1 };
	...
	while (TRUE) 
		{
		//connectDataPortQ210 won't return until its got 4 good connections.
		connectDataPortQ210( moduleSocket  );
		mAllDataSocketsGood = TRUE;

		//Now that a connection has been established with all of the CCD data ports,
		// this thread is ready for connection from the Quantum210Control thread.
		...
		while ( xos_socket_accept_connection( &assemblerQueueServer,
			...
			{
			LOG_WARNING("imageAssemblerRoutineQ210: waiting for connection from Quantum210Control thread.");
			}

		LOG_INFO("imageAssemblerRoutineQ210: got connection from Quantum210Control thread.");
		...
		if ( xos_send_dcs_text_message( &assemblerQueueSocket,
												  "ready!" ) != XOS_SUCCESS )
			{
			LOG_WARNING("imageAssemblerThread: error writing 'ready' to Quantum210Control thread.");
			goto disconnect_Quantum210_data_socket;
			}
		...
		while (TRUE)
			{
			LOG_INFO("imageAssemblerRoutineQ210:  reading next message from xform queue...");
			...
			LOG_INFO1("imageAssemblerRoutineQ210: selectResult: %d\n", selectResult);
			...
			clock_gettime( CLOCK_REALTIME, &time_stamp );
			LOG_INFO2("TIME: %f imageAssemblerRoutineQ210 got_message:{%s}\n", TIMESTAMP(time_stamp), messageBuffer.textInBuffer);

			/* parse the message */
			sscanf( messageBuffer.textInBuffer, "%s %s %s %d %d %d %d ", 
					  filename, directory, userName, &runIndex, &detectorMode, &binning, &imageType );
			
			//LOG_INFO1( "imageAssemblerRoutineQ210:  Received: %s %s %d %d %d\n", filename, directory, runIndex, binning, imageType );
			LOG_INFO1("imageAssemblerRoutineQ210:  EXPECTING IMAGE %s\n", filename);

			//LOG_INFO("imageAssemblerRoutineQ210: starting to read out image...\n");
			//
			...
			if ( mImageFree[ imageBufferIndex ] == FALSE ) 
				{
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_WARNING2("TIME: %f imageAssemblerRoutineQ210 wasting_time_waiting_for_image(%d)_to_be_written\n", TIMESTAMP(time_stamp),imageBufferIndex );
				LOG_WARNING("imageAssemblerRoutineQ210: ************** LOSING TIME WAITING FOR IMAGE TO BE WRITTEN OUT !..\n");
				if ( xos_semaphore_wait( &imageWriteSemaphore[imageBufferIndex], 0 ) != XOS_SUCCESS )		 
					{
					LOG_SEVERE("imageAssemblerRoutineQ210: error waiting for semaphore");
      xos_error_exit("Exit.");
					}
				
				clock_gettime( CLOCK_REALTIME, &time_stamp );
				LOG_INFO2("TIME: %f imageAssemblerRoutineQ210 image(%d)_now_written\n", TIMESTAMP(time_stamp), imageBufferIndex );
				LOG_INFO("imageAssemblerRoutineQ210: semaphore posted, image written out...\n");
				}
			...
			if ( img_set_dimensions ( image, colCount  * SQRT_NUM_MODULES , rowCount * SQRT_NUM_MODULES  ) != 0)
				{
				LOG_SEVERE("imageAssemblerRoutineQ210: Could not allocate memory for mRawImage");
      xos_error_exit("Exit.");
				}
			...
			if ( xos_semaphore_set_initialize( &moduleSemaphoreSet ) == XOS_FAILURE )
				{
				LOG_SEVERE("imageAssemblerRoutineQ210:  error initializing semaphore set.");
      xos_error_exit("Exit.");
				//return XOS_FAILURE;
				}
			...
			for ( module = 0; module < NUM_MODULES; module++)
				...
				totalImageWidth = colCount*SQRT_NUM_MODULES;
				...
				if ( xos_semaphore_set_get_next( & moduleSemaphoreSet, 
					...
					{
					LOG_INFO1("imageAssemblerRoutineQ210: could not get semaphore for module %d\n",module); 
					LOG_SEVERE("imageAssemblerRoutineQ210:  cannot get semaphore." );
      xos_error_exit("Exit.");
					//return XOS_FAILURE;
					}
				
				if ( xos_thread_create(&chipReaderThread[module], chipReaderRoutineQ210, &area[module]  ) != XOS_SUCCESS)
					{
					LOG_SEVERE("imageAssemblerRoutineQ210: could not start new thread.");
      xos_error_exit("Exit.");
					}
				}

			//LOG_INFO1("imageAssemblerRoutineQ210: waiting for semaphores from all NUM_MODULES chips...\n");

			clock_gettime( CLOCK_REALTIME, &time_stamp );
			LOG_INFO2("TIME: %f imageAssemblerRoutineQ210 waiting_for_assembler_threads %s\n", TIMESTAMP(time_stamp),filename );
			...
			if ( xos_semaphore_set_wait( & moduleSemaphoreSet, 60000 ) != XOS_WAIT_SUCCESS )
				{
				LOG_SEVERE("imageAssemblerRoutineQ210:   timeout for threads to read the chip." );
      xos_error_exit("Exit.");
				//return XOS_FAILURE;
				}
			...
			if ( mAllDataSocketsGood == TRUE )
				{
				//LOG_INFO("imageAssemblerRoutine: ****************received image*********\n");
                ...
                memcpy(imageDescriptor[imageBufferIndex].imgdetheader, area[0].detheader, 10240);
      ...
      xos_error_exit("Exit.");
		...
		disconnect_Quantum210_data_socket:

		// Close the Quantum210Control thread socket to let it know that this
		// thread is having serious problems.
		if ( xos_socket_destroy( &assemblerQueueSocket ) != XOS_SUCCESS )
			{
			LOG_WARNING("imageAssemblerRoutineQ210:"
						 " error disconnecting from the Quantum210Control thread");
			}
		...
		for ( module = 0; module < NUM_MODULES; module ++)
			...
			if ( xos_socket_destroy( &moduleSocket[module] ) != XOS_SUCCESS )
				LOG_WARNING("imageAssemblerRoutineQ210: error disconnecting from detector");
			}
		}
...
xos_result_t connectDataPortQ210 ( xos_socket_t * moduleSocketAddress )
	...
	while ( connected < NUM_MODULES )
		...
		for ( module = 0; module < NUM_MODULES; module ++ ) 
			...
			if ( socketStatus[module] != XOS_SUCCESS )
				...
				if ( xos_socket_create_and_connect( &moduleSocketAddress[module],
					...
					{
					LOG_INFO1("connectDataPortsQ210: error connecting to module %d .",module);
					xos_thread_sleep(1000);
					}
				else
					{
					socketStatus[module] = XOS_SUCCESS;
					LOG_INFO2("connectDataPortsQ210: Connect to det_api_module:%d on port %d...\n",
							 module,
							 mDataPort[module] );
					}
				}
		...
		if ( selectResult == -1 )
			{
			LOG_WARNING("connectDataPortsQ210: error checking socket status\n"); 
			}
			...
			if (socketStatus[module] == XOS_SUCCESS )
				...
				if ( FD_ISSET(moduleSocketAddress[module].clientDescriptor, &allSockets) != 0 )
					...
					if ( xos_socket_read( &moduleSocketAddress[module],
						...
						{
						LOG_INFO1("connectDataPortQ210: Got error on CCD data socket, module %d", module );
						}
					else
						{
						LOG_INFO1("connectDataPortQ210: data socket already readable on module: '%c'!\n", module );
						}
					socketStatus[module] = XOS_FAILURE;
					...
					if ( xos_socket_destroy( &moduleSocketAddress[module] ) != XOS_SUCCESS )
						LOG_WARNING("connectDataPortQ210: error disconnecting from detector");
					}
				}
			}
		xos_thread_sleep(1000);
...
XOS_THREAD_ROUTINE chipReaderRoutineQ210( void *arg )
	...
	LOG_INFO3("TIME: %f chipReaderRoutineQ210(%d) started_reading_header %s\n",
	...
	if ( xos_socket_read( area->moduleSocket, (char *)buffer, 512 ) != XOS_SUCCESS )
		{
		LOG_INFO1("chipReaderRoutineQ210[%d]: Socket error while reading header.\n",area->chipNumber);
		mAllDataSocketsGood = FALSE;
		//post the semaphore
		if ( xos_semaphore_post( area->semaphorePointer ) != XOS_SUCCESS )
			{
			LOG_SEVERE("chipReaderRoutineQ210: could not post semaphore.");
      xos_error_exit("Exit.");
			}
		XOS_THREAD_ROUTINE_RETURN;
		}
 ...
 // figure out how long the full header is

    /* find first instance of "HEADER_SIZE" keyword in the header */
    int header_bytes = (int) header_value_Q210("HEADER_BYTES",(char *) buffer);

    // now read the rest of the header
    if (header_bytes > 512)
        {
        string = buffer + 512;
        if ( xos_socket_read( area->moduleSocket, (char *)string, header_bytes-512 ) != XOS_SUCCESS )
        {
            printf("chipReaderRoutine[%d]: Socket error while reading rest of header.\n",area->chipNumber);
            mAllDataSocketsGood = FALSE;
            //post the semaphore
            if ( xos_semaphore_post( area->semaphorePointer ) != XOS_SUCCESS )
            {
            xos_error_exit("chipReaderRoutine: could not post semaphore.");
            }
            XOS_THREAD_ROUTINE_RETURN;
        }
    ...
    // Should check to see that header_bytes is not larger than 1024
    memcpy(area->detheader, buffer, 10240);

	clock_gettime( CLOCK_REALTIME, &time_stamp_1 );
	LOG_INFO3("TIME: %f chipReaderRoutineQ210(%d) finished_reading_header %s\n",
			 TIMESTAMP(time_stamp_1),
			 area->chipNumber,
			 area->filename);

	totalImageWidth = area->width * SQRT_NUM_MODULES;
	LOG_INFO1("chipReaderRoutineQ210[%d]: reading data...\n",area->chipNumber);
	for( row = 0; row < area->height;  row++)
		{
		//LOG_INFO1("chipReaderRoutineQ210[%d]: reading row %d...\n",area->chipNumber, row);
		// read a byte
		if ( xos_socket_read( area->moduleSocket, (char *)buffer, area->width * 2 ) != XOS_SUCCESS )
			{
			LOG_INFO1("chipReaderRoutineQ210[%d]: Socket error while reading data.\n",area->chipNumber);
			mAllDataSocketsGood = FALSE;			
			//post the semaphore
			if ( xos_semaphore_post( area->semaphorePointer ) != XOS_SUCCESS )
				{
      ...
      xos_error_exit("Exit.");
		...
		for ( col = 0, dataPtr = buffer;
			...
			{
			//LOG_INFO1("chipReaderRoutineQ210[%d]: %d\n",  area->chipNumber, col);
	...
	//LOG_INFO1("chipReaderRoutineQ210[%d]: collected %d\n", area->chipNumber,dataCount );
	

	clock_gettime(  CLOCK_REALTIME, &time_stamp_2 );
	LOG_INFO3("TIME: %f chipReaderRoutineQ210(%d) finished_reading_data %s\n",
	...
	if ( TIMESTAMP(time_stamp_2) - TIMESTAMP(time_stamp_1) > 2.0 )
		...
		LOG_INFO("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
	//	for (row = 0; row < area->height; row ++)
	//		LOG_INFO1("TIME: %f chipReaderRoutineQ210(%d): row %d %s\n",
...
XOS_THREAD_ROUTINE imageWriterThreadRoutineQ210( void *args )
	...
	writeImageQ210 ( thisImage->image, 
						  thisImage->directory,
						  thisImage->filename,
						  thisImage->messageBuffer,
                          thisImage->imgdetheader );
...
xos_result_t writeImageQ210(  img_handle 	 image, 
	...
	// interpret other contents of the header which are generated by the ADSC frame grabber code
	if(1)
	{
		char	*string;
		char buffer[10240];

		if(NULL != (string = header_string_Q210("ACC_TIME", detector_header, 1)))
	    {
			sprintf(buffer,"%d",atoi(string));
			img_set_field(image, "ACC_TIME", buffer);
            printf("Setting ACC_TIME to %s in header\n", buffer);
		}
        else
            printf("header_string_Q210 returned NULL on ACC_TIME keyword search\n");
		if(NULL != (string = header_string_Q210("CREV", detector_header, 1)))
		{
			sprintf(buffer,"%d",atoi(string));
			img_set_field(image, "CREV", buffer);
		}
		if(0 == strcmp(binningString, "2x2"))
		{
			if(NULL != (string = header_string_Q210("BIN_TYPE", detector_header, 1)))
			{
				if(0 == strncmp("HW", string, 2))
				{
					img_set_field(image, "ADC", "fast");
					img_set_field(image, "BIN_TYPE", "HW");
				}
				else if(0 == strncmp("SW", string, 2))
				{
					img_set_field(image, "ADC", "slow");
					img_set_field(image, "BIN_TYPE", "SW");
				}
			}
		}
		else
	...
	chmod( fullpath, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
...
/* added by JMH 6-22-07 */
char *header_string_Q210(char *keyword, char *header, int last)
{
    char *string;
    int keylen = strlen(keyword);

    /* start at the beginning of the header */
    string = header;
    ...
    if(! last) {
        ...
        if(string == NULL) {
	    printf("header_string_Q210(): error %s does not occur in header\n",keyword);
	    return NULL;
        }
    ...
    if(string == NULL) {
	/* no "=", perhaps keyword just does not exist here? */
	return NULL;
    }
    /* skip the "=" */
    ++string;
...
/* added by JMH 6-22-07 */
double header_value_Q210(char *keyword, char *header, int last)
{
    double value;
    char *string = header_string_Q210(keyword,header,last);
    if(string == NULL) {
	printf("header_value_Q210(): error %s does not occur in header\n",keyword);
	return 0.0;
    }
