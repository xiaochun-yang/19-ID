...
xos_result_t Dmc2180::init_connection( )
	...
	if ( xos_socket_address_set_ip_by_name( &serverAddress, hostname.c_str() ) == XOS_FAILURE)
	{
	...
	for ( int axis = 0; axis < DMC2180_MAX_ENCODERS; axis ++ )
		...
		if (relativeEncoder[axis].axisUsed ) 
			{
			relativeEncoder[axis].get_current_position( &storedEncoderValue[axis] );
			}
		}
	...
	//command the dmc2180 to close all sockets
	if ( (execute("XQ #ShutAll,1",response, &error_code,FALSE) == XOS_FAILURE) || (error_code !=0))
	{
		LOG_INFO1("%s\n",response);
	...
	if ( xos_thread_create( &unsolicitedMessageThread, dmc2180_unsolicited_handler, 
									(void *) this ) != XOS_SUCCESS )
		{
...
//yang adds for testing. redeifne the addrerss
//	tempIPArray[0]=10;
//	tempIPArray[1]=0;
//	tempIPArray[2]=0;
//	tempIPArray[3]=7;
	...
	execute( command2, response, &error_code,FALSE);
        LOG_INFO1("MG _IHF2 response %s \n" ,response);

	
	LOG_INFO("Create message to report new port number to dmc2180");
	sprintf( command2, "IHF=%d,%d,%d,%d <%d >2",
				tempIPArray[0],
				tempIPArray[1],
				tempIPArray[2],
				tempIPArray[3],
				xos_socket_get_local_port( &unsolicitedHandler ) );
	...
	execute( command2, response, &error_code, FALSE);
       LOG_INFO1("MG _IHF0 response %s \n" ,response);
	

	sprintf( command2, "MG _IHF1" );
	execute( command2, response, &error_code, FALSE);
       LOG_INFO1("MG _IHF1 response %s \n" ,response);

	sprintf( command2, "MG _IHF3" );
	execute( command2, response, &error_code, FALSE);
       LOG_INFO1("MG _IHF3 response %s \n" ,response);

	//	LOG_INFO(response);
	if (error_code != 0 )
		{
		xos_error("dmc2180_initialize -- could not initialize asynchronous message port.\n");
		return XOS_FAILURE;
		}

	LOG_INFO("connect complete");

	/* set limits to active high */
...
xos_result_t Dmc2180::execute( char * command, char * response, int * error_code, xos_boolean_t silent)
	{
...
xos_result_t Dmc2180::download( char * response, int * error_code)
	...
	if ( xos_socket_write ( & socket, script.c_str() , script.length() ) != XOS_SUCCESS )
		{
...
/*the dmc2180 class.*/
xos_result_t Dmc2180::motor_store_stepper_configuration( char * axisCharacter, int * index )
	{
...
xos_result_t Dmc2180_motor::init( )
	...
	if ( isStepper )
       ...
       /* 	if ( assertMotorType((char*)dmc2180->expectedStepperMotorType.c_str()) == XOS_FAILURE) xos_error_exit("Please set the expected motor type correctly in the galil using the MT command");
	*/
		}
		...
		if (error_code != 0 )
        	...
        	if ( assertMotorType((char*)dmc2180->expectedServoMotorType.c_str()) == XOS_FAILURE) xos_error_exit("Please set the expected motor type correctly in the galil using the MT command");

		sprintf( command, "KD%c=%s", axisLabel, PIDderivative.c_str() );
				
		if ( controller_execute( command,  &error_code,FALSE ) == XOS_FAILURE )
			{
			xos_error("Dmc2180::initialize_motors -- Could not set motor pid parameters");
			return XOS_FAILURE;
			}
	...
	if ( set_position( initPosition ) == XOS_FAILURE )
		{
		xos_error("Dmc2180::initialize_motors -- could not set motor position\n");
		return XOS_FAILURE;
		};
	...
	if ( set_acceleration( ) == XOS_FAILURE )
		{
		xos_error("Dmc2180::initialize_motors -- could not set motor acceleration\n");
		return XOS_FAILURE;
		};	
	return XOS_SUCCESS;
	}
...
xos_result_t  Dmc2180_motor::set_acceleration( )
	...
	/* set the accelleration on the Galil board */
	sprintf( command, "AC%c=%ld",
				axisLabel, speed / acceleration * 1000 );
	
	if ( controller_execute( command, &error_code, FALSE ) == XOS_FAILURE )
		return XOS_FAILURE;

	sprintf( command, "DC%c=%ld", axisLabel, speed / acceleration * 1000 );
...
xos_result_t  Dmc2180RelativeEncoder::get_current_position( dcs_scaled_t * position)
	{
...
xos_result_t  Dmc2180AnalogEncoder::get_current_position( dcs_scaled_t * position)
	...
	controller_execute( command, &error_code, FALSE );
	 ...
	 //LOG_INFO1("Xiaochun command : %s\n",command);
	
	if (error_code == 0)
		{
		*position = (dcs_scaled_t)atof( lastResponse ) / scale_factor;
		//LOG_INFO2("yang_ion_chamber ion=%s position=%lf\n",lastResponse,*position);
		}
	else
		{
		LOG_WARNING("Error getting current position\n");
...
xos_result_t  Dmc2180AbsoluteEncoder::get_current_position( dcs_scaled_t * position)
	{
	int error_code;
	/* get position of axis */
...
xos_result_t Dmc2180_motor::start_move( dcs_unscaled_t  new_destination,
	...
	/* construct and send message to dmc2180 */
	sprintf( command,
				"PA%c=%ld",axisLabel, new_destination );
	...
	if ( stopCode != 0 && stopCode != 100)
		...
		if ( motion_complete == 0 )
			...
			if ( stopCode != 1 && stopCode != 101 )
				...
				if ( unscaledReference == unscaledPosition )
					{
					LOG_INFO("isMoving: STOP LEVEL4b: reference = position\n"); 
...
xos_result_t Dmc2180::get_stop_codes( int *codes )
	...
	sscanf( response, "%d,%d,%d,%d,%d,%d,%d,%d", 
			  codes, codes+1, codes+2, codes+3, codes+4, codes+5, codes+6,codes+7);
...
xos_result_t Dmc2180_device::controller_execute( char * message, int *error_code, xos_boolean_t silent) 
	{
	if ( !silent)
		LOG_INFO( message );
...
xos_result_t Dmc2180::kick_watchdog(int kickValue)
	...
	//Now kick the wathdog on the dmc2180.
