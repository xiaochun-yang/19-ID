...
xos_result_t dmc2180ConfigureServoMotor( Dmc2180 & dmc2180 )
   ...
   gConfig.getRange( gDhsInstanceName + std::string(".servo"), servoList);
	...
	for (; i != servoList.end(); ++i)
		{
		strncpy(lineStr, (*i).c_str(), 500);
		if ( sscanf( lineStr, "%s %s %s %s %s",
						 motorName,
                   channel, pidDerivative, pidProportional, pidIntegrator ) != 5 )
   			{
	   		//only throw away blank lines
		   	if ( strcmp(lineStr,"\n") != 0 )
				   {
			      ...
			      printf("====================CONFIG ERROR=================================\n");
				   printf("Invalid servo line in config file: \n%s\n",lineStr);
               printf("Example:\n");
               printf("%s.servo=motorName channel Derivative Proportional Integrator\n",gDhsInstanceName.c_str());
               xos_error_exit("Exit.");

				   LOG_INFO1("Invalid line in config file: %s\n",lineStr);
				   exit(1);
				   }
			   }
		...
		LOG_INFO2("%s index is %d\n", motorName,deviceIndex);
         ...
         int motor_index;
	      if ( dmc2180.motor_store_servo_configuration( (char *)channel,
               &motor_index,
               (char *)pidDerivative,
               (char *)pidProportional,
               (char *)pidIntegrator ) == XOS_FAILURE)
		   {
...
xos_result_t setVolatileMotorConfig( Dmc2180 & dmc2180, int motor_index, xos_index_t deviceIndex )
   {
    ...
    dmc2180.motor[motor_index].acceleration = dhs_database_get_acceleration(deviceIndex);
...
xos_result_t dmc2180_messages( xos_thread_t	*pThread,
	...
	while ( xos_thread_message_receive( pThread, (xos_message_id_t *) &messageID,&semaphore, &message ) == XOS_SUCCESS )
		{
		...
		if ( messageID == DHS_CONTROLLER_MESSAGE_BASE )
			...
			switch (  ((dhs_card_message_t *) message)->CardMessageID )
				...
				case DHS_MESSAGE_KICK_WATCHDOG:
					...
					result = dmc2180_kick_watchdog( (dhs_watchdog_kick_message_t *) message, dmc2180, semaphore );
					if (result != XOS_SUCCESS ) return XOS_FAILURE;
					break;
				default:
					xos_error("dmc2180_messages: unhandled controller message %d", 
		...
		switch ( ((dhs_generic_message_t *) message)->deviceType )
			...
			default:
				...
				xos_error("dmc2180_messages:  unhandled device type %d", 
...
xos_result_t dmc2180_motor_configure( dhs_motor_configure_message_t	*message, 
	...
	dhs_database_set_acceleration( deviceIndex, message->acceleration );
...
xos_result_t dmc2180_local_motor_poll( xos_index_t deviceIndex )
	...
	//LOG_INFO("DMC2180: polling device %d\n", deviceIndex);
...
xos_result_t dmc2180_motor_set( dhs_motor_set_message_t	*message, 
	...
	//LOG_INFO1("yangset motor message=%f\n", message->position);
...
xos_result_t dmc2180_set_speed_acceleration( xos_index_t		deviceIndex,
	...
	(*motor).speed = speed;
	(*motor).acceleration = accelerationTime;
...
xos_result_t dmc2180_motor_start_oscillation( dhs_motor_start_oscillation_message_t	*message,
	...
	double				oscTime;
	dcs_scaled_t		scaledDestination;
	dcs_unscaled_t		currentPosition;
	dcs_unscaled_t		unscaledDestination;
	dcs_unscaled_t		unscaledOscillationRange;
	dcs_unscaled_t		speed;
	dcs_unscaled_t		accelerationTime;
	dcs_unscaled_t		acceleration;
	dcs_unscaled_t		rampTime;
	dcs_unscaled_t		rampDistance;
	int					errorCode;
	dcs_flag_t			reverse;
	dcs_flag_t			shutterUsed;
	...
	/* store message variables locally */
	deviceIndex		= message->deviceIndex;
	shutterIndex	= message->shutterDeviceIndex;
	oscTime			= message->oscTime;
	...
	/* access device data */
	motor = (Dmc2180_motor * )
		 dhs_database_get_volatile_data( deviceIndex );

	/* access controller data for the oscillation */
	dmc2180 = (Dmc2180 *)( (*motor).dmc2180);

	/*make sure that another vector sequence isn't running*/
   if ( (*dmc2180).isVectorActive() == TRUE)
	  	{
	  	xos_error("Vector sequence already running on %d", (*dmc2180).hostname.c_str());
 	  	dhs_database_release_device_mutex( deviceIndex );
	  	/* signal calling thread */
	  	xos_semaphore_post( semaphore );
 		/* report failure */
	...
	/*check to see if direction of motor is reversed*/
	reverse = (dcs_flag_t)dhs_database_get_reverse_flag( deviceIndex );


	currentPosition = (*motor).lastPosition;

	unscaledOscillationRange = SCALED2UNSCALED(message->oscRange, dhs_database_get_scale_factor( deviceIndex ) );
	
	/* calculate unscaled destination */
	unscaledDestination =
		currentPosition + unscaledOscillationRange;

	/* calculate scaled destination */
	scaledDestination = unscaledDestination / dhs_database_get_scale_factor( deviceIndex );
	
	/* calculate speed of oscillation */
	speed = (long) ( (double)unscaledOscillationRange / oscTime );
	...
	accelerationTime = dhs_database_get_acceleration( deviceIndex );

	/* get acceleration rate */
	acceleration = (speed * 1000) / accelerationTime;

	/* how long to spend on ramp up? */
	if ( accelerationTime < 100 )
		...
		{
		rampTime = 2 * accelerationTime;
		}

	/* what distance to ramp up in? */
	rampDistance = (rampTime * speed) / 1000;

	/* set a minimum ramp distance */
	if ( rampDistance < 3 ) 
		{
		rampDistance = 3;
		}
	...
	LOG_INFO4("speed			= %ld\nacceleration	= %ld\n"
				"rampTime		= %ld\nrampDistance	= %ld\n",
				speed, acceleration, rampTime, rampDistance );

	if ( shutterIndex == 9999 )
		{
		shutter_channel = 0; //not used
		shutterUsed = (dcs_flag_t)0;
		LOG_INFO(buffer);
		}
	else
 		{
		/* access device data */
		shutter = (Dmc2180_shutter * ) dhs_database_get_volatile_data( shutterIndex );	
		shutter_channel = (*shutter).channel;
		shutterUsed = (dcs_flag_t)1;
		}
	...
	if ( reverse ) 
		...
		sprintf( buffer,
					...
					"FrontFil=%d;"
					"StopOsc=0;"
					"XQ #ShutREV,2;"		/*execute Shutter program in thread #2*/
					"VM %cN;"				/*vector motion on Axis & N(ull) axis*/
					"CR %ld,90,-180;"		/*circular motion */
					"VP %ld,0;"				/*Vector Position*/
					"CR %ld,270,-180;"		/*circular motion*/
		...
		sprintf( buffer,
					...
					"FrontFil=%d;"
					"StopOsc=0;"
					"XQ #ShutFWD,2;"		/*execute Shutter program in thread #2*/
					"VM %cN;"				/*vector motion on volatileData->axisLabel & N(ull) axis*/
					"CR %ld,90,180;"		/*circular motion */
					"VP %ld,0;"				/*Vector Position*/
					"CR %ld,270,180;"		/*circular motion*/
					"VE;"						/*vector sequence end*/
	...
	//LOG_INFO( buffer );
	errorCode = (*dmc2180).execute( buffer, response, &errorCode, FALSE );
	//LOG_SEVERE("error = %d\n", errorCode );
	...
	if ( errorCode != 0 )
		...
		//				handle_move_start_error( deviceIndex, errorCode, volatileData );	
		/* release exclusive access to database entry */
		dhs_database_release_device_mutex( deviceIndex );
		/* report failure */
		return XOS_FAILURE;
	...
	(*dmc2180).setVectorActive( TRUE );
   /*oscillation requires only one motor in vector move*/
	(*dmc2180).setNumVectorComponents( 1 );
	(*motor).isVectorComponent = TRUE;
...
xos_result_t dmc2180_kick_watchdog( dhs_watchdog_kick_message_t *message,
	...
	if ( mHutchDoorBit == TRUE )
		...
		if  (dmc2180.getDigitalInput( &digitalInput ) == XOS_SUCCESS )
			{
			//const char * doorStatus = ( (int)digitalInput & (int)pow((double)2, (double)(mHutchDoorBitChannel -1))) ? "closed" : "open";
			hutchDoorBit = ( (int)digitalInput & (int)pow((double)2, (double)(mHutchDoorBitChannel -1)));
			motorStopBit = ( (int)digitalInput & (int)pow((double)2, (double)(mMotorStopChannel -1)))  >> (mMotorStopChannel-1);
...
//	Dmc2180  * dmc2180;
	...
	if ( xos_socket_accept_connection( &socket, newSocket ) != XOS_SUCCESS )
		{ 
		// LOG_SEVERE("yangxx Error accepting connection from client");
		goto FAILURE;
		}
	...
	while (cnt < 2000)
		...
		// LOG_INFO2("message[0]:%d:%c",message[0],message[0]);
		...
		if (message[0] == '\n')
			...
			LOG_INFO2("unsolicitedMessage: %s commandToken: %s",unsolicitedMessage, commandToken);   //yang test messages from galil
			...
			if (messageLookup == messageTable.end())
				...
				{
				LOG_INFO2("%s received message with enum %d\n",(*dmc2180).hostname.c_str(), messageLookup->second );
				}
			...
			/* wait for semaphores */
			if ( xos_semaphore_wait( &semaphore, 10000 ) != XOS_SUCCESS )
				{
