...
xos_result_t writeImagePipeQ315( image_descriptor_t * img, int file);
xos_result_t sendPipeInputToImpersonFile( image_descriptor_t * img, int file );
...
xos_result_t writeImageFileQ315( image_descriptor_t * img);
XOS_THREAD_ROUTINE imageWriterThreadRoutineQ315( void *args );
xos_result_t finalizeHeaderQ315(image_descriptor_t * img);
xos_result_t connectDataPortQ315 ( xos_socket_t * moduleSocketAddress );
char *header_string_Q315(char *keyword, char *header, int last=0);
double header_value_Q315(char *keyword, char *header, int last=0);
...
xos_result_t quantum315Configuration( )
	...
	std::string mStoredDarksString = gConfig.getStr("quantum315.storedDarks");

	
    std::string useImperson = gConfig.getStr("quantum315.useImperson");
    if ( useImperson == "Y" || useImperson == "y" || useImperson == "T" || useImperson == "t" ) {
        mImpHost = gConfig.getStr("quantum315.impHost");
        mImpPort = gConfig.getInt(std::string("quantum315.impPort"),61001);
    	mUseImperson =  TRUE;
    } else {
        mUseImperson = FALSE;
    }
   ...
   if ( sscanf(dataHostnames.c_str(),"%s %s %s %s %s %s %s %s %s",
         ...
         mDataHostname[6],	mDataHostname[7],	mDataHostname[8])  != NUM_MODULES )
   {
   ...
   if ( sscanf(dataPortList.c_str(),"%d %d %d %d %d %d %d %d %d",
	      ...
	      &mDataPortInt[6], &mDataPortInt[7], &mDataPortInt[8]) != NUM_MODULES )
   {
   ...
   {
	   xos_error("quantum315Configuration: storedDarks not defined in config file.");
	   mStoredDarks = FALSE;
   }
   ...
   // 
	...
	// add the device to the local database
	if ( dhs_database_add_device( "detectorStatus", "string", mThreadHandle, 
											&deviceIndex, &deviceType ) == XOS_FAILURE )
		{
		LOG_WARNING("Could not add detectorType string.");
		return XOS_FAILURE;
   ...
   char dcssCommand[MAX_DCSS_RETURN_MESSAGE];
	...
	while (TRUE) 
		...
		if ( messageID == DHS_MESSAGE_STRING_REGISTER )
			{
			LOG_INFO("register string\n");
			
			xos_semaphore_post( semaphore );

	        sprintf( dcssCommand, "htos_set_string_completed detectorType normal Q315CCD");

	        /* send the message to the server */
	        dhs_send_to_dcs_server( dcssCommand );

	        
	        sprintf( dcssCommand, "htos_set_string_completed detectorStatus normal NA");
		...
		if ( messageID == DHS_MESSAGE_OPERATION_REGISTER )
			{
			LOG_INFO("handleMessagesQ315: registered operation\n");
...
xos_result_t requestSubFrameQ315( xos_socket_t * commandSocket,
	...
	if ( mWriteRawImages ) 
		...
		sprintf( assembleCommand, "%s %s %s %d %d %d %d %d %f %f %f %f %f %f %f %f %s",
					rawFilename,
					frame.directory,
					frame.userName,
					frame.runIndex,
...
XOS_THREAD_ROUTINE imageAssemblerRoutineQ315( void *args )
	...
	while (TRUE) 
		{
		//connectDataPortQ315 won't return until it has 9 good connections.
		...
		while (TRUE)
			...
			/* parse the message */
			sscanf( messageBuffer.textInBuffer, "%s %s %s %d %d %d %d %*s %*s %*s %*s %*s %*s %*s %*s %*s %s", 
					  filename, directory, userName, &runIndex, &detectorMode, &binning, &imageType, sessionId );
      ...
      xos_error_exit("Exit.");
			...
			if ( xos_semaphore_set_wait( & moduleSemaphoreSet, 0 ) != XOS_WAIT_SUCCESS )
				{
			...
			if ( mAllDataSocketsGood == TRUE ) {
				...
				image_descriptor_t * imgPtr = &imageDescriptor[imageBufferIndex];
				
				sscanf(messageBuffer.textInBuffer, "%*s %*s %s %*s %*s %d %*s %*s %s %s %s %s %s %s %s",
						imgPtr->userName, &imgPtr->binning, imgPtr->oscStart, imgPtr->oscRange, imgPtr->distance, imgPtr->wavelength,
						imgPtr->beamCenterX, imgPtr->beamCenterY, imgPtr->exposureTime);
				
				//strcpy( imageDescriptor[imageBufferIndex].messageBuffer , messageBuffer.textInBuffer );
				strcpy( imageDescriptor[imageBufferIndex].sessionId, sessionId );
				strcpy( imageDescriptor[imageBufferIndex].userName, userName );
...
XOS_THREAD_ROUTINE chipReaderRoutineQ315( void *arg )
	...
	int header_bytes = (int) header_value_Q315("HEADER_BYTES",(char *) buffer);
	...
	if (header_bytes > 512)
	{
		...
		{
			LOG_WARNING1("chipReaderRoutine[%d]: Socket error while reading rest of header.\n",area->chipNumber);
			mAllDataSocketsGood = FALSE;
			//post the semaphore
			if ( xos_semaphore_post( area->semaphorePointer ) != XOS_SUCCESS )
			{
	...
	if ( TIMESTAMP(time_stamp_2) - TIMESTAMP(time_stamp_1) > 2.0 )
		{
		LOG_WARNING1("MODULE %d read slow\n",area->chipNumber);
		}
...
XOS_THREAD_ROUTINE sendPipeInputToImperson( void *args ) {
	pipeAndImage_t * pipeAndImage = (pipeAndImage_t *)args;
	
	LOG_INFO("reading image data from imglib output" );
	/* This is the child process.  */
	sendPipeInputToImpersonFile( pipeAndImage->img, pipeAndImage->pipe );
	
	xos_semaphore_post( &pipeAndImage->semaphore );
...
XOS_THREAD_ROUTINE imageWriterThreadRoutineQ315( void *args ) {
	...
	finalizeHeaderQ315(thisImage);

	if ( mUseImperson) {
		LOG_INFO("Creating PIPE");
		/* Create the pipe.  */
		if (pipe (mypipe)) {
			xos_error_exit("Create pipe failed.");
		}
		...
		/* get the next semaphore */
		if ( xos_semaphore_create( &t.semaphore, 0 ) != XOS_SUCCESS ) {
			LOG_SEVERE("Quantum315Thread: cannot create semaphore." );
			xos_error_exit("Exit.");
		}
		
		LOG_INFO("Create thread.");
		xos_thread_t    pipeThread;
		if ( xos_thread_create( &pipeThread, sendPipeInputToImperson , (void *)&t) != XOS_SUCCESS )
			{
	      LOG_SEVERE("Quantum315Thread: error creating internal message thread");
	      xos_error_exit("Exit.");
	      }		
		...
		LOG_INFO("output image data to pipe" );
		/* This is the parent process.  */

		LOG_INFO("writing file to pipe");
		writeImagePipeQ315( thisImage, mypipe[1]);
		
		LOG_INFO("Done writing file");
		
		xos_semaphore_wait( &t.semaphore,0);
		LOG_INFO("Post semaphore");

		
	} else {
		writeImageFileQ315 ( thisImage );
	}
...
xos_result_t finalizeHeaderQ315(image_descriptor_t * img) {
	img_handle image = img->image;
	char * detector_header = img->imgdetheader;
	char *string;	
	
	if (img->binning == 1) {
		img_set_field(image, "BIN", "none");
		img_set_field(image, "PIXEL_SIZE", "0.051294");
		img_set_field(image, "ADC", "slow");
	} else {
		img_set_field(image, "BIN", "2x2");
		img_set_field(image, "PIXEL_SIZE", "0.102588");
		

		if (NULL != (string = header_string_Q315("BIN_TYPE", detector_header, 1))) {
	...
	img_set_field(image, "DISTANCE", img->distance);
	img_set_field(image, "WAVELENGTH", img->wavelength);
	img_set_field(image, "OSC_START", img->oscStart);
	img_set_field(image, "OSC_RANGE", img->oscRange);
	img_set_field(image, "PHI", img->oscStart);
	img_set_field(image, "BEAM_CENTER_X", img->beamCenterX);
	img_set_field(image, "BEAM_CENTER_Y", img->beamCenterY);
	img_set_field(image, "TIME", img->exposureTime);

	// interpret other contents of the header which are generated by the ADSC frame grabber code
	char buffer[10240];	
	if (NULL != (string = header_string_Q315("ACC_TIME", detector_header, 1))) {
		sprintf(buffer, "%d", atoi(string));
		img_set_field(image, "ACC_TIME", buffer);
		LOG_FINEST1("Setting ACC_TIME to %s in header\n", buffer);
	} else
		LOG_FINEST("header_string_Q315 returned NULL on ACC_TIME keyword search\n");
	if (NULL != (string = header_string_Q315("CREV", detector_header, 1))) {
		sprintf(buffer, "%d", atoi(string));
...
xos_result_t writeImageFileQ315( image_descriptor_t * img) {
	...
	sprintf( fullpath, "%s/%s", img->directory, img->filename );
	img_write_smv (img->image, fullpath, 16);
...
xos_result_t writeImagePipeQ315( image_descriptor_t * img, int file) {
	...
	img_write_stream_smv (img->image, 16, stream );
...
// ********************************************************************
xos_result_t q315WriteImageWriteFailureResponseToDcss( std::string fullPathName )
{
...
xos_result_t sendPipeInputToImpersonFile( image_descriptor_t * img, int file ) {
	...
	XosTimeCheck check("read_image"); //raii
	
	ImpFileAccess imp( mImpHost, img->userName, img->sessionId);

	FILE *stream = fdopen( file,"r");
	if (stream ==0 ) {
		LOG_SEVERE("could not open pipe for reading image");
		return XOS_FAILURE;
	}

	LOG_INFO2("writing %s as %s", img->filename, img->userName);

    try {
    	sprintf( fullpath, "%s/%s", img->directory, img->filename );
    	imp.sendStreamToFile( fullpath, stream );
        fclose(stream);
    ...
    q315WriteImageWriteFailureResponseToDcss(fullpath);
...
char *header_string_Q315(char *keyword, char *header, int last)
{
	char *string;
	...
	if(! last) {
		...
		if(string == NULL) {
			LOG_WARNING1("header_string_Q315(): error %s does not occur in header\n",keyword);
			return NULL;
		}
...
double header_value_Q315(char *keyword, char *header, int last)
{
	double value;
	char *string = header_string_Q315(keyword,header,last);
	if(string == NULL) {
		LOG_WARNING1("header_value_Q315(): error %s does not occur in header\n",keyword);
		return 0.0;
	}
