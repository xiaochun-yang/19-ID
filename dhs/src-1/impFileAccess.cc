...
xos_result_t ImpFileAccess::sendStreamToFile( std::string fullPath, FILE * stream) {
	...
	std::string uri = "";
        uri += std::string("/writeFile?impUser=") + mUserName
               + "&impSessionID=" + mSessionId
               + "&impFilePath=" + fullPath
               + "&impBackupExist=true"
               + "&impFileMode=0740";
	...
	LOG_INFO1("imperson set up, ready to read stream, %s", uri.c_str() );
	
	while ((numRead = fread(buf, sizeof(char), bufSize, stream)) > 0) {
