...
#define DMC2180_MAX_DIGITAL_INPUT_BLOCK 2
...
class Dmc2180_digitalInput : public Dmc2180_device
	{
	public:

	int					state;
	xos_boolean_t	  	channel_used;
	string				stringMembership;
	string				inputName;
	int					channel;
	string				lowVoltageStr;
	string				highVoltageStr;
	};
...
private:
	...
	xos_result_t timedExposureServoMotor( dhs_motor_start_oscillation_message_t & exposeCmd);
	xos_result_t timedExposureStepperMotor( dhs_motor_start_oscillation_message_t & exposeCmd);
...
public:
	...
	xos_boolean_t isScriptParticipant;
	int scriptThreadNumber;
...
public:
	...
	xos_result_t start_move( dcs_unscaled_t new_destination,
			char * error_string );
	xos_result_t    start_home( char * deviceName, char * error_string );
	...
	xos_boolean_t checkScriptDone();
	xos_result_t handleStop( );

	xos_result_t abort_move_soft();
	xos_result_t abort_move_hard();
	xos_result_t timedExposure( dhs_motor_start_oscillation_message_t & exposeCmd);
...
public:
	...
	std::string limitSwitchPolarity;
	std::string sampleRateMs;
	...
	Dmc2180_digitalInput digitalInput[DMC2180_MAX_DIGITAL_INPUT_BLOCK * 8];
	...
	xos_result_t assertLimitSwitchPolarity( );
	xos_result_t assertSampleRate();
	...
	string updateString(string stringName);
