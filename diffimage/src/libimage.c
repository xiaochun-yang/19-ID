 ...
 * - The parser will look for { character until it finds one. 
 *   It will read character by character  until eof is reached, 
 *   which will make it very slow before this parser can decide that 
 *   the file is not in the smv format.
 * - Once it has found { character, it will expect to find HEADER_BYTES in 
 *   the next line. 
 * 
 */
int img_read_smvheader (img_handle img, FILE *file)
{
  ...
  if (dimension [0] == 2048 && dimension [1] == 2048)

    status |= img_set_field (img, "DETECTOR", "ADSC QUANTUM210");

  if (dimension [0] == 6144 && dimension [1] == 6144)
...
int img_read_smvdata (img_handle img, FILE *file)
{
  ...
  while ((readcount = (int)fread (data + datacount, 1, 4096 - datacount, file)) > 0)
  {
...
int img_read_smv_header(img_handle img, const char *name)
{
  FILE * file;
...
int img_write_smv (img_handle   img, 
                 const char   *name,
                 unsigned int  bits) {
	  ...
	  int code = img_write_stream_smv(img, bits,file);
...
int img_write_stream_smv (img_handle   img, 
                   unsigned int  bits, FILE *file)
{
  ...
  
  {
    ...
    "BEAM_CENTER_Y",
	 "OVERLOAD_CUTOFF",    /* Pixel Overload (counts)                       */
	 "ACC_TIME",    /* Total ccd acc time                       */
	 "CREV",    /* controller revision  0 for non-r. 1 for r detectors */
	 "BIN_TYPE",
     NULL
  };
  ...
  for (tag = tags; *tag; tag++)
  {
    ...
    if (val)
      ...
      bytes += (int)(strlen (*tag) + strlen (val) + 3);
  }  
  ...
  if (fputs (data, file) == EOF) return img_BAD_WRITE;

  bytes -= (int)strlen (data);
  ...
  if (fputs ("}\014", file) == EOF)  return img_BAD_WRITE;
                 
  bytes -= 2;

  if (bytes < 0) return img_BAD_ARGUMENT;

  while (bytes > 0)
  {
    if (fputc ('\040', file) == EOF) return img_BAD_WRITE;
  ...
  while (pixel != stop_pixel)
  {
    ...
    if (little)
      ...
      else
      {
        c [0] = (unsigned char)(value);
        c [1] = (unsigned char)(value >>  8);
        c [2] = (unsigned char)(value >> 16);
        c [3] = (unsigned char)(value >> 24);
      }
    else
    ...
    if (data_size >= 4096)
    {
      done = (int)fwrite (data, 1, data_size, file);
        
      if (done <= 0) return img_BAD_WRITE;
...
/**
 * The first 4 bytes (reversed order of the first 4 bytes) 
 * must represent an integer number 
 * 1200, 1600, 1800, 2000, 2300, 2400, 3000, or 3450.
 **/
...
int img_read_mar300_header (img_handle img, const char *name)
{
  FILE * file;
...
/**
 * The first 4 bytes or the first 4 bytes in reversed order
 * must represent number 1234.
 **/
...
int img_read_mar345_header (img_handle img, const char *name)
{
  FILE * file;
...
//  img_set_field(img, "IMAGE_TYPE", "unknown");
  
  status = img_set_number (img, "OVERLOAD_CUTOFF", "%g", 250000);

  /*****************************************************
   * TIFF FAMILY
   * - The first 2 bytes must be characters II or MM.
   * - The next 2 bytes (or the reversed byte order) must represent a 16 bit 
   *   unsigned integer 42, 
   *****************************************************/

  // This format is a subset of tiff
  // Expecting 1024 bytes of standard tiff header, which is not used; 
  // followed by 3072 bytes of private header. Then followed by
  // the raw image, whose size in bytes is determined by fields
  // in the private header section.
  status = img_read_mar165(img, name);

  if (status == 0) {
//  	  img_set_field(img, "IMAGE_TYPE", "mar165");
	  xos_log("%s image is in mar165 format\n", name);
	  return 0;
  }

  /*****************************************************
   * MAR FAMILY
   * - The first 4 bytes (or the reversed byte order) must
   *   represent an integer. Each type of mar format expects
   *   a certain value of this integer such as 1234, 1200 etc.
   *****************************************************/

  // The first 4 bytes the first 4 bytes in reversed order
  // must represent an integer 1234
  status = img_read_mar345 (img, name);

  if (status == 0) {

//  	 img_set_field(img, "IMAGE_TYPE", "mar345");
	 xos_log("%s image is in mar345 format\n", name);
	 return 0;
  }
...
//  	 img_set_field(img, "IMAGE_TYPE", "mar300");
	 xos_log("%s image is in mar300 format\n", name);
     return 0;
  }
  ...
  /*****************************************************
   * CBF FAMILY
   * - The first 15 bytes must be ascii "###CBF: VERSION"
   *****************************************************/

  // The first 4 bytes must represent 
  status = img_read_cbf (img, name);

  if (status == 0)

  {
//  	 img_set_field(img, "IMAGE_TYPE", "cbf");
	 xos_log("%s image is in cbf format\n", name);
     return 0;
  }

#ifdef SUPPORT_MINICBF
  status = img_read_minicbf (img, name);
  if (status == 0)

  {
//  	 img_set_field(img, "IMAGE_TYPE", "minicbf");
	 xos_log("%s image is in minicbf format\n", name);
     return 0;
  }
#endif
  ...
  /*****************************************************
   * SMV FAMILY
   * - This is going to be the slowest one to test.
   *   The parser reads one character at a time until
   *   it finds '{' character.
   *****************************************************/

  // Potentially slow; it will read character by character from the file
  // until it finds { character before it can tell whether or not the
  // file is in smv format.
  status = img_read_smv (img, name);

  if (status == 0)

  {
	 status = img_set_number (img, "OVERLOAD_CUTOFF", "%g", 65535);

//  	 img_set_field(img, "IMAGE_TYPE", "smv");
	 xos_log("%s image is in smv format\n", name);
     return 0;
  }
 ...
 * Read image header
 */
int img_read_header(img_handle img, const char *name)
{
  int status;
  ...
  // the raw image, whose size in bytes is determined by fields
  // in the private header section.
  status = img_read_mar165_header(img, name);
  ...
  // The first 4 bytes the first 4 bytes in reversed order
  // must represent an integer 1234
  status = img_read_mar345_header(img, name);
...
//  	 img_set_field(img, "IMAGE_TYPE", "mar345");
	...
	// must represent an integer number 
	// 1200, 1600, 1800, 2000, 2300, 2400, 3000, or 3450.
  status = img_read_mar300_header(img, name);
...
//  	 img_set_field(img, "IMAGE_TYPE", "mar300");
  ...
  // The first 4 bytes must represent 
  status = img_read_cbf_header(img, name);
...
#ifdef SUPPORT_MINICBF
  status = img_read_minicbf_header(img, name);
...
//  	 img_set_field(img, "IMAGE_TYPE", "minicbf");
  ...
  // until it finds { character before it can tell whether or not the
  // file is in smv format.
  status = img_read_smv_header(img, name);
