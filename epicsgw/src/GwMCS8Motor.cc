...
GwBaseMotor::MotorField GwMCS8Motor::m_fields[CHID_END] = {
    //name       monitor   type      
...
GwMCS8Motor::GwMCS8Motor( const char* name, const char* localName, bool real )
...
, m_sendConfigPending(false)
{
    ...
    if (pSep == NULL) {
        LOG_SEVERE2( "MCS8Motor %s cannot find device prefix in localname {%s}",
            name, localName
        );
        return;
    } else {
        ...
        strcpy( m_mrn, pSep + 1 );
        LOG_FINEST3( "MCS8Motor %s got DEVICE: {%s} MRN: {%s}",
            name, m_device, m_mrn
        );
    }
    connectEPICS( );
}
void GwMCS8Motor::fillPVMap( ) {
...
void GwMCS8Motor::sendMCS8MoveCompleted( ) {
    const char* pStatus;
...
void GwMCS8Motor::sendMCS8Config( ) {
    //ignore lock for now
...
void GwMCS8Motor::updateDCSSByState( int triggerIndex ) {
    LOG_FINEST2( "MCS8Motor %s updateDCSSByState index %d",
        m_name, triggerIndex
    );
    switch (m_dcsStatus) {
...
void GwMCS8Motor::updateDCSSByData( int triggerIndex ) {
    switch (triggerIndex) {
    case CHID_MON:
        updateDCSSByMON( );
        break;
...
void GwMCS8Motor::updateDCSSByBUSY( ) {
    ...
    if (m_busyCurrent == 0) {
        ...
        if (m_dcsStatus != DCS_DEVICE_ACTIVE) {
            ...
            LOG_FINEST1( "MCS8 motor %s got BUSY=true while active",
                m_name
            );
        }
    }
}
void GwMCS8Motor::updateDCSSByMON( ) {
...
void GwMCS8Motor::updateDCSSByConfig( ) {
    switch (getDcsStatus( )) {
    case DCS_DEVICE_WAITING_ACK:
    case DCS_DEVICE_ACTIVE:
    case DCS_DEVICE_ABORTING:
...
void GwMCS8Motor::updateDCSSByPoll( ) {
    switch (m_dcsStatus) {
    case DCS_DEVICE_WAITING_ACK:
    ...
    default:
        LOG_WARNING2( "MCS8Motor %s polled when dcsStatus=%s",
...
void GwMCS8Motor::updateDCSSByRefresh( ) {
    if (m_busyCurrent) {
        m_dcsStatus = DCS_DEVICE_ACTIVE;
        sendMoveStarted( );
    } else {
...
void GwMCS8Motor::move( double newPosition ) {
...
void GwMCS8Motor::stop( ) {
    ...
    case DCS_DEVICE_ACTIVE:
        ...
        if (!flushEPICSOnePV( CHID_ST_CMD )) {
            LOG_WARNING1( "aborting MCS8Motor %s failed", m_name );
        } else {
            LOG_FINEST1( "aborting MCS8Motor %s", m_name );
            ca_flush_io( );
        }
        break;
...
void GwMCS8Motor::fillPVValPointers( int index ) {
    switch (index) {
    case CHID_ST_CMD:
...
void GwMCS8Motor::updateDCSSByLimits( int index ) {
