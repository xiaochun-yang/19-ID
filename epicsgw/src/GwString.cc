...
bool GwString::convertStringToNative( const char* pString ) {
    ...
    if (type == DBR_CHAR && count > 1 && m_waveformString) {
        if (strlen(pString) > (size_t)count) {
            char wmsg[1024] = {0};
        ...
        strncpy( (char*)pVal, pString, count );
        return true;
    }
    ...
    if (type == DBR_STRING) {
        ...
        if (strlen(pString) > MAX_STRING_SIZE) {
            char wmsg[1024] = {0};
        ...
        strcpy( (char*)pVal, pString );
        return true;
    }

    LOG_FINEST( "converting for array" );
    //use local copy for strtok
    size_t ll = strlen(pString) + 1;
    if (bufferLength < ll) {
        if (contents) { delete [] contents; }
        if (bufferLength == 0) {
            bufferLength = 1;
        }
        while (bufferLength < ll) {
            bufferLength *= 2;
        }
        contents = new char[bufferLength];
        memset( contents, 0, bufferLength );
	LOG_FINEST1( "local buffer size to %lu", bufferLength );
    }
    char sep[] = " \t";
    char* lasts = NULL;
    strcpy( contents, pString );

    LOG_FINEST1( "copied ll=%lu", ll );
    ...
    for (i = 0; i < count; ++i) {
        ...
        case DBR_LONG:
    		LOG_FINEST1( "dealing with %d", i );
            LOG_FINEST2( "string at %p val at %p", pStart, (dbr_long_t*)pVal + i );
    ...
    m_PVArray[0].putCount = i;
    LOG_FINEST1( "converted new contents: count=%ld", i );
    return true;
}
void GwString::sendContents( const char* newContents ) {
    ...
    if (!m_stringTypeRead && !convertStringToNative( newContents )) {
        DcsMessage* pMsg = m_pDcsMsgManager->NewStringCompletedMessage(
                    m_name,
                    "convert_failed",
                    m_contentsCurrent );
