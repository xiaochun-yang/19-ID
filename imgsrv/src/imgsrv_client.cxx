...
/**
 * Imported global data defined in imgsrv_main
 *
 */
extern int gListeningPort[];
extern std::string gTempJpgDirectory;
extern int gMaxClientCount;
...

{
	...
	/* local variables */
	size_t handlerIndex = (size_t)arg; /*cast from 'void*' to 'int' loses precision*/
	xos_socket_t connectionServer;
	xos_socket_t *newClient;
	xos_thread_t clientThread;

	/* create the server socket */
	while (xos_socket_create_server( &connectionServer,
			(xos_socket_port_t)gListeningPort[ handlerIndex ] ) != XOS_SUCCESS) {
		LOG_SEVERE1(
				"incoming_client_handler -- Error creating listening socket on port %d.\n",
				gListeningPort[handlerIndex]);
...
/****************************************************************
 *
 * @brief Thread routine for handling a connection using the http protocol.
 * Called by incoming_client_handler routine.
 *
 * Uses xos_socket for receiving and sending http messages.
 * The request URI must be contain a resource which is
 * a command supported by this server (getImage, getHeader or getThumbnail).
 * The query part of the URI must contain the following parameters:
 *
 * fileName: An image file name accessible by the given sessionId
 * userName: User log in name used for authentication.
 * sesionId: Session id generated by the Authentication server
 *           after the user has been authenticated. The session
 *           id is valid for a limited amount of time.
 * sizeX: Horizontal size in pixels to which the image will be scaled.
 * sizeY: Vertical size in pixels to which the image will be scaled.
 * zoom: Zoom factor. 1.0 is the natural zoom level. >1.0 the image
 *       will be zoomed in. <1.0 (but > 0.0) the image will be zoomed out.
 * gray: gray scale. 400 is the natural scale.
 * percentX: Horizontal position of the image to be placed in the middle.
 *           The value is 0.0-1.0 as a ratio of the total horizontal
 *           size of the image. The center of the image is at
 *           percentx=0.5 and percenty=0.5. Bottom left corner
 *           of the image is (0.0, 0.0), top left corner is (0.0, 1.0),
 *           top right corner is (1.0, 1.0) and bottom right corner
 *           is (1.0, 0.0).
 * percentY: Vertical position of the image to be placed in the middle.
 *
 * Note that parameter names are case-sensitive.
 *
 * For example:
 * http://blctlxx:6003/getImage?userName=xxx&sessionId=xxx&fileName=xxx&sizeX=xxx&sizeY=xxx&zoom=xxx&gray=xxx&percenX=xxx&percentY=xxx
 *
 * @param arg Thread routine argument which is a pointer to socket.
 * @return Thread return value.
 *
 ****************************************************************/
XOS_THREAD_ROUTINE http_client_handler(void *arg) {
	...
	try {
		ImgServerHandler::handleRequest(socket);
	} catch (XosException& e) {
