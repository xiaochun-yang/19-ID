...
body DiffImage::loadImage { } {
	...
	# Get the image from the image server
	 if { [catch {
		...
		# Need to check this since the call to httpsmb::geturl is asyncronous
		...
		set url "http://$host:$port/$command?userName=$userName&sessionId=$sessionId&fileName=$fileName"
		...
		if { $mode == "async" } {
			...
			set httpObjName [httpsmb::geturl $url -command "$this loadImageCB"]
			
			httpsmb::wait $httpObjName
...
body DiffImage::drawImage { } {
	 ...
	 if { [catch {
		...
		if { $status != "ok" } {
			...
			httpsmb::cleanup $httpObjName
			return -code error "Error Got http error status $status"

		} elseif { $replycode != 200 } {

			# http response code is not 200
			httpsmb::cleanup $httpObjName
			return -code error "Error Got http error response code from imgsrv"
		...
		image create photo $imageName -palette 8/8/8 -format jpeg -data [encoding convertto $httpObj(body)]
