...
typedef struct {
  ...
  xos_socket_t * socket;/* target stream */
...
METHODDEF(boolean) empty_output_buffer (j_compress_ptr cinfo)
	...
	if (xos_socket_write(dest->socket, (char*)dest->buffer,OUTPUT_BUF_SIZE) != XOS_SUCCESS)
      {
...
#ifdef DEBUG_JPEGWRITE
	if (debugFile) {
		fwrite((char*)dest->buffer, sizeof(char), OUTPUT_BUF_SIZE, debugFile);
	}
#endif
...
term_destination_for_http (j_compress_ptr cinfo)
{
  ...
  if (datacount > 0) {
  	if (xos_socket_write(dest->socket, (char*)dest->buffer, datacount) != XOS_SUCCESS) {
	  sprintf(cinfo->err->msg_parm.s,"xos_failure");
	}

#ifdef DEBUG_JPEGWRITE
	if (debugFile)
		fwrite((char*)dest->buffer, sizeof(char), datacount, debugFile);
#endif
...
term_destination (j_compress_ptr cinfo)
{
  ...
  if (xos_socket_write(dest->socket,warning,OUTPUT_BUF_SIZE) != XOS_SUCCESS) {
	  sprintf(cinfo->err->msg_parm.s,"xos_failure");
  }
