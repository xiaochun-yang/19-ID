...
BOOL RobotControls::serverStillConnected()
{
        ssize_t x;
        char    ch;
        int     arg;
	BOOL    rn;
		...
		arg=0;
                ...
                //******check if the server still being connected******//
                //first change to non blocking mode
                fcntl(sockfd, F_SETFL, O_NONBLOCK);
                x=recv(sockfd, &ch,1,MSG_PEEK);
                LOG_FINEST1("RobotControls::serverStillConnected: The seek value x=%d\n",x);
                if(x>0){
                        LOG_FINEST("Have data, leave it in the buffer\n");
			error_comm = FALSE;
                        rn= TRUE;
                }
                else if(x==0){
                        LOG_WARNING("Get a handle FIN from robot. Robot is offline\n");
			error_comm = TRUE;
                        rn= FALSE;
                }
                else{
				...
				error_comm = FALSE;
                        	...
                        	LOG_WARNING1("communication Handle error %d\n",errno);
				error_comm = TRUE;
                                rn= FALSE;
			}
                }
...
BOOL  RobotControls::CheckConnection()
{
	...
	else
	{
			...
			while(TRUE)
			{
				if(!(ReadRobotStatus(sockfd, status)))
				{
					...
					if(count++>2)
					{
						strcpy( status_buffer, "error can't read robot status, robot may offline");
                                		return TRUE;
					}
					continue;
				}
...
int RobotControls::ConnectRobotServer()
{
        int result, ret;
...
//      address.sin_addr.s_addr = inet_addr("192.168.1.40");
	...
	//set non blocking mode in order to set the timeout
        fcntl(sockfd, F_SETFL, O_NONBLOCK);
...
int RobotControls::readRobotState(int fd)
{
	...
	char status[50]="";
        ...
        for(i=0;;i++)
        {
				...
				/*
                                if(state==0){
					LOG_FINEST( "robot is ready");
					// return TRUE;
				}
				else{
					LOG_FINEST( "robot is not ready");
					// return FALSE;
				}
				*/
				return(state);
...
/////////////////////////////////////////////////////////////////////////////
int RobotControls::disConnectRobotServer()
{
