...
void RobotService::ThreadMethod( )
{
	...
	while (TRUE)
	{
		...
		//check to see if the robot still connected
		if(!ConnectDensoRobot())
		{
			LOG_FINEST( "Yangx 333");
			m_CmdStop =1;
			SetStatus(STOPPED);
		}
		else
			LOG_FINEST( "Yangx 444");
...
BOOL RobotService::disConnectDensoRobot()
{
int	ret;
	ret = m_pRobot->disConnectRobotServer();
	if(ret == 1)
		return TRUE;
	else
		return FALSE;
//        return(m_pRobot->disConnectRobotServer());
}

///////////////////////////////////////////////////////////////////////////////////////////
BOOL RobotService::ConnectDensoRobot()
{
int 	ret;
BOOL    Ret;
char    status_buffer[20];
	ret = m_pRobot->ConnectRobotServer();

	if(ret == -1){
		Ret = FALSE;
//		DcsMessage* pReply = m_MsgManager.NewOperationUpdateMessage("robot is offline", "error");
		strcpy(status_buffer, "error");
		DcsMessage* pReply = m_MsgManager.NewOperationCompletedMessage( m_pCurrentMessage, status_buffer );
		SendoutDcsMessage(pReply);
                LOG_WARNING("RobotService: Robot is offline");
		SetStatus(STOPPED);
	}
	else{
		Ret= TRUE;
		strcpy(status_buffer, "normal");
                DcsMessage* pReply = m_MsgManager.NewOperationCompletedMessage( m_pCurrentMessage, status_buffer );
                SendoutDcsMessage(pReply);
                LOG_WARNING("RobotService: Robot is online");
	}
	return Ret;

//	return(m_pRobot->ConnectRobotServer());
/*
        if(m_pRobot->ConnectRobotServer()) 
        {
                SendoutDcsMessage(m_MsgManager.NewOperationCompletedMessage("robot is ready", "normal"));
                LOG_FINEST("RobotService: Robot is ready");
        }
        else
        {
                SendoutDcsMessage(m_MsgManager.NewOperationCompletedMessage("robot is not ready", "warning"));
                LOG_WARNING("RobotService: Robot is not ready");
