...
puts "server $serverName port: $serverPort"
...
source $SIM_DIR/detectorOperations.tcl
source $SIM_DIR/queue.tcl



set testMode false 
if { $testMode }  {
    puts "WARNING: running in test mode!"
    source tests/pilatusTests.tcl
    DCS::DhsProtocol dcss localhost $serverPort -_reconnectTime 1000 -callback "handle_stoh_messages" -networkErrorCallback "handle_network_error" -hardwareName detector 
    Pilatus detector ::config
    dcss connect
    after 500 testDetector
} else {
    DCS::DhsProtocol dcss $serverName $serverPort -_reconnectTime 1000 -callback "handle_stoh_messages" -networkErrorCallback "handle_network_error" -hardwareName detector 
    Pilatus detector ::config
    dcss connect
}
