   ...
   global TCL_CLIBS_DIR
	...
	foreach directory $env(TCLLIBPATH) {
		...
		if { [file tail $directory] == $BluIceDirectoryName} {
			...
			set TCL_CLIBS_DIR [file join $DCS_DIR tcl_clibs $env(OSTYPE)]
...
if { [catch {
   ...
   set beamline [lindex $argv 0]
	...
	puts "\033]2;sr570 dhs for $beamline on $env(HOST)\07"
...
proc loadSr570Config {} {
   set deviceNames [lindex [config getList sr570.deviceNames] 0]

   foreach name $deviceNames {
      foreach {hostname port} [config getStr sr570.device.$name] {
         Sr570 sr570$name -name $name -hostname $hostname -port $port
      }
   }
...
source $SIM_DIR/sensatronic.tcl
source $SIM_DIR/sr570Class.tcl

loadSr570Config

DCS::DhsProtocol dcss $serverName $serverPort -_reconnectTime 1000 -callback "handle_stoh_messages" -networkErrorCallback "handle_network_error" -hardwareName sr570 

dcss connect
#pollTemperature [config getStr sensatronic.updateRate]
