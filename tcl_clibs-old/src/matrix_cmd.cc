...
int DcsScan2DDataCmd( ClientData cdata, Tcl_Interp* interp, int objc, Tcl_Obj *CONST objv[] )
{
    ...
    if (!strcmp( argv1, "setup" ))
    {
        ...
        if (objc < 8)
        {
            Tcl_WrongNumArgs( interp, 2, objv, "numRow y0 y1 numColumn x0 x1" );
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDoubleFromObj( interp, objv[4], &y1 ) != TCL_OK)
        {
            Tcl_SetResult( interp, "y1 is wrong", TCL_STATIC );
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDoubleFromObj( interp, objv[7], &x1 ) != TCL_OK)
        {
            Tcl_SetResult( interp, "x1 is wrong", TCL_STATIC );
            return TCL_ERROR;
        }
        if (!pScan2DData->setup( numRow, y0, y1, numColumn, x0, x1 ))
        {
