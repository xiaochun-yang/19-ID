    ...
    if (xia_usb_handle == NULL) {
        ...
        while ((p != NULL) && (xia_usb_handle == NULL) && (rv == 0)) {
            ...
            while ((q != NULL) && (xia_usb_handle == NULL) && (rv == 0)) {
				...
				{
                            	if ((rv = usb_set_configuration(xia_usb_handle, xia_usb_device->config[0].bConfigurationValue))<0)
					{
					printf("Usb set configuration error %d\n",rv);
					}
				else
					{
					if ((rv = usb_claim_interface(xia_usb_handle, 0))<0)
						{
						printf("Usb claim interface %d\n",rv);
						}
					}
                        	}
                    }
...
XIA_EXPORT int XIA_API xia_usb2_open(int device_number, HANDLE *hDevice)
{
    ...
    int                            rv = 0;
    static int                    first = TRUE;
    long int 			new_buses=0;
    long int 			new_devices=0;
    int				device_found=0, device_count=0;
    ...
    if (xia_usb_handle == NULL) {
	    ...
	    printf("Initializing usblib library\n");
            usb_init();
            usb_set_debug(0);
	    printf("Finding available USB buses\n");
            new_buses = usb_find_busses();
	    new_devices = usb_find_devices();
            first = FALSE;
	...
	if (p != NULL)
		{
		printf("Found USB bus list\n");
		printf("Scanning for specified USB device..\n");
		}
		...
		device_count++;
			...
			device_found = 1;
				...
				printf("Sending reset to USB device\n");
				// Reset fixed problem of dead XIA DPP after killing this DHS
				// Unclean kill of this DHS left usb interface/XIA DPP in unknown state.  Reset fixes it.
				usb_reset(xia_usb_handle);
				printf("Setting USB device configuration\n");
				if ((rv = usb_set_configuration(xia_usb_handle, xia_usb_device->config[0].bConfigurationValue))<0)
					{
					printf("Usb set configuration error %d attempted to set configuration %d\n",rv,xia_usb_device->config[0].bConfigurationValue);
					}
				else
					{
					printf("Claim USB device interface\n");
					if ((rv = usb_claim_interface(xia_usb_handle, 0))<0)
						{
						printf("Usb claim interface %d\n",rv);
						}
					}
				printf("Connect to USB device complete\n");
                        	}
                    }
                }
                q = q->next;
	...
	printf("Cannot find specified USB device\n");
	printf("Check cat /proc/bus/usb/devices for device.  I bet its not there.!\n");
	exit(0);
	}
...
{
    if (hDevice) {
	printf("Reset USB device, ready for future connections\n");
	usb_reset(xia_usb_handle);
	printf("Releasing interface\n");
	if (usb_release_interface(xia_usb_handle, 0)<0)
		printf("Problem releasing interface\n");
	printf("Closing usb\n");
        if (usb_close(xia_usb_handle)<0)
		printf("Problem closing usb device\n");
        xia_usb_handle = NULL;
        xia_usb_device = NULL;
    }
