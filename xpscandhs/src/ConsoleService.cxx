...
ConsoleService::OperationToMethod ConsoleService::m_OperationMap[] =
{//  name,								immediately, method to call
	{"init_8bm_cons",					FALSE, &ConsoleService::Init8bmCons},
	{"start_monitor_counts",				FALSE, &ConsoleService::StartMonitorCounts},
        {"stop_monitor_counts",                                 FALSE, &ConsoleService::StopMonitorCounts},
	{"read_monitor_counts",					FALSE, &ConsoleService::ReadMonitorCounts},
	{"readAnalog",						FALSE, &ConsoleService::ReadAnalog},
        {"readOrtecCounters",                                   FALSE, &ConsoleService::readOrtecCounters},
	{"ReadOrtecCounters",                                   FALSE, &ConsoleService::ReadOrtecCounters},
	{"move_to_new_energy",                   		FALSE, &ConsoleService::MoveToNewEnergy},
	{"get_current_energy",           			FALSE, &ConsoleService::GetCurrentEnergy},
	{"mono_status",						FALSE, &ConsoleService::MonoStatus},
};

ConsoleService::MotorNameStruct ConsoleService::m_MotorMap[] = {
        {"MFirst",                      MOTOR_FIRST},
	...
	m_pInstantOperation(NULL)
{
    ...
    m_pConsole = new ConsoleSim;

    m_pConsole->SetSleepSemaphore( &m_SemStopOnly );
}
...
BOOL ConsoleService::ConsumeDcsMessage( DcsMessage* pMsg )
{
	LOG_FINEST( "+ConsoleService::ConsumeDcsMessage" );

	//safety check
	if (pMsg == NULL)
	{
		LOG_WARNING( "ConsoleService::ConsumeDcsMessage called with NULL msg" );
		LOG_FINEST( "-ConsoleService::ConsumeDcsMessage" );
		return TRUE;
	}
	...
	switch ( pMsg->ClassifyMessageType() )
   	{
      		...
      		case DCS_ABORT_MSG:  
         		LOG_FINEST("ConsoleService::ConsumeDcsMessage: abort unfinished operations");
...
/*                                                                      
     		 ...
     		 case DCS_ION_CHAMBER_REGISTER_MSG:
         		LOG_FINEST("-ConsoleDhsService::ConsumeDcsMessage: consume ion chamber message.XXXXXX");
         		m_MsgManager.DeleteDcsMessage( pMsg );
         		//consume this message without doing anything
         		return TRUE;
*/                                                                      
		  case DCS_UNKNOWN_MSG:
         		LOG_FINEST( "ConsoleDhs::ConsumeDcsMessage: not recognized, pass on" );
         		return FALSE;
                                                                      
      		  default:
        		break;
   	}
   	LOG_FINEST( "ConsoleDhs::ConsumeDcsMessage: not a message for that can be handled, pass on" );
   	return FALSE;
}

/////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::HandleKnownOperations( DcsMessage* pMsg )
{
...
//            }//if can ignore
         ...
         LOG_FINEST( "-ConsoleService::ConsumeDcsMessage: we consume it" );
         return TRUE;
...
\************************************************************************/
BOOL ConsoleService::HandleKnownMotors( DcsMessage *pMsg )
{
...
////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::registerMotor(DcsMessage* pMsg)
{
...
bool ConsoleService::HandleIonChamberRequest( DcsMessage* pMsg )
{
/*   std::string tmp;
...
////////////////////////////////////////////////////////////////////////////
/* bool ConsoleService::HandleKnownStrings(DcsMessage* pMsg)
{
      m_pInstantMessage = pMsg;
...
*/
////////////////////////////////////////////////////////////////////////////
void ConsoleService::SendoutDcsMessage( DcsMessage* pMsg )
{
	LOG_FINEST1( "ConsoleService::SendoutDcsMessage( %s )", pMsg->GetText( ) );

	if (!ProcessEvent( pMsg ))
	{
		LOG_INFO1( "ConsoleService: no one listening to this message, delete it: %s", pMsg->GetText( ) );
		m_MsgManager.DeleteDcsMessage( pMsg );
	}
}

////////////////////////////////////////////////////////////////////////////
void ConsoleService::ThreadMethod( )
{
	//init the console
	if (m_pConsole == NULL || !m_pConsole->Initialize( ))
	{
	...
	while (TRUE)
	{
		...
		if (m_CmdStop)
		{
			...
			{
				//immediately return
				LOG_INFO( "Console thread emergency exit" );
				return;
			}
			else
			{
				//break the loop and clean up
				LOG_INFO( "Console thread quit by STOP" );
				break;
			}
		}//if stopped
...
//?                     	m_inAborting = false;
				...
				if (m_pCurrentMessage->m_PrivateData >= 0 && m_pCurrentMessage->m_PrivateData < sizeof(m_OperationMap)/sizeof(m_OperationMap[0])){
                                	LOG_INFO("IN ConsoleService::ThreadMethod starting method\n");
                                	(this->*m_OperationMap[m_pCurrentMessage->m_PrivateData].m_pMethod)();
                        	}
                        	else {
                                	LOG_WARNING("ConsoleDhsService::ThreadMethod: should never be here, the match was done before it was put into queue\n");
                        	}
                        	break;
...
//                                      LOG_WARNING1( "GetMotorPosition(): %lf", CurrentPosition );
	...
	//clean up before exit
    	...
    	LOG_INFO("Console thread stopped and EvtStopped set");
    	SetStatus( STOPPED );
}
...
void ConsoleService::WrapConsoleMethod( PTR_CONSOLE_FUNC pMethod )
{
	char status_buffer[Console::MAX_LENGTH_STATUS_BUFFER + 1] = {0};
	//run the function and get result
// LOG_FINEST("WrapConsoleMethod 1");
// LOG_FINEST1("m_pCurrentMessage->GetOperationArgument( ):%s", m_pInstantMessage->GetOperationArgument( ) );
// LOG_FINEST("WrapConsoleMethod 2");

	while (!(m_pConsole->*pMethod)( m_pCurrentMessage->GetOperationArgument( ), status_buffer ))
	{
        	status_buffer[Console::MAX_LENGTH_STATUS_BUFFER] = '\0';
		//update
		...
		if (m_CmdStop)
		{
			LOG_INFO( "ConsoleService::WrapConsoleMethod: got stop during looping" );
...
////////////////////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::MoveToTargetEnergy(double NewEnergy)
{
        ...
        {
              LOG_WARNING("ConsoleService Error: Move Energy failed ");
	      return FALSE;              
        }
	return TRUE;
}
...
///////////////////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::MonoStable()
{
...
BOOL ConsoleService::DcmOnLine()
{
        ...
        {
                LOG_WARNING("Could not get any value from Console, try again");
                return FALSE;
        }
...
///////////////////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::GetEnergy(double* energy, double c_energy)
{

        // Get the current wavelength from xPSCAN
        if( (get_current_energy_from_control(energy)) !=0 )
        {
              LOG_WARNING("ConsoleService Error: Get Current Energy failed ");
	      return FALSE;              
        }
...
///////////////////////////////////////////////////////////////////////////////////////////
BOOL ConsoleService::ConnectX4a()
{
	...
	// Get the current Energy from Console at startup process
//	int channel = m_pCurrentMessage->m_PrivateData;
//      double NewEnergy = m_pCurrentMessage->GetMotorPosition( );
	double NewEnergy;
	...
	if(GetEnergy(&NewEnergy, 12658.0)) // the second parameter in this function is only for debuggin. Just give a any number.
        ...
        else
        {
        	SendoutDcsMessage(m_MsgManager.NewMotorDoneMessage( "energy", NewEnergy, "error" ) );
                LOG_WARNING("ConsoleService: Get current energy is failed");
        }        
	return TRUE;
}

///////////////////////////////////////////////////////////////////////////////////////////
void ConsoleService::Init8bmCons()
{
	WrapConsoleMethod( &Console::Init8bmCons );
}

void ConsoleService::StartMonitorCounts()
{
        WrapConsoleMethod( &Console::StartMonitorCounts );
}

void ConsoleService::StopMonitorCounts()
{
                                        LOG_FINEST("Stop 1");       
	 WrapConsoleMethod( &Console::StopMonitorCounts );
                                        LOG_FINEST("Stop 2");			
}

void ConsoleService::ReadMonitorCounts()
{
        WrapConsoleMethod( &Console::ReadMonitorCounts );
}

void ConsoleService::ReadAnalog()
{
        WrapConsoleMethod( &Console::ReadAnalog );
}

void ConsoleService::ReadOrtecCounters()
{
        WrapConsoleMethod( &Console::ReadOrtecCounters);
}
...
void ConsoleService::GetCurrentEnergy()
{
        WrapConsoleMethod( &Console::GetCurrentEnergy );
}

void ConsoleService::MoveToNewEnergy()
{
        WrapConsoleMethod( &Console::MoveToNewEnergy );
}

void ConsoleService::MonoStatus( )
{
	WrapConsoleMethod( &Console::MonoStatus );
}
